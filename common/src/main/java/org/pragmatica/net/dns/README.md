...
// Useful when sharing with Server or other components
```
...
Share event loop between DNS resolver and TCP server:

```java
// Server creates event loops
var server = Server.server(config, channelHandlers).await().unsafe();

// DNS shares the worker group (access via reflection or expose getter)
// Or create shared event loop externally:
var sharedLoop = new MultiThreadIoEventLoopGroup(4, NioIoHandler.newFactory());
var server = Server.serverWithEventLoop(config, channelHandlers, sharedLoop);
var resolver = DomainNameResolver.domainNameResolver(dnsServers, sharedLoop);
```
