...
import org.pragmatica.net.AsyncCloseable;
...
public interface DomainNameResolver extends AsyncCloseable {
    ...
    static DomainNameResolver domainNameResolver(List<InetAddress> servers) {
        ...
        return new Resolver(
        DnsClient.dnsClient(eventLoop), prepareServers(servers), buildDnsCache(), eventLoop, true);
    }
    ...
    static DomainNameResolver domainNameResolver(List<InetAddress> servers, EventLoopGroup eventLoop) {
        return new Resolver(
        DnsClient.dnsClient(eventLoop), prepareServers(servers), buildDnsCache(), eventLoop, false);
    }
    ...
    private static ConcurrentHashMap<DomainName, Promise<DomainAddress>> buildDnsCache() {
        ...
        domainName("localhost"), InetAddress.getLoopbackAddress(), Duration.ofSeconds(0));
