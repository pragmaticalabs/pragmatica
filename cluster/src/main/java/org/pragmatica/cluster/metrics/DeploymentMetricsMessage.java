...
public sealed interface DeploymentMetricsMessage extends ProtocolMessage {
    ...
    record DeploymentMetricsEntry(
        String artifact,          // Artifact as string
        String nodeId,            // NodeId as string
        long startTime,           // T0: Blueprint change
        long loadTime,            // T1: LOAD received
        long loadedTime,          // T2: LOADED committed
        long activateTime,        // T3: ACTIVATE committed
        long activeTime,          // T4: ACTIVE committed
        String status             // SUCCESS, FAILED_LOADING, FAILED_ACTIVATING, IN_PROGRESS
    ) {}
    ...
    record DeploymentMetricsPing(
        NodeId sender,
        Map<String, List<DeploymentMetricsEntry>> metrics  // artifact -> list of entries
    ) implements DeploymentMetricsMessage {}
