...
public interface RaftNodeExecutor {
     ...
     */
    default void execute(@Nonnull Runnable task) {
        submit(task);
    }
    ...
    static RaftNodeExecutor create() {
        record raftNodeExecutor(ScheduledExecutorService executor) implements RaftNodeExecutor, RaftNodeLifecycleAware {
            @Override
        ...
        return new raftNodeExecutor(newSingleThreadScheduledExecutor());
    }
}
