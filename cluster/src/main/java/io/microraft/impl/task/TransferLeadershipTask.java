...
public class TransferLeadershipTask implements Runnable {
    ...
    private boolean checkLeadershipTransfer(RaftState state) {
        ...
        if (!node.committedMembers().getVotingMembers().contains(targetEndpoint)) {
    ...
    private void transferLeadership(RaftState state) {
        ...
        if (leaderState == null) {
            LOGGER.debug("{} not retrying leadership transfer since not leader...", node.localEndpointName());
        ...
        if (leadershipTransferState == null || !leadershipTransferState.endpoint().equals(targetEndpoint)) {
            LOGGER.error("{} no leadership transfer state for target endpoint: {}", node.localEndpointName(),
                    targetEndpoint.id());
            return;
        }

        int tryCount = leadershipTransferState.incrementTryCount();
        RaftConfig config = node.config();

        if (config.leaderHeartbeatTimeoutSecs() <= tryCount * config.leaderHeartbeatPeriodSecs()) {
            String msg = node.localEndpointName() + " leadership transfer to " + targetEndpoint.id() + " timed out!";
            LOGGER.warn(msg);
        ...
        if (state.commitIndex() < state.log().lastLogOrSnapshotIndex()) {
            LOGGER.warn("{} waiting until all appended entries to be committed before transferring leadership to {}",
                        node.localEndpointName(), targetEndpoint.id());
            scheduleRetry(state);
            return;
        }

        if (tryCount > 1) {
            LOGGER.debug("{} retrying leadership transfer to {}", node.localEndpointName(), targetEndpoint.id());
        } else {
            LOGGER.info("{} transferring leadership to {}", node.localEndpointName(), targetEndpoint.id());
        }

        leaderState.followerState(targetEndpoint).resetRequestBackoff();
        node.sendAppendEntriesRequest(targetEndpoint);

        BaseLogEntry entry = state.log().lastLogOrSnapshotEntry();
        RaftMessage request = node.modelFactory().createTriggerLeaderElectionRequestBuilder()
                                  .setGroupId(node.groupId()).setSender(node.localEndpoint()).setTerm(state.term())
    ...
    private void scheduleRetry(RaftState state) {
        try {
            node.executor().schedule(() -> transferLeadership(state),
                                     node.config().leaderHeartbeatPeriodSecs(), SECONDS);
        } catch (Throwable t) {
            LOGGER.error(node.localEndpointName() + " failed to schedule retry of leadership transfer to "
                    + targetEndpoint.id(), t);
        }
    }
