...
/**
 * Scheduled by {@link RaftNode#query(Object, QueryPolicy, Optional, Optional)} to perform
...
public final class QueryTask implements Runnable {
    ...
    public QueryTask(RaftNode raftNode, Object operation, QueryPolicy policy, long minCommitIndex,
    ...
    public void run() {
        try {
            ...
            LOGGER.error("{}{} query failed", raftNode.localEndpointName(), queryPolicy, t);
    ...
    private void queryWithEventualConsistency() {
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(raftNode.localEndpointName() + " Querying: " + operation + " with policy: " + queryPolicy
                    + " in term: " + state.term());
        }
    ...
    private void queryWithLinearizability() {
        ...
        if (state.logReplicationQuorumSize() > 1) {
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(raftNode.localEndpointName() + " Adding query at commit index: " + commitIndex
    ...
    private boolean verifyRaftNodeStatus() {
        ...
        if (status == INITIAL) {
            LOGGER.debug("{} Won't {} query {}, since Raft node is not started.", raftNode.localEndpointName(),
