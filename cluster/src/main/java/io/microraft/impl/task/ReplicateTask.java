...
public final class ReplicateTask implements Runnable {
    ...
    public ReplicateTask(RaftNode raftNode, Object operation, OrderedFuture future) {
    ...
    public void run() {
        try {
            ...
            LogEntry entry = raftNode.modelFactory().createLogEntryBuilder().setTerm(state.term())
            ...
            LOGGER.error(raftNode.localEndpointName() + " " + operation + " could not be replicated to leader: "
                    + raftNode.localEndpoint(), t);
    ...
    private void prepareGroupOp(long logIndex, Object operation) {
        if (operation instanceof UpdateRaftGroupMembersOp groupOp) {
