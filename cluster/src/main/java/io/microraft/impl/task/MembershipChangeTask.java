...
public final class MembershipChangeTask implements Runnable {
    ...
    public MembershipChangeTask(RaftNode raftNode, OrderedFuture future, RaftEndpoint endpoint,
    ...
    public void run() {
        try {
            ...
            var effectiveMembers = state.effectiveGroupMembers();
            var members = new LinkedHashSet<>(effectiveMembers.getMembers());
            var votingMembers = new LinkedHashSet<>(effectiveMembers.getVotingMembers());

            switch (membershipChangeMode) {
                case ADD_LEARNER :
                    if (members.contains(endpoint)) {
                        String msg = endpoint + " already exists in " + members + " of group " + raftNode.groupId();
                        future.fail(new IllegalArgumentException(msg));
                        return;
                    } else if (members.size() - votingMembers.size() == MAX_LEARNER_COUNT) {
                        String msg = "Cannot add " + endpoint + " to group " + raftNode.groupId() + " as 3rd "
                                + LEARNER;
                        future.fail(new IllegalArgumentException(msg));
                        return;
                ...
                case ADD_OR_PROMOTE_TO_FOLLOWER :
                    if (votingMembers.contains(endpoint)) {
                        String msg = endpoint + " is already a voting member in group " + raftNode.groupId();
                        future.fail(new IllegalArgumentException(msg));
                        return;
                    }
                ...
                case REMOVE_MEMBER :
                    if (!members.contains(endpoint)) {
                        String msg = endpoint + " does not exist in " + members + " of group " + raftNode.groupId();
                        future.fail(new IllegalArgumentException(msg));
                        return;
                    } else if (votingMembers.size() == 1 && votingMembers.contains(state.localEndpoint())
                            && endpoint.equals(state.localEndpoint())) {
                        String msg = "Cannot remove " + endpoint + " from singleton group " + raftNode.groupId();
            ...
            LOGGER.info("{} New group members after {} of {}: {}, voting members: {}", raftNode.localEndpointName(),
                        membershipChangeMode, endpoint.id(), members.stream().map(RaftEndpoint::id).collect(toList()),
                        votingMembers.stream().map(RaftEndpoint::id).collect(toList()));

            RaftGroupOp operation = raftNode.modelFactory().createUpdateRaftGroupMembersOpBuilder()
                                            .setMembers(members)
                                            .setVotingMembers(votingMembers).setEndpoint(endpoint)
            ...
            future.fail(new RaftException("Internal failure", raftNode.leaderEndpoint(), t));
        }
    }

    private boolean verifyRaftNodeStatus() {
        RaftNodeStatus status = raftNode.status();
        if (status == INITIAL) {
