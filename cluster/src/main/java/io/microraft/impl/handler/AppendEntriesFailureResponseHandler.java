 ...
 */
public class AppendEntriesFailureResponseHandler extends RaftNodeStatusAwareTask.AbstractResponseHandler<AppendEntriesFailureResponse> {
    ...
    private boolean updateNextIndex(AppendEntriesFailureResponse response) {
        ...
        if (followerState == null) {
            LOGGER.warn("{} follower/learner: {} not found for {}.", localEndpointStr(), follower.id(), response);
            return false;
        }
        ...
        if (response.getExpectedNextIndex() == nextIndex) {
            ...
            if (nextIndex <= matchIndex) {
                LOGGER.error("{} Cannot decrement next index: {} below match index: {} for follower: {}",
                        localEndpointStr(), nextIndex, matchIndex, follower.id());
                return false;
            }
