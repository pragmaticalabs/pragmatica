...
public class MajorityFailureTest {
    ...
    public void testMajorityFailure() {
        ...
        int term1 = leader.termState().term();
        ...
        // sake of example.
        eventually(() -> {
            RaftNodeReport report = leader.report().join().getResult();
            assertThat(report.role()).isEqualTo(RaftRole.FOLLOWER);
        ...
        RestoredRaftState restoredState = RaftTestUtils.restoredState(followers.get(0));
        RaftStore raftStore = RaftTestUtils.raftStore(followers.get(0));
        ...
        int term2 = newLeader.termState().term();
