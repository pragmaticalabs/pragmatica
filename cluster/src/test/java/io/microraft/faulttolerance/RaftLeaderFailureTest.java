...
public class RaftLeaderFailureTest extends BaseTest {
    ...
    public void testRaftLeaderFailure() {
        ...
        // indices repeatedly
        eventually(() -> {
            RaftNodeReport leaderReport = leader.report().join().getResult();
            long leaderLastLogIndex = leaderReport.log().getLastLogOrSnapshotIndex();
            assertThat(leaderLastLogIndex).isGreaterThan(0);
            for (RaftNode follower : group.nodesExcept(leader.localEndpoint())) {
                RaftNodeReport followerReport = follower.report().join().getResult();
                long followerLastLogIndex = followerReport.log().getLastLogOrSnapshotIndex();
