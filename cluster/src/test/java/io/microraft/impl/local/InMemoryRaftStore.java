...
public final class InMemoryRaftStore implements RaftStore {
    ...
    private static class SnapshotPersistenceState {
        ...
        SnapshotEntry toSnapshotEntry() {
            if (isCompleted()) {
                return new DefaultSnapshotEntryOrBuilder().setTerm(term)
                                                          .setIndex(snapshotIndex)
                                                          .setSnapshotChunks(new ArrayList<>(chunks.values()))
                                                          .setGroupMembersView(membersView)
                                                          .build();
            }
    ...
    public synchronized void persistSnapshotChunk(@Nonnull SnapshotChunk snapshotChunk) {
        if (snapshotPersistenceState == null || snapshotPersistenceState.snapshotIndex != snapshotChunk.getIndex()) {
            snapshotPersistenceState = new SnapshotPersistenceState(snapshotChunk.getTerm(),
                                                                    snapshotChunk.getIndex(),
    ...
    public synchronized RestoredRaftState toRestoredRaftState() {
        var restoredEntries = entries.stream()
                                     .filter(entry -> flushedSnapshotEntry == null || entry.getIndex() > flushedSnapshotEntry.getIndex())
                                     .toList();
