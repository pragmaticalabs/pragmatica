...
public class LocalTransport implements Transport {
    ...
    public void send(@Nonnull RaftEndpoint target, @Nonnull RaftMessage message) {
        if (localEndpoint.equals(target)) {
            throw new IllegalArgumentException(localEndpoint.id() + " cannot send " + message + " to itself!");
        }
        ...
        try {
            ...
            if (maybeAlteredMessage == null) {
                LOGGER.error(message + " sent from " + localEndpoint.id() + " to " + target.id()
                        + " is altered to null!");
                return;
            }
    ...
    public void discoverNode(RaftNode node) {
        RaftEndpoint endpoint = node.localEndpoint();
        ...
        if (existingNode != null && existingNode != node && existingNode.status() != TERMINATED) {
    ...
    public void undiscoverNode(RaftNode node) {
        ...
        nodes.remove(node.localEndpoint(), node);
    }
