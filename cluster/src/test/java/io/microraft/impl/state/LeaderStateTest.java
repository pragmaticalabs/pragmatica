...
public class LeaderStateTest {
    ...
    public void test_initialState() {
        for (RaftEndpoint endpoint : remoteEndpoints) {
            FollowerState followerState = state.followerState(endpoint);
    ...
    public void test_nextIndex() {
        ...
        for (RaftEndpoint endpoint : remoteEndpoints) {
            ...
            state.followerState(endpoint).nextIndex(index);
            indices.put(endpoint, index);
        }

        for (RaftEndpoint endpoint : remoteEndpoints) {
            int index = indices.get(endpoint);
            assertThat(state.followerState(endpoint).nextIndex()).isEqualTo(index);
        }
    }
    ...
    public void test_matchIndex() {
        ...
        for (RaftEndpoint endpoint : remoteEndpoints) {
            ...
            state.followerState(endpoint).matchIndex(index);
            indices.put(endpoint, index);
        }

        for (RaftEndpoint endpoint : remoteEndpoints) {
            long index = indices.get(endpoint);
            assertThat(state.followerState(endpoint).matchIndex()).isEqualTo(index);
        }
    ...
    public void test_matchIndex_nonVotingMembers() {
        int logIndex = 10;
        for (RaftEndpoint endpoint : remoteEndpoints) {
            state.followerState(endpoint).matchIndex(++logIndex);
        }
