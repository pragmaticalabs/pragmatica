...
public class SnapshotTest extends BaseTest {
    ...
    @Test
    @Timeout(value = 300_000L, unit = MILLISECONDS)
    ...
    private void when_followersMatchIndexIsUnknown_then_itInstallsSnapshot(
        ...
        RaftConfig config = RaftConfig.newBuilder()
                                      ...
                                      .setTransferSnapshotsFromFollowersEnabled(transferSnapshotFromFollowersEnabled)
                                      .build();
        group = LocalRaftGroup.start(3, config);
    ...
    public void when_slowFollowerReceivesAppendRequestThatDoesNotFitIntoItsRaftLog_then_itTruncatesAppendRequestEntries() {
        ...
        RaftConfig config = RaftConfig.newBuilder()
                                      .setAppendEntriesRequestBatchSize(appendEntriesRequestBatchSize)
                                      .setCommitCountToTakeSnapshot(commitCountToTakeSnapshot)
                                      .setMaxPendingLogEntryCount(pendingEntryCount)
                                      .build();
        group = LocalRaftGroup.start(5, config);
