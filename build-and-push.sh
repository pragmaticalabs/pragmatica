...
ENTRYPOINT ["sh", "-c", "exec java -XX:+ExitOnOutOfMemoryError \${JAVA_OPTS:--Xmx512m -XX:+UseZGC} -Djdk.virtualThreadScheduler.parallelism=\${VTHREAD_PARALLELISM:-\$(nproc)} -jar /app/aether-node.jar --config=\${CONFIG_PATH:-/app/aether.toml} --node-id=\${NODE_ID:-node-1} --port=\${CLUSTER_PORT:-8090} --management-port=\${MANAGEMENT_PORT:-8080} \${PEERS:+--peers=\$PEERS}"]
DFILE
