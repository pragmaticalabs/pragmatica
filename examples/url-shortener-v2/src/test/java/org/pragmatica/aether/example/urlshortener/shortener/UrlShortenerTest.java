...
class UrlShortenerTest {
    ...
    class VersionTracking {
        @Test
        void shortenResponse_includesVersion() {
            var request = ShortenRequest.shortenRequest("https://example.com/version-test").unwrap();

            urlShortener.shorten(request)
                        .await(TimeSpan.timeSpan(10).seconds())
                        .onFailureRun(() -> fail("Expected success"))
                        .onSuccess(response -> assertThat(response.version()).isEqualTo("2.0"));
        }

        @Test
        void resolveResponse_includesVersion() {
            var url = "https://example.com/version-resolve";
            var shortCode = urlShortener.shorten(ShortenRequest.shortenRequest(url).unwrap())
                                        .await(TimeSpan.timeSpan(10).seconds())
                                        .unwrap()
                                        .shortCode();
