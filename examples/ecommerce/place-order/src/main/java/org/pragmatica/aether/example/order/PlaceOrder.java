...
public interface PlaceOrder {
    ...
    record PlaceOrderRequest(String customerId,
        ...
        public record RawPaymentMethod(String cardNumber, String expiryMonth, String expiryYear,
        ...
        public static PlaceOrderRequest placeOrderRequest(String customerId,
                                                          ...
                                                          String discountCode) {
            return new PlaceOrderRequest(customerId, items, address, payment, shipping,
    ...
    record OrderConfirmation(OrderId orderId,
        ...
        public enum OrderStatus { CONFIRMED, PROCESSING, SHIPPED, DELIVERED, CANCELLED }

        public static OrderConfirmation confirmed(ValidOrder order,
                                                  PriceBreakdown pricing,
                                                  PaymentResult payment,
                                                  Shipment shipment) {
            return new OrderConfirmation(order.orderId(), order.customerId(), order.items(),
                                         pricing, payment, shipment, OrderStatus.CONFIRMED, Instant.now());
        }

        public Money total() { return pricing.total(); }

        public String summary() {
            return String.format("Order %s confirmed. Total: %s. Tracking: %s. Estimated delivery: %s",
                                 orderId.value(), pricing.total(), shipment.trackingNumber(), shipment.estimatedDelivery());
        }
    }
    ...
    record ValidOrder(OrderId orderId,
        ...
        public static Result<ValidOrder> validOrder(PlaceOrderRequest raw) {
            return Result.all(CustomerId.customerId(raw.customerId()),
                         ...
                         .map((customerId, items, address, payment) ->
                                  new ValidOrder(OrderId.generate(), customerId, items, address,
                                                 payment, raw.shippingOption(), raw.discountCode()));
        }
        ...
        private static Result<PaymentMethod> validatePayment(PlaceOrderRequest.RawPaymentMethod raw) {
            return PaymentMethod.paymentMethod(raw.cardNumber(), raw.expiryMonth(), raw.expiryYear(),
                                               raw.cvv(), raw.cardholderName());
        }
    }
    ...
    sealed interface OrderError extends Cause {
        ...
        record OutOfStock(List<ProductId> products) implements OrderError {
            @Override
            public String message() {
                return "Items out of stock: " + products.stream().map(ProductId::value).toList();
            }
        }
...
class PlaceOrderImpl implements PlaceOrder {
    ...
    private Promise<OrderWithPricing> calculateFullPricing(ValidOrder order) {
        var pricePromise = pricing.calculatePrice(CalculatePriceRequest.calculatePriceRequest(order.customerId(), order.items()));
        var shippingPromise = fulfillment.calculateShipping(CalculateShippingRequest.calculateShippingRequest(order.items(), order.shippingAddress()));
    ...
    private Promise<OrderWithPricing> applyDiscount(ValidOrder order, PriceBreakdown basePrice, ShippingQuote shippingQuote) {
        var discountRequest = order.discountCode()
                                   .map(code -> ApplyDiscountRequest.applyDiscountRequest(order.customerId(), basePrice.subtotal(), code))
                                   .or(() -> ApplyDiscountRequest.withoutCode(order.customerId(), basePrice.subtotal()));
    ...
    private Promise<OrderWithPricing> calculateTaxAndBuildPrice(ValidOrder order, PriceBreakdown basePrice,
                                                                ShippingQuote shippingQuote, DiscountResult discount) {
        return basePrice.subtotal()
                        .subtract(discount.discountAmount())
                        .map(subtotalAfterDiscount -> CalculateTaxRequest.calculateTaxRequest(subtotalAfterDiscount, order.shippingAddress()))
                        .async()
    ...
    private Promise<PriceBreakdown> buildFinalPrice(PriceBreakdown basePrice, ShippingQuote shippingQuote,
                                                    ValidOrder order, DiscountResult discount, TaxResult tax) {
    ...
    private Promise<OrderWithReservation> reserveStock(OrderWithPricing context) {
        var reserveRequest = ReserveStockRequest.reserveStockRequest(context.order().orderId(), context.order().items());
    ...
    private Promise<OrderWithPayment> processPayment(OrderWithReservation context) {
        var paymentRequest = ProcessPaymentRequest.processPaymentRequest(
            context.context().order().orderId(),
    ...
    private Promise<OrderComplete> createShipment(OrderWithPayment context) {
        ...
        var shipmentRequest = CreateShipmentRequest.createShipmentRequest(
            order.orderId(), order.items(), order.shippingAddress(), order.shippingOption());
