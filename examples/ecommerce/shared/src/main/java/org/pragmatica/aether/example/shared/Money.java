...
public record Money(BigDecimal amount, Currency currency) {
    ...
    public static Result<Money> money(String amount, String currencyCode) {
        return Result.all(parseAmount(amount), parseCurrency(currencyCode)).flatMap(Money::money);
    }
    ...
    public Result<Money> subtract(Money other) {
        return verifySameCurrency(other).flatMap(_ -> money(amount.subtract(other.amount).max(BigDecimal.ZERO),
                                                            currency));
    }
