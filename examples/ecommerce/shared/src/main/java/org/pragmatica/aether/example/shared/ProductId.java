...
public record ProductId(String value){
    ...
    public static Result<ProductId> productId(String raw){
        return Verify.ensure(raw, Verify.Is::notBlank, INVALID_PRODUCT_ID).map(String::trim)
    ...
    private static boolean isValidFormat(String value){
        return value.length() >= 3 && value.length() <= 20 && value.chars().allMatch(c -> Character.isLetterOrDigit(c) || c == '-');
    }
}
