...
public record GetOrderStatusSlice() implements Slice {
    ...
    public record GetOrderStatusResponse(OrderId orderId, OrderStatus status, Money total,
    ...
    public record ValidGetOrderStatusRequest(OrderId orderId) {
        public static Result<ValidGetOrderStatusRequest> validGetOrderStatusRequest(GetOrderStatusRequest raw) {
            return OrderId.orderId(raw.orderId()).map(ValidGetOrderStatusRequest::new);
        }
    }
    ...
    public List<SliceMethod<?, ?>> methods() {
        return List.of(new SliceMethod<>(MethodName.methodName("getOrderStatus").expect("Invalid method name: getOrderStatus"),
                                         this::execute, new TypeToken<GetOrderStatusResponse>() {}, new TypeToken<GetOrderStatusRequest>() {}));
    }

    @Override
    public List<SliceRoute> routes() {
        return List.of(SliceRoute.get("/api/orders/{orderId}", "getOrderStatus").withPathVar("orderId").build());
    }
    ...
    private Promise<GetOrderStatusResponse> findOrder(ValidGetOrderStatusRequest validRequest) {
        return repository().findById(validRequest.orderId().value())
                           .fold(() -> orderNotFound(validRequest), this::toResponse);
    }
    ...
    private Promise<GetOrderStatusResponse> toResponse(OrderRepository.StoredOrder order) {
        var items = order.items().stream().map(this::toOrderItem).toList();
        return Promise.success(new GetOrderStatusResponse(order.orderId(), order.status(), order.total(),
                                                          items, order.createdAt(), order.updatedAt()));
    }
