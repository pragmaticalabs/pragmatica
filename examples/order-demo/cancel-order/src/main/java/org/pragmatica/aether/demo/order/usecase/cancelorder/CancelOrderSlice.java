...
public record CancelOrderSlice() implements Slice {
    ...
    public record ValidCancelOrderRequest(OrderId orderId, String reason) {
        public static Result<ValidCancelOrderRequest> validCancelOrderRequest(CancelOrderRequest raw) {
            return OrderId.orderId(raw.orderId()).map(orderId -> new ValidCancelOrderRequest(orderId, raw.reason()));
        }
    }
    ...
    public sealed interface CancelOrderError extends Cause {
        record InvalidRequest(String details) implements CancelOrderError {
            @Override public String message() { return "Invalid request: " + details; }
        }

        record OrderNotFound(String orderId) implements CancelOrderError {
            @Override public String message() { return "Order not found: " + orderId; }
        }

        record OrderNotCancellable(String orderId, String reason) implements CancelOrderError {
            @Override public String message() { return "Order " + orderId + " cannot be cancelled: " + reason; }
        }

        record StockReleaseFailed(Cause cause) implements CancelOrderError {
            @Override public String message() { return "Failed to release stock: " + cause.message(); }
        }
    }
    ...
    private static final Set<OrderStatus> CANCELLABLE_STATUSES = Set.of(OrderStatus.PENDING, OrderStatus.CONFIRMED, OrderStatus.PROCESSING);
    ...
    public List<SliceMethod<?, ?>> methods() {
        return List.of(new SliceMethod<>(MethodName.methodName("cancelOrder").expect("Invalid method name: cancelOrder"),
                                         this::execute, new TypeToken<CancelOrderResponse>() {}, new TypeToken<CancelOrderRequest>() {}));
    }

    @Override
    public List<SliceRoute> routes() {
        return List.of(SliceRoute.delete("/api/orders/{orderId}", "cancelOrder").withPathVar("orderId").withBody().build());
    }
    ...
    private Promise<OrderWithContext> findAndValidateOrder(ValidCancelOrderRequest validRequest) {
        return repository().findById(validRequest.orderId().value())
                           .fold(() -> orderNotFound(validRequest), order -> validateCancellable(order, validRequest));
    }
    ...
    private Promise<OrderWithContext> validateCancellable(StoredOrder order, ValidCancelOrderRequest validRequest) {
        if (!CANCELLABLE_STATUSES.contains(order.status())) {
            return new CancelOrderError.OrderNotCancellable(validRequest.orderId().value(),
    ...
    private Promise<StockReleased> releaseStock(String reservationId) {
        return invoker().invokeAndWait(INVENTORY, "releaseStock", new ReleaseStockRequest(reservationId), StockReleased.class);
    }

    private Promise<OrderWithReleases> validateReleases(List<Result<StockReleased>> results, OrderWithContext context) {
        var allSuccess = results.stream().allMatch(Result::isSuccess);
        if (!allSuccess) {
    ...
    private CancelOrderResponse updateAndConfirm(OrderWithReleases context) {
        repository().updateStatus(context.request().orderId().value(), OrderStatus.CANCELLED);
        return new CancelOrderResponse(context.request().orderId(), OrderStatus.CANCELLED, context.request().reason(), Instant.now());
    }
