...
public record Money(BigDecimal amount, Currency currency) {
    ...
    public Result<Money> add(Money other) {
        return verifySameCurrency(other).map(_ -> new Money(amount.add(other.amount).setScale(2, RoundingMode.HALF_UP),
                                                            currency));
    }

    public Result<Money> subtract(Money other) {
        return verifySameCurrency(other).flatMap(_ -> money(amount.subtract(other.amount), currency));
    }

    public Result<Money> multiply(BigDecimal factor) {
        return money(amount.multiply(factor).setScale(2, RoundingMode.HALF_UP),
                     currency);
    }
