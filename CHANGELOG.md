...
- **Grammar generation switched to `peglib-maven-plugin`** — both PostgreSQL (`aether/pg-tools/pg-parser`) and Java 25 (`jbct/jbct-parser`) parsers now regenerate via the `org.pragmatica-lite:peglib-maven-plugin` 0.4.2, configured under a `generate-parser` profile in each module's `pom.xml`. `peglib` itself bumped 0.2.2 → 0.4.2. The old custom generators — `aether/pg-tools/pg-parser/src/test/java/.../GenerateStandaloneParser.java` (test-class entry point) and `jbct/scripts/generate-parser.sh` (shell + embedded Java) — deleted. New regeneration command: `mvn -Pgenerate-parser -pl aether/pg-tools/pg-parser,jbct/jbct-parser generate-sources`. Generated parsers checked in unchanged-in-spirit (CST shape stable across the version bump); both downstream reactors (pg-tools 9 modules, jbct 31 modules) compile and pass tests

### Fixed
- **`aether cluster destroy` leaked every cloud VM** — `ClusterDestroyCommand` only drained nodes + removed the registry entry, never invoking `BootstrapCleanup.cleanup(state)` (which already existed but was only called on bootstrap *failure*). Result: every successful Hetzner bootstrap leaked all 5+ VMs indefinitely, generating recurring cost. Now invokes cleanup before registry-remove; new `--keep-resources` flag preserves the old behavior for debugging. Idempotent against missing state files (warn + proceed)
