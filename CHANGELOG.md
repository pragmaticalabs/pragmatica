...
### Fixed
- **Phantom KV entries wedge consensus writes after cumulative destructive suites** — `ClusterDeploymentManager.cleanupStale{NodeArtifact,NodeRoutes,Slice}Entries` previously filtered by `activeNodes.get()` (topology minus passives), which still included CTM-provisioned phantoms that were unreachable but not yet evicted from `nodeStatesById`. Phantom NodeArtifact/NodeRoutes survived through multiple destructive suites, pointing forwarding at dead addresses; by the time test 13-edge-cases ran, `cluster.apply(...)` on a blueprint publish was timing out with `Promise timed out after 10000ms`. New `healthyNodeSet()` filters by `NodeState.health == HEALTHY` so any SUSPECTED/unreachable peer's KV entries are eligible for eviction. Periodic `reconcile()` now also calls `cleanupStaleNodeArtifactEntries` + `cleanupStaleSliceEntries` (previously only fired on activation), so drift converges without waiting for a leader handoff
