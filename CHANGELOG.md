...
- **Grammar generation switched to `peglib-maven-plugin` (0.2.2 → 0.5.0)** — both PostgreSQL (`aether/pg-tools/pg-parser`) and Java 25 (`jbct/jbct-parser`) parsers now regenerate via the `org.pragmatica-lite:peglib-maven-plugin` 0.5.0, configured under a `generate-parser` profile in each module's `pom.xml`. `peglib` itself bumped 0.2.2 → 0.5.0 (skipping 0.3.x and 0.4.x). The old custom generators — `aether/pg-tools/pg-parser/src/test/java/.../GenerateStandaloneParser.java` (test-class entry point) and `jbct/scripts/generate-parser.sh` (shell + embedded Java) — deleted. New regeneration command: `mvn -Pgenerate-parser -pl aether/pg-tools/pg-parser,jbct/jbct-parser generate-sources`. Two `CstNode.NonTerminal` construction sites in `jbct-format` updated for the 0.5.0 record signature change (added `id` field). Generated parsers checked in; both downstream reactors compile and pass tests
