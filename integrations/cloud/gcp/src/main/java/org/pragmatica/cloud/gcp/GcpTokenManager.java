...
public class GcpTokenManager {
    ...
    private static Promise<TokenResponse> parseTokenResponse(org.pragmatica.http.HttpResult<String> result,
                                                             JsonMapper mapper) {
        if (result.isSuccess()) {
    ...
    private Result<String> signRsa256(String input) {
        return parsePrivateKey(config.privateKeyPem()).flatMap(key -> signWithKey(key, input));
    }

    private static Result<String> signWithKey(RSAPrivateKey key, String input) {
        return Result.lift(t -> new GcpError.AuthError("JWT signing failed", Option.some(t)),
    ...
    static Result<RSAPrivateKey> parsePrivateKey(String pem) {
        return Result.lift(t -> new GcpError.AuthError("Failed to parse private key", Option.some(t)),
