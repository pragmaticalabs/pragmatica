...
record RestS3Client(S3Config config, HttpOperations http) implements S3Client {
    ...
    public Promise<Unit> putObject(String key, byte[] content, String contentType) {
        ...
        return contentMd5(content).map(md5 -> Map.of("content-type", contentType, "content-md5", md5))
                         .flatMap(headers -> AwsSigV4Signer.sign(config.toAwsConfig(),
                                                                 S3_SERVICE,
                                                                 "PUT",
                                                                 url,
                                                                 headers,
    ...
    public Promise<Option<byte[]>> getObject(String key) {
        ...
        return AwsSigV4Signer.sign(config.toAwsConfig(),
                                   S3_SERVICE,
                                   "GET",
                                   url,
                                   Map.of(),
    ...
    private Promise<HttpResult<String>> sendPutRequest(String url,
                                                       byte[] content,
                                                       String contentType,
                                                       Map<String, String> signedHeaders) {
        return contentMd5(content).map(md5 -> buildPutRequest(url, content, contentType, md5, signedHeaders))
                         .async()
                         .flatMap(http::sendString);
    }

    private static HttpRequest buildPutRequest(String url,
                                               byte[] content,
                                               String contentType,
                                               String md5,
                                               Map<String, String> signedHeaders) {
    ...
    private Promise<HttpResult<Unit>> sendHeadRequest(String url, Map<String, String> signedHeaders) {
        var builder = HttpRequest.newBuilder().uri(URI.create(url)).method("HEAD", BodyPublishers.noBody());
    ...
    private static Promise<Unit> checkPutSuccess(HttpResult<String> result, String key) {
        ...
        return S3Error.fromResponse(result.statusCode(),
                                    result.body(),
                                    key)
                      .promise();
    }
    ...
    static Result<String> contentMd5(byte[] content) {
        return liftDigest(content, "MD5").map(Base64.getEncoder()::encodeToString);
    }

    private static Result<byte[]> liftDigest(byte[] content, String algorithm) {
        return Result.lift(t -> new S3Error.NetworkError("Failed to compute " + algorithm, Option.option(t)),
