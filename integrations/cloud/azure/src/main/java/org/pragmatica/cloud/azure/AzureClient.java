...
public interface AzureClient {
    ...
    static AzureClient azureClient(AzureConfig config, HttpOperations http) {
        return new AzureClientRecord(config,
                                     http,
                                     JsonMapper.defaultJsonMapper(),
                                     AzureTokenManager.azureTokenManager(config));
    }
    ...
    static AzureClient azureClient(AzureConfig config,
                                   HttpOperations http,
                                   JsonMapper mapper,
                                   AzureTokenManager tokenManager) {
...
record AzureClientRecord(AzureConfig config, HttpOperations http, JsonMapper mapper, AzureTokenManager tokenManager) implements AzureClient {
    ...
    public Promise<VirtualMachine> createVm(CreateVmRequest request) {
        return tokenManager.getManagementToken(http, mapper)
                           .flatMap(token -> putJson(computeVmPath(request.name()),
                                                     request,
    ...
    public Promise<Unit> deleteVm(String vmName) {
        return tokenManager.getManagementToken(http, mapper)
                           .flatMap(token -> delete(computeVmPath(vmName),
                                                    token));
    }
    ...
    public Promise<VirtualMachine> getVm(String vmName) {
        return tokenManager.getManagementToken(http, mapper)
                           .flatMap(token -> getJson(computeVmPath(vmName),
                                                     VirtualMachine.class,
                                                     token));
    }
    ...
    public Promise<Unit> restartVm(String vmName) {
        var path = config.resourcePrefix()
    ...
    public Promise<VirtualMachine> updateTags(String vmName, Map<String, String> tags) {
        return tokenManager.getManagementToken(http, mapper)
                           .flatMap(token -> patchJson(computeVmPath(vmName),
                                                       Map.of("tags", tags),
                                                       VirtualMachine.class,
                                                       token));
    }
    ...
    public Promise<List<ResourceRow>> queryResources(String kqlQuery) {
        var request = ResourceGraphRequest.resourceGraphRequest(List.of(config.subscriptionId()),
                                                                kqlQuery);

        return tokenManager.getManagementToken(http, mapper)
                           .flatMap(token -> postJson("/providers/Microsoft.ResourceGraph/resources?api-version=" + RESOURCE_GRAPH_API_VERSION,
                                                      request,
                                                      ResourceGraphResponse.class,
                                                      token))
    ...
    private <T, R> Promise<R> postJson(String path, T body, Class<R> responseType, String token) {
        return serializeBody(body).flatMap(json -> http.sendString(buildPost(config.baseUrl() + path,
                                                                             json,
                                                                             token)))
                            .flatMap(result -> parseResponse(result, responseType));
    }
    ...
    private Promise<Unit> postEmpty(String path, String token) {
        return http.sendString(buildPost(config.baseUrl() + path,
                                         "",
                                         token))
    ...
    private <T, R> Promise<R> putJson(String path, T body, Class<R> responseType, String token) {
        return serializeBody(body).flatMap(json -> http.sendString(buildPut(config.baseUrl() + path,
                                                                            json,
                                                                            token)))
                            .flatMap(result -> parseResponse(result, responseType));
    }
    ...
    private <T, R> Promise<R> patchJson(String path, T body, Class<R> responseType, String token) {
        return serializeBody(body).flatMap(json -> http.sendString(buildPatch(config.baseUrl() + path,
                                                                              json,
                                                                              token)))
                            .flatMap(result -> parseResponse(result, responseType));
    }
    ...
    private Promise<Unit> delete(String path, String token) {
        return http.sendString(buildDelete(config.baseUrl() + path,
                                           token))
                   .flatMap(this::checkSuccess);
    }
    ...
    private <T> Promise<T> parseResponse(HttpResult<String> result, Class<T> responseType) {
        ...
        return AzureError.fromResponse(result.statusCode(),
                                       result.body(),
                                       mapper)
                         .promise();
    }
    ...
    private static HttpRequest buildDelete(String url, String token) {
        return authorizedRequest(url, token).DELETE()
                                .build();
    }
