...
public sealed interface DeploymentMetricsMessage extends ProtocolMessage {
    ...
    record DeploymentMetricsPing(NodeId sender, Map<String, List<DeploymentMetricsEntry>> metrics) implements DeploymentMetricsMessage {}

    /// Deployment metrics pong sent by nodes in response to ping.
    /// Contains local deployment metrics.
    record DeploymentMetricsPong(NodeId sender, Map<String, List<DeploymentMetricsEntry>> metrics) implements DeploymentMetricsMessage {}
}
