...
import static org.pragmatica.statemachine.Transition.transition;
...
class StateMachineTest {
    ...
    void stop_clearsAllInstances() {
        ...
        var inMemory = InMemoryStateMachine.inMemoryStateMachine(definition);

        inMemory.create("order-1", "user-1")
                .flatMap(info -> inMemory.create("order-2", "user-2"))
                .flatMap(info -> inMemory.listInstances())
                .await()
                .onFailureRun(Assertions::fail)
                .onSuccess(ids -> assertThat(ids).hasSize(2));
    }
}
