...
class OverLengthPathTest {
    ...
    void parameterRoute_overLengthPath_isAMiss_notADispatch() {
        ...
        assertThat(router.findRoute(DELETE, "/config/nodes/node-1/some.key/anything").isEmpty())
            .as("one segment too many must not reach a handler that binds two")
            .isTrue();
    ...
    void arityZeroRoute_keepsPrefixTolerance_forRemainderConsumingHandlers() {
        var files = Route.<R>get("/static/")
                         .withoutParameters()
                         .to(_ -> Promise.success(new R("file")))
                         .asJson();
