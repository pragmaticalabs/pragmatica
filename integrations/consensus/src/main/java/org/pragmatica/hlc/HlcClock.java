...
public final class HlcClock {
    ...
    private HlcTimestamp advanceForLocalEvent() {
        ...
        if (physicalNow > latestPhysical) {
            ...
            // Local events have no Result boundary to surface failure through. When the 16-bit
            // counter saturates within a single physical millisecond, carry the overflow into the
            // next physical tick (counter reset to 0). This preserves strict monotonicity of the
            // packed value without leaking an exception through the bare-HlcTimestamp contract.
            if (counter > MAX_COUNTER) {
                latestPacked = pack(latestPhysical + 1, 0);
            } else {
    ...
    private Result<HlcTimestamp> mergeRemoteTimestamp(HlcTimestamp remote) {
        ...
        if (counter > MAX_COUNTER) {
            return new HlcError.CounterOverflow(counter, MAX_COUNTER).result();
        }
