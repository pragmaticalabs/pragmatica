...
- `DefaultStreamPublisher` and `StreamWriteRouter` now route by the resolved HRW owner, the rule
  ...
  is present locally.
  [mechanism: unit-level only, pinned by `aether/aether-stream/src/test/java/org/pragmatica/aether/stream/OwnerAuthorizedWritesTest.java` — no multi-node run]
...
- Ownership-lag window (HRW has moved, the leader has not yet committed): a forwarded publish refused
  ...
  [mechanism: unit-level only, pinned by `aether/aether-stream/src/test/java/org/pragmatica/aether/stream/forward/StreamForwardHandlerTest.java` — no multi-node run]
...
- `ReplicationReceiveHandler` refuses (does not apply, does not ack) a batch whose sender cannot be the
  ...
  [mechanism: unit-level only, pinned by `aether/aether-stream/src/test/java/org/pragmatica/aether/stream/replication/ReplicationSenderValidationTest.java` — no multi-node run]
- The `system:cluster-events` publisher now has a forward client and the HRW owner resolver, so
  ...
  replica ring.
  [mechanism: unit-level only, pinned by `aether/aether-stream/src/test/java/org/pragmatica/aether/stream/SystemStreamFactoriesTest.java` — no multi-node run]
