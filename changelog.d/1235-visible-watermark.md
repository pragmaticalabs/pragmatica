...
- **A consumer could act on an event that owner failover then erased.** The ring notified push listeners
  ...
  **durable** (the owner's WAL fsync, or a replica's own WAL write) and **visible**. On the owner,
  ...
  consumer reads) are bounded by visible. Push listeners fire when visible advances, never on the bare
  append. A stream with `minSyncReplicas <= 1` and no WAL makes an event visible as soon as it is
...
- **Replication reads are served from the appended head.** Replica catch-up and survivor pulls now send
  `ReadForward` with a new `catchup` flag (`StreamForwardClient.readRemoteCatchup`), and the source
  answers those reads up to its appended head. Without the flag, a lagging replica could never fetch the
  ...
  replay bound is the appended head.
- **A replica-local read is bounded by the replica's own durability**, not by the owner's min-sync
  acknowledgements, because a replica does not learn the owner's visible position. `[unverified: a
