...
- **#832 review round 4 item A: `acquireLock`'s propagated cause is now typed `SchemaError.LockAcquisitionFailed` (409) instead of a bare `Cause` — not a retry/classification behavior change, since lock refusals short-circuit `executeMigrationFlow` before `classifyFailure` is ever reached [mechanism: `acquireLock(...).flatMap(_ -> runMigration(...))` short-circuits on failure, so `handleMigrationFailure`/`classifyFailure` never run].
