...
- **A batch is not atomic, and its result now says so per event.** `StreamPublisher.publishBatch` and
  ...
  `Published(offset)` is durably in the log; `OutcomeUnknown(cause)` was refused or timed out AFTER it may
  have been appended, so it MAY be in the log and retrying it can duplicate unless the event carries a
  stable key (#1237); `NotAttempted(cause)` never reached the write path, is NOT in the log and is safe to
