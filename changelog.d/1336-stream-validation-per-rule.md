### Fixed (2026-09-20 — #1336: one invalid stream binding silently dropped every binding in the blueprint)
...
- **The rule table.** GATING (the publish is refused with `422` and every failure found, nothing is
  ...
  knobs, unparseable address), #1282's `source-reserved-kind`, and #576's `inert-stream-config-key`/
  `inert-consumer-config-key` [verified: `StreamResourceValidatorPartitionTest`, 10 cases]. A slice using
...
- `StreamValidationFailures` now implements `HttpStatusAware` (`422`). It was not status-aware — moot
  while the deploy path swallowed it, load-bearing now that a gating rule refuses the publish; the
  body-path `conflicting-stream-declaration` refusal (#1066) moves from `500` to `422` with it.
