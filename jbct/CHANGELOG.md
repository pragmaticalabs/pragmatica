...
### Fixed
- slice-processor: **route factory near-misses warn instead of silently skipping validation** (#662). Factory detection for generated route request construction (#605/#643) compared candidate parameter types against record component types by `TypeMirror.toString()` equality, so any spelling difference — a type-use annotation on a factory parameter, a boxed/primitive split — silently fell back to the canonical constructor: the exact validation skip #643 fixed, reproduced with no diagnostic. The match is now semantic — `Types.isSameType` on the mirrors, with boxing accepted in one direction only (a boxed factory parameter over a primitive component: the call site boxes, a total conversion; the reverse would auto-unbox a possibly-null accessor into an NPE→500 and is refused as a near-miss), and the old textual comparison kept as a compatibility fallback for wildcard-containing spellings `isSameType` refuses — so purely cosmetic spellings match again; and a *factory-shaped* method (static, component-count arity, returning `Result` of the record in some spelling) that still fails the match draws a WARNING at its declaration naming the expected component types and the found signature, before the canonical-constructor fallback stands unchanged. Full erasure is deliberately not applied (it would equate `List<String>` with `List<Integer>` and emit a non-compiling factory call). "First declared wins" among multiple matches is untouched — its reliance on `getEnclosedElements()` declaration order is the non-blocking residual noted in #662. No `ENVELOPE_FORMAT_VERSION` bump: routes-source diagnostics only — generated sources byte-identical and manifests structurally identical (timestamp metadata lines only) across unfixed→fixed builds. New `factorymismatch` harness drives the real processor through in-test javac with the module's framework classpath: the near-miss warning asserted (red pre-fix), canonical fallback still compiles, exact-match control stays warning-free, a type-use-annotated factory parameter is now detected and used (red pre-fix), the boxed-parameter-over-primitive-component match is mutation-pinned (deleting the boxing clause turns it red), and the primitive-parameter-over-boxed-component near-miss warning is pinned (red against a bidirectional boxing clause). slice-processor 303 + slice-processor-tests 63 green
- Linter: **JBCT-UC-02's fact-consumer type gate reads the parameter's TYPE node, no longer annotation-stripped source text** (#661). The gate introduced in #656 was meant to fail closed but its regex surgery failed OPEN on two spellings, both granting the exemption on genuinely Request-typed parameters: an annotation argument containing `)` (`@Pattern(regexp = "(ab)+") CreateOrderRequest` — `[^)]*` stops at the first paren inside the argument, leaving `+")` as the parsed "type") and a fully-qualified annotation (`@com.acme.Traced CreateOrderRequest` — `@\w+` consumes only `@com`, leaving `.acme.Traced` as the parsed "type"). The type now comes from the parameter's `Type` CST node via the new `CstNodes.typeSimpleName` (the last name segment's identifier read at token level — FQN types, type annotations, and type arguments all resolve to the erased simple name; a comment inside the type cannot leak in), and the gate DENIES the exemption when no name segment is readable (a published fact is never a primitive array) — fail-closed restored in both directions. Both spellings pinned red-first; plain, FQN-typed, paren-argument and generic fact consumers pinned exempt. jbct-lint 783 green
- slice-processor: **dependency methods inherited from super-interfaces are proxied instead of being invisible to the scan** (#663, sibling of #612's silence family). `collectProxyMethods` walked only `getEnclosedElements()` of the dependency interface, so a method declared on a super-interface never reached the dependency-method scan: the processor emitted no diagnostic, the truncated proxy record still `implements`-ed the interface, and javac then failed on the GENERATED factory ("inventoryService is not abstract and does not override abstract method stockOf(java.lang.String)") — an error anchored in code the user never wrote, with nothing pointing at the actual gap; an inherited non-`Promise` method likewise bypassed the #612 gate and surfaced as the same missing-override noise. The scan now walks the super-interface closure breadth-first — declared methods first, then supers level by level in declaration order, so generation stays deterministic — matching Java interface-inheritance semantics: one proxy entry per signature, the nearest declaration winning an override chain; signatures and generated parameter/return types resolve through `Types.asMemberOf`, so a generic super's `lookup(K)` seen through `extends KeyedQuery<String>` is proxied as `lookup(String)` and its override collapses onto it instead of duplicating. The #612 gate applies to inherited methods identically, with the error naming the declaring interface plus an "(inherited by dependency ...)" clause; only `abstract` methods are proxy candidates (javax.lang.model reports implicit modifiers, so this one check exempts `static`, `default`, AND Java 9+ `private` interface methods at every level — a modifier-skip list let a private super-interface method change generated output for a previously-compiling slice), and explicit re-declarations of `java.lang.Object`'s public methods (`toString()`/`hashCode()`/`equals(Object)`, JLS 9.2) are excluded from proxy candidacy, keyed on type identity via `Types.isSameType` rather than string spelling so an annotated `equals(@Nullable Object)` still counts — the generated record satisfies them itself. Flat dependency interfaces are byte-for-byte unchanged (pinned: exact proxy-record header). No `ENVELOPE_FORMAT_VERSION` bump (GA freeze; recorded in the envelope-versioning history): the affected inputs previously produced non-compiling generated code, so no previously-loadable artifact changes shape. Red-first: the three inheritance tests fail on the unfixed generator with the missing-override diagnostics quoted above. slice-processor 313 + slice-processor-tests 57 green
...
- slice-processor: **request-record validating factories now reach the production path** (#605, owner decision (a)). Path/query params always composed per-param value-object validation, but the RECORD-level `static Result<Self> factory(components...)` was structurally unreachable: pure-body records arrived Jackson-built through the canonical constructor and merged/path/query-built records were constructed with `new` — so url-shortener-v2 shipped a validating factory that only its unit tests ever called. The rule, stated once on the detection helper and applied at every construction site (all eight route shapes, security and plain branches): wherever the generated route constructs the request record, it constructs through the declared validating factory when one exists — `Type.factory(args).mapError(cause -> HttpStatus.BAD_REQUEST.with(cause)).async().flatMap(...)`, the same typed-400 shape path/query parameters already produce — and keeps the canonical-constructor path **byte-identical** when none does (all five pre-existing generated Routes files diff-identical to a pre-change baseline). Pure-body routes re-validate by decomposing the deserialized record through its accessors into the factory. Factory detection: static method returning `Result<Self>` whose parameter types match the canonical components in order; on duplicates the first declared wins with a WARNING naming both. New `factoryslice` fixture covers body-with-factory, body-without (canonical path unchanged), merged path+body, and path-only; mutation-checked (forcing detection to `none` turns 6 of 11 assertions red, the 4 no-factory controls stay green). No `ENVELOPE_FORMAT_VERSION` bump — routes-source only. slice-processor 301 + slice-processor-tests 57 green
- slice-processor: **non-`Promise` dependency methods are a compile error instead of a silent drop** (#612). `extractPromiseTypeArg` never checked that the return type IS `Promise` — it took the first type argument of ANY generic type — so the defect was double: a generic non-Promise method (`Result<T>`, `Option<T>`) was silently misread as `Promise<T>` and generated a wrong-shaped proxy, while a non-generic one (`String`, bare values) vanished from the proxy with nothing at compile time pointing at the cause. The erasure is now checked against `org.pragmatica.lang.Promise`, and every abstract instance method of a dependency interface that fails it reports an ERROR anchored on the method element, stating the rule ("a slice dependency method must return Promise<T> — slice-to-slice calls are remote-capable; Result/Option/bare values are local shapes and belong on a step or leaf") — the same refuse-loudly policy as codec tag collisions and unregistered resources. `default` and `static` members stay exempt (not proxy candidates), pinned in-test. No `ENVELOPE_FORMAT_VERSION` bump: the change adds an error path and tightens which methods generate; the emitted factory/manifest structure is unchanged. slice-processor 303 + slice-processor-tests 46 green — no existing slice was refused, so Promise-only was already the de-facto contract; now it is the enforced one
- Linter: **JBCT-STY-04's sealed-utility detection is structural, no longer a whole-body text scan** — found by the typed-error pilot migration, which the rule promptly mis-flagged. The old predicate was `declText.contains("static ")` over the sealed interface's ENTIRE text, so a nested record's static member — exactly what every migrated cause record now carries as its `FACTORY` — made the enclosing sum type a "utility interface" and drew the bogus demand to add a `record unused()` placeholder: the subtree-attribution bug class, fourth occurrence in this arc (REC-01 double-count, the obligations census, CAUSE-02's nested-interface double emission, now STY-04). Utility now means: at least one static method declared DIRECTLY on the interface (`FileTypeClassifier.directMethods` + `isStatic`), and NO nested type implementing the interface — a sum type already has permitted subtypes, so the 'unused'-record demand never applies to it regardless of static helpers. Strictly narrower by construction (a direct static method implies the old text match), so the fix can only remove findings, and every removal class is a false positive: nested-static attribution, and sum types with static members. The conformant-fixture verdict is unchanged (its `record unused()` now bails via the variant check instead of `hasUnusedRecord`). Two near-miss pins: the distilled migrated-hierarchy shape stays silent; a genuine sealed utility missing `unused` still fires

### Changed
- Linter: **CAUSE pack severities frozen** (#713, versioned against JBCT 5.0.0) — `LintConfig.DEFAULT` now carries CAUSE-01/02/04 at ERROR (the typed-error-lint-spec's proposed table adopted verbatim), CAUSE-03/05/07/08 stay WARNING, CAUSE-06 remains unimplemented and outside the freeze. Basis: the track-A census (38/38 CAUSE-02 true positives, zero false positives, prescribed-shape enums fully silent across the 65-file ticketing corpus) plus the #638 pilot migration. Both reference corpora stay green through explicit migration-bound overrides — the monorepo root `jbct.toml` pins CAUSE-01/02 to warning until #720's migration burns down the 39+291 gated-corpus sites, ticketing pins CAUSE-02 until its held typed-error migration lands — while CAUSE-04 carries no override anywhere (zero hits in both corpora, ERROR bites immediately). The frozen values are pinned executable in `LintConfigCauseFreezeTest` (a change to any entry is a change to the freeze, needing its own ruling), and the flip was verified empirically on three corpora with the built CLI: ticketing + its override → 0 errors; a gated aether module + the root override → 0 errors; the same module under a bare config in an isolated working directory → 74 errors, exactly the census's 10 CAUSE-01 + 64 CAUSE-02 (the isolation matters: `--config` folds per-key over the auto-discovered project chain, so a bare config inside the repo still inherits the root override — designed `PartialConfig` behavior, noted for anyone re-verifying)
