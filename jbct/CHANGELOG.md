...
### Added
- New module `jbct-derive` (#443 phase A) — the `next_step` derivation engine's foundation: schema-v0.1 answer-sheet model + TOML parsing (parse-don't-validate, line-fidelity errors incl. actionable unquoted-date guidance), the entry gate (UNPRICED / UNSCOPED / UNDECOMPOSED / UNTRIAGED / BARE_ILITY / MISSING_SHAPE / MISSING_DOMAIN_SHAPE with the book's vocabulary; UNKNOWN rows pass and propagate), and the `jbct check-sheet` CLI (exit 0 clean / 1 gate errors). SPEC.md synced from the book's working files; the four published runs' answer sheets transcribed from their prose sources into schema form as gate goldens (provenance documented; all four pass the gate clean). Derivation pipeline (prune→press→resolve→verify) is phase B. Apache-2.0, depends on jbct-core only
