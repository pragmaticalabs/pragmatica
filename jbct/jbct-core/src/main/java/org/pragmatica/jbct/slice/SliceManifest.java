...
public record SliceManifest(String sliceName,
    ...
    public record SliceDependency(String interfaceQualifiedName, String artifact, String version, boolean external) {
    ...
    public static Result<SliceManifest> load(Path manifestPath) {
        return org.pragmatica.lang.io.FileOps.readBytes(manifestPath)
    ...
    private static String getPropertyOrEmpty(Properties props, String key) {
        return Option.option(props.getProperty(key)).or("");
    }
    ...
    private static Option<Integer> parseCount(String countStr) {
        return Result.lift1(Causes::fromThrowable, Integer::parseInt, countStr).option();
    }
    ...
    private static void parseIndexedConfigRefs(Properties props,
                                               String countKey,
                                               String prefix,
                                               String typeKey,
    ...
    private static List<String> findInnerClasses(Path baseFile, Path classesDir) {
        return listInnerClassPaths(baseFile, classesDir);
    }

    private static List<String> listInnerClassPaths(Path baseFile, Path classesDir) {
        var parent = baseFile.getParent();
        var simpleName = baseFile.getFileName().toString().replace(".class", "");

        return list(parent).map(paths -> paths.stream()
