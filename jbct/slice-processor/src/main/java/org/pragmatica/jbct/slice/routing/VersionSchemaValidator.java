...
public sealed interface VersionSchemaValidator {
    ...
    static Option<String> checkDuplicateBindKey(int version, List<String> bindKeys) {
        ...
        for (var key : bindKeys) {
            if (!seen.add(key)) {
                return Option.some("Version v" + version
                                  + " declares bind key '" + key
                                  + "' more than once in [v" + version
                                  + ".routes].");
            }
        }
    ...
    static Option<String> checkSingleDefault(Set<Integer> defaultVersions) {
        return defaultVersions.size() > 1
               ? Option.some("Multiple versions declare defaultIfMissing = true: " + defaultVersions.stream()
                                                                                                    .sorted()
                                                                                                    .map(v -> "v" + v)
                                                                                                    .toList()
    ...
    static Option<String> checkSunsetFormat(int version, String sunsetDate) {
        return isIsoDate(sunsetDate)
               ? Option.none()
               : Option.some("Version v" + version
                            + " has an invalid 'sunset' value '" + sunsetDate
    ...
    static Option<String> checkMethodResolved(int version, String bindKey, String methodName, boolean methodResolved) {
        return methodResolved
               ? Option.none()
               : Option.some("Version v" + version
                            + " bind key '" + bindKey
                            + "' resolves to slice method '" + methodName
                            + "', which does not exist. Add the method or specify method = \"...\".");
    }
