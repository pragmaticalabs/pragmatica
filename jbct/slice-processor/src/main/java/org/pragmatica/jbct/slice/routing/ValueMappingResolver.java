...
public final class ValueMappingResolver {
    ...
    public static Map<String, Binding> resolveRecordComponents(TypeMirror recordType) {
        ...
        if (recordType instanceof DeclaredType dt
            && dt.asElement() instanceof TypeElement te
            && te.getKind() == ElementKind.RECORD) {
            for (var component : te.getRecordComponents()) {
                resolve(unwrapOption(component.asType())).onPresent(binding ->
                    result.put(component.getSimpleName().toString(), binding));
            }
        }
    ...
    private static TypeMirror unwrapOption(TypeMirror type) {
        if (type instanceof DeclaredType dt
            && dt.asElement() instanceof TypeElement te
            && te.getQualifiedName().contentEquals("org.pragmatica.lang.Option")
    ...
    private static Option<Binding> bindingFor(TypeElement voType, ExecutableElement method) {
        ...
        return Option.present(new Binding(voType.getQualifiedName().toString(), typeArgs.get(1).toString()));
    }
}
