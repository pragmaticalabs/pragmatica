...
@Mojo(name = "verify-slice", defaultPhase = LifecyclePhase.VERIFY, requiresDependencyResolution = ResolutionScope.COMPILE)
public class VerifySliceMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        if (!result.errors().isEmpty()) {
            throw new MojoFailureException("Slice validation failed with " + result.errors().size() + " error(s)");
        }

        if (failOnWarning && !result.warnings().isEmpty()) {
            throw new MojoFailureException("Slice validation failed with " + result.warnings().size() + " warning(s)");
        }
    ...
    private ValidationResult validateSlice() {
        ...
        Stream.of(manifestResult, sliceManifestResult, scopeResult, sliceScopeResult).forEach(partial -> {
    ...
    private PartialResult checkSliceManifests() {
        var sliceDir = new File(project.getBuild().getOutputDirectory(),
                                "META-INF/slice");
    ...
    private PartialResult checkSliceDependencyScopes() {
        ...
        for (var artifact : project.getDependencyArtifacts()) {
            ...
            if (!Artifact.SCOPE_PROVIDED.equals(scope) && !Artifact.SCOPE_TEST.equals(scope)) {
                errors.add("Slice dependency " + artifact.getGroupId()
