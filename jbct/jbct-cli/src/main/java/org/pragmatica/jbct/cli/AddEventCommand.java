...
@Command(name = "add-event", description = "Add pub-sub event annotations (Publisher + Subscription) to an existing Aether slice project", mixinStandardHelpOptions = true)
public class AddEventCommand implements Callable<Integer> {
    @Parameters(paramLabel = "<event-name>", description = "Event name in kebab-case (e.g., expensive-order)")
    String eventName;

    @Option(names = {"--package", "-p"}, description = "Package for annotations. Starts with '.' = relative to basePackage. Default: .shared.resource")
    String packageOverride;

    @Option(names = {"--dir", "-d"}, description = "Project directory (default: current directory)")
    Path projectDir;

    @Option(names = {"--config-key"}, description = "Override messaging config key (default: messaging.<event-name>)")
    String configKey;
    ...
    private void printResult(Path dir, EventResult result) {
        ...
        System.out.println("  Publisher:  @" + result.adder().eventCamelCase()
                          + "Publisher Publisher<YourEvent> publisher");
        System.out.println("  Subscriber: @" + result.adder().eventCamelCase()
                          + "Subscription on handler method returning Promise<Unit>");
    }
}
