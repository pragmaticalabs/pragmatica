...
@Command(name = "doc", description = "Print the class-level documentation header of a Pragmatica Core class or package", mixinStandardHelpOptions = true)
public class DocCommand implements Callable<Integer> {
    @Parameters(paramLabel = "<name>", description = "Class FQCN, simple name, or package name to document", arity = "1")
    String name;

    @picocli.CommandLine.Option(names = {"--api"}, description = "Append a heuristic '## Public API' section listing top-level method signatures")
    boolean api;

    @picocli.CommandLine.Option(names = {"--jar"}, description = "Explicit core sources jar to read from")
    Path jar;

    @picocli.CommandLine.Option(names = {"--use-version"}, description = "Use this core version from the local Maven repository")
    String useVersion;

    @Override
    public Integer call() {
        return SourceResolver.resolve(name,
                                      Option.option(jar),
                                      Option.option(useVersion))
    ...
    private Result<String> extract(String content) {
        return api
               ? HeaderExtractor.extractWithApi(content, name)
               : HeaderExtractor.extract(content, name);
    }
