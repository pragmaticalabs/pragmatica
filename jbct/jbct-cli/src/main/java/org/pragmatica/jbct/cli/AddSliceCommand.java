...
@Command(name = "add-slice", description = "Add a new slice to an existing Aether slice project", mixinStandardHelpOptions = true)
public class AddSliceCommand implements Callable<Integer> {
    @Parameters(paramLabel = "<sliceName>", description = "Name of the new slice (e.g., Analytics)")
    String sliceName;

    @Option(names = {"--package", "-p"}, description = "Override the slice package (default: basePackage.sliceNameLowercase)")
