...
@Command(
 name = "fix-slice",
 description = "Fill missing manifest-derived config (slices/<Name>.toml files and resources.toml sections). "
              + "Add-only and idempotent: never overwrites author content. Requires a prior 'mvn compile' so the "
              + "slice manifests under target/classes/META-INF/slice exist.",
