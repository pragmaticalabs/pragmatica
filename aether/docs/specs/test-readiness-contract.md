...
1. **Generation snapshot is converged** — `/api/cluster/generation` reports `core.members[]` with cardinality ≥ `NODE_COUNT - 1`. (The N-1 floor mirrors Property 3 and tolerates the seed-node special case where the leader's own `NodeLifecycleKey` isn't written until after first KV quiesce — see §6 follow-up.)
...
- **Seed-node lifecycle write** (RC2) — the seed node (initial leader at cluster bootstrap) does not write its own `NodeLifecycleKey` entry to KV-Store. Effect: the seed node is missing from `/api/nodes/lifecycle` (mass form) and from `/api/cluster/generation` `core.members[]` even though it's operational, elected leader, and present in `/api/cluster/topology` `coreNodes[]`. Property 1 of §1.1 therefore floors at `N-1` rather than strict `N`. The underlying bug should be fixed in RC2 so that the seed node writes its `NodeLifecycleKey` on transition to ON_DUTY just like joining nodes do, and Property 1 can be tightened back to strict equality. Code sites: `aether/aether-deployment/src/main/java/.../LifecycleWriter.java` + the seed-node bootstrap path in `aether/node/src/main/java/.../AetherNode.java`.
