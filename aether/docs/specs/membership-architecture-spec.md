...
|---|---|---|---|---|---|---|---|---|---|
| S01 | JOINING-window kill | Node killed during JOINING (before SWIM HEALTHY) | U | CONN→EVI | UNK→UNREACH within 5-7s | JOINING → DECOMMISSIONED (via TransportUnreachable, ungated) | Put(DECOMMISSIONED) within ≤15s | `/api/status` excludes peer within ≤15s | `aether/tests/integration/suites/02-chaos/test-joining-window-kill.sh` |
...
| S14 | Transport-only failure | QUIC connection drops but SWIM still HEALTHY (rare, e.g., specific protocol issue) | H (unchanged) | CONN→EVI | REACH→UNREACH (within period) | ON_DUTY → DECOMMISSIONED (Transport cell, gated by aggregator UNREACH) | Put(DECOMMISSIONED) within ≤15s | excluded within ≤15s | covered by S01 family of tests |
