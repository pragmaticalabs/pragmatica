...
| S02 | ON_DUTY single non-leader kill | Steady-state non-leader killed | H→F | CONN→EVI | REACH→UNREACH | ON_DUTY → DECOMMISSIONED (via Transport, gated by aggregator UNREACH) | Put(DECOMMISSIONED) within ≤25s | `/api/status` excludes within ≤25s | covered by `test-kill-node.sh` (existing, retimed) |
| S03 | ON_DUTY two simultaneous non-leader kills | Two non-leaders killed in <1s | H→F (both) | CONN→EVI (both) | REACH→UNREACH (both) | Both ON_DUTY → DECOMMISSIONED in parallel | Put(DECOMMISSIONED) × 2 within ≤25s | `/api/status` excludes both within ≤25s; `pick_non_leader(count=2)` succeeds | covered by `test-kill-multiple.sh` (existing, retimed) |
| S04 | Brief transport flap < 5s | Transient network blip, peer reconnects within 5s | H (unchanged) | CONN→EVI→RC | REACH→UNK→REACH (within period) | nop (gate blocks transient) | none | no change | covered by FSM unit test `gateBlocks_transportUnreachableIsNop_zeroWrites` |
| S05 | 2-vs-3 partition (majority side) | Network partition, this side has 3 nodes | H→F for 2 minority peers | CONN→EVI for 2 | REACH for 3 in-side, UNREACH for 2 partitioned (strict-majority threshold 3 met by 3 in-side observers — Rabia is CFT, the aggregator cannot distinguish kill from partition) | ON_DUTY → DECOMMISSIONED for the 2 partitioned peers (gate fires). In parallel: minority side `SelfDrainCoordinator` trips (each minority node sees ≥3 unreachable peers) | Majority: Put(DECOMMISSIONED) × 2. Minority: none (self-drain bypasses KV) | Majority `/api/status` excludes 2 partitioned peers within ~15-25s. Minority HTTP returns 503 within ~9s; processes exit within ~38s | `aether/tests/integration/suites/12-network/test-partition-quorum-gate.sh` |
| S06 | Partition heal | Connection restored after S05 (before minority processes exit) | F→H | EVI→RC | UNREACH→REACH | Majority side: nop (entries already DECOMMISSIONED — terminal). Minority side: still mid-drain (uninterruptible) — exits anyway, then orchestrator restart re-introduces them as fresh nodes via CTM (S20 path) | none on heal itself; restarted minority writes Put(ON_DUTY) per peer (S20) | All 5 nodes ON_DUTY within ≤60s after minority restart completes (CTM provisioning + cold-boot) | same test as S05 (rejoin phase) |
...
| S08 | Drain timeout | Drain doesn't complete within hard deadline | * | * | * | DRAINING → FAILED_DRAIN | Put(FAILED_DRAIN) | `/api/status` shows FAILED_DRAIN | covered by FSM unit test `operatorDrain_then_awaitDrainAckFailure_writesFailedDrain` |
| S09 | Drain during partition | Operator drains while node is partitioned | * | * | UNREACH | DRAINING but drain protocol may not complete; FAILED_DRAIN on timeout | Put(DRAINING), Put(FAILED_DRAIN) on timeout | DRAINING then FAILED_DRAIN | same code path as S08 (no dedicated test) |
...
| S11 | Restart inside revival TTL | Killed node restarts with same NodeId, new incarnation, within TTL window | F→H (new incarnation) | EVI→CONN | UNREACH→REACH | DECOMMISSIONED stays terminal (H.4: revival path removed; the historical TTL/refractory defences are obsolete now that SWIM is authoritative for "alive"); new incarnation joins as fresh JOINING via CTM | none (DECOMMISSIONED is terminal); new slot via CTM | new KSUID identity in `/api/status`; old DECOMMISSIONED entry persists | covered by FSM unit test `decommissioned_swimHealthy_isNop_hSeriesNoRevival` |
| S12 | Restart outside revival TTL | Same as S11 but after DecommissionedAtomGc removed the entry | * | * | * | UNTRACKED → ON_DUTY direct via SwimHealthy (SWIM-driven path); CTM-mediated rejoin writes JOINING then ON_DUTY | SWIM-driven path: Put(ON_DUTY). CTM-mediated path: Put(JOINING), Put(ON_DUTY) | clean rejoin | covered by FSM unit test `untracked_swimHealthy_writesOnDuty_legacyConsumers` |
| S13 | SWIM-only failure (transport OK) | SWIM marks peer FAULTY but QUIC connection OK | H→F | CONN (unchanged) | REACH (aggregator from QUIC observations) | nop (gate blocks: aggregator says REACH) | none | no change | covered by FSM unit test `quorumSnapshotReachable_swimFaultyOnOnDuty_isNop` |
...
| S15 | Cold-start formation | 5 nodes start simultaneously | U→H | (none)→CONN×4 | UNK→REACH within 2 periods | UNTRACKED → ON_DUTY direct for each peer (SwimHealthy-driven; SWIM only emits on state-change, so JOINING is never observed via SWIM and is skipped) | Put(ON_DUTY) per peer (JOINING is reserved for CTM-slot-claim path) | all 5 ON_DUTY within ≤30s | covered by existing `test-cluster-formation.sh` |
| S16 | Cold-start + simultaneous kill | One node killed during cold-start (before its SWIM HEALTHY) | U (stays U for killed) | CONN→EVI (killed) | UNK→UNREACH (no SWIM context yet) | If SlotClaimed fired before kill: Provisioning → Joining → DECOMMISSIONED via TransportUnreachable. If SlotClaimed did not fire: UNTRACKED stays UNTRACKED. SWIM observations during Provisioning (rare race: SWIM gossip before SlotClaimed propagation) are nop — defensive, the slot lifecycle owns advancement | If SlotClaimed fired: Put(DECOMMISSIONED). Otherwise: no writes | killed peer absent from `/api/status` | covered by FSM unit test family in `ClusterMembershipReducerTest` (Provisioning + transport / SWIM cells) |
| S17 | Aggregator quorum lost — two sub-cases | (a) snapshot present but all UNKNOWN (cluster running, observers silent); (b) no snapshot at all (cold-start, leader just elected, aggregator first emission pending) | * | * | (a) UNK (insufficient observers); (b) no snapshot — gate falls back to `ALWAYS_CONFIRMED` | (a) nop for transport-gated cells (`isConfirmedUnreachable` returns false on UNKNOWN). (b) Permissive: gate returns true → reducer trusts the upstream event source (typically transport-unreachable will trigger decommission). SWIM cells (SwimDeparted, SwimFaulty during JOINING) unchanged in both sub-cases | (a) none. (b) Whatever the upstream event drives — typically Put(DECOMMISSIONED) | (a) unchanged. (b) peer dropped from `/api/status` per the upstream event | covered by FSM unit tests in `ReducerAggregatorGateTest` (gate-snapshot-UNKNOWN + gate-no-snapshot variants) |
| S18 | Leader kill + re-election | Current leader killed | H→F (from new leader's view) | CONN→EVI (from new leader's view) | UNREACH (eventually, after new leader's aggregator catches up) | New leader writes (OnDuty, SwimFaulty/Transport) → DECOMMISSIONED for old leader | Put(DECOMMISSIONED) for old leader within ≤25s post-election | new leader visible in `/api/status`; old leader excluded | covered by existing `test-kill-leader.sh` |
| S19 | Quorum-loss → self-drain | Node loses contact with ≥ `(N/2)+1` peers (strict majority — Rabia is CFT, not BFT) for ≥8s | * | EVI for majority | UNK or UNREACH | none from this node's FSM (it self-drains, doesn't write) | none (self-drain bypasses KV) | this node's HTTP returns 503/refused within ~9s; process exits within ~38s | `aether/tests/integration/suites/02-chaos/test-self-drain-quorum-loss.sh` |
| S20 | Self-drain → restart → rejoin | After S19, orchestrator restarts the node | U→H | (none)→CONN×N | UNK→REACH | UNTRACKED → ON_DUTY direct (SWIM-driven; same path as S15); CTM-mediated rejoin writes JOINING then ON_DUTY | SWIM-driven path: Put(ON_DUTY). CTM-mediated path: Put(JOINING), Put(ON_DUTY) | `/api/status` shows node ON_DUTY within ≤60s | same test as S19 (rejoin phase) |

---

### 16.1 Self-Drain Protocol

A node self-drains when it cannot reach `(N/2)+1` peers (counting itself) for ≥ `triggerThreshold` seconds — strict majority, Rabia is CFT (not Byzantine, so `⌈N/2⌉+1` is not required). Self-drain is **uninterruptible once started** — quorum restoration mid-drain does not abort.
...
4. When tracker reaches zero in-flight, OR timeout fires: invoke `jvmExit` (production: `Runtime.getRuntime().halt(2)`; Forge / single-JVM test runtimes supply a hook that signals the supervising driver instead)
...
- `jvmExit` is pluggable: production hardcodes `halt(2)`; in-JVM test runtimes (Forge) MUST supply a different hook to avoid taking down the whole test JVM

**Why self-drain alongside cross-side decommissioning?**

The aggregator cannot distinguish a kill (peer dead, won't return) from a partition (peer alive on the other side) — both produce identical observations from any one side. Rather than picking one default behavior:

- **Majority side** observes the minority as UNREACHABLE with strict-majority quorum and writes `Put(DECOMMISSIONED)` for them via the ON_DUTY transport-gated cell (S05). This is fast (~15-25s) and removes the failed peers from the operator-visible cluster.
- **Minority side** independently observes itself as below quorum and self-drains, exiting the process within ~38s. This bounds the partitioned-away peers' lifetime even if the partition is permanent.

Both mechanisms converge to the same end-state (peers gone, KV consistent). Partition heal before minority exits is handled by S06: the minority is already mid-drain (uninterruptible), exits anyway, and is re-introduced by orchestrator restart as a fresh node via CTM (NodeId churn — DECOMMISSIONED entries are terminal per H.4).

The historical "do not decommission across a partition" intent is moot in a CFT model: the aggregator has no oracle to tell partition from kill, and the cost of the symmetric design (a partitioned-away node briefly seeing itself decommissioned on heal, before its halt completes) is bounded and clean.
