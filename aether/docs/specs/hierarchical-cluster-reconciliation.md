...
| H-T02 | MISSING (strengthened live proof pending) | `RabiaReorderedDeliveryTest` pins duplicate notification counts and gaps. The earlier `HierarchicalDecisionReplayTest` passed locally and again in the completed matrix at `1bf7ff14d`, proving bounded replay without visible value rollback. | The strengthened fixture now filters actual probe-key Decisions, requires phase retention then eviction on the receiving engine, and counts real live ValuePut notifications through a transparent observer (excluding snapshot replay). That newer fixture has not yet been compiled/executed; neither the earlier pass nor sender-side eviction proves this added live notification-count assertion. Rerun it and final-head CI. |
...
- `hierarchy-review-final-forge-matrix.log`: source `1bf7ff14d`, BUILD SUCCESS,
  ...
  **earlier** Decision replay fixture, worker runtime replay, core resize and leader
  observation grace and `SliceInvocationTest` (nine cases). The newer receiver-eviction/live-notification replay fixture still
  requires its own rerun.
...
The new core-candidate retirement correction awaits its unit/build/Forge gates. The latest
`SliceVersionLifecycleTest` locality correction also awaits rerun: version
introspection describes locally hosted slices, so the fixture now queries an actual ACTIVE
host instead of an arbitrary management node. Final-head runtime CI remains required after
