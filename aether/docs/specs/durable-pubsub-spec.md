...
- **Rebuild is one operator procedure of three ORDERED steps (#1304)**: CAPTURE the replay range from the group cursor (per source partition, the earliest retained offset through the head) → RESET the store to a new generation in ONE indivisible step (generation advanced, read model cleared with the generation slot preserved, REBUILDING over the captured range) → REWIND the group cursor over that range. The generation in the idempotency key makes prior-pass claims inert — without it, retained claims from the first pass would dedup the entire replay into a no-op (review finding 3). Capture touches nothing, and the rewind comes last so no replay delivery reaches the old generation. `[mechanism: Projection.rebuild chains capture → ProjectionStore.resetToNewGeneration → rewind; order pinned by ProjectionTest.rebuild_capturesFirst_resetsInOneStep_andRewindsLast, which records the store's state at each cursor call]`
  ...
  - **Limits, stated.** (i) A refused live delivery retries within its §6 budget; past it, it is dead-lettered and applies when redriven, after the replay. (ii) Any replay offset that completes without reaching the store — an attempt that met `IN_PROGRESS` and exhausted its retries into the DLQ, a poison fold, an undecodable event quarantined raw, a subscriber that does not route every event to `onEvent` — is SKIPPED on the group's committed cursor, never on a later delivery: the consumer commits past an event only once it is acknowledged or dead-lettered (`ConsumerRuntimeState.completeDeadLetter` → `advanceCursor`), so `Projection.onCursorCommitted(partition, cursor)` moves the partition to `max(next, cursor)` and takes it LIVE once the cursor passes its head. Commits count only after the rebuild's rewind, since an earlier commit reflects the old cursor position. That event is absent from the rebuilt model until its DLQ entry is redriven. Accepting any higher delivered offset instead would let an early or zombie delivery skip, and so lose, every replay offset below it. `[mechanism: ProjectionStore.cursorCommitted / replayRewound; pinned by ProjectionTest.IdempotencyGuard.rebuilding_skipsADeadLetteredReplayOffset_onTheCommittedCursor, rebuilding_goesLive_whenTheCommittedCursorPassesTheHead and rebuilding_ignoresACursorCommitReportedBeforeTheRewind]` The committed-cursor skip is the facade half; it cures the stall once #1333 wires the runtime call. `[unverified: NO PRODUCTION CALLER — the runtime call to Projection.onCursorCommitted is not wired, because a Projection instance is not reachable from the consumer runtime and the replay cursor (capture/rewind) is the unbuilt D3 surface, both tracked by #1333; and at rc4 the durable dispatch (StreamConsumerManager.deliverTopicEvent) delivers no MessageContext, tracked by #1295 / PR #1310]`
