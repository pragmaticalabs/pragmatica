...
- **Durable — `publish(T): Promise<Unit>`:** resolves when the event is durably appended (owner + `min-sync − 1 ≥ 1` peer acks). Publisher latency is bounded by replication latency and is **independent of subscribers** — subscriber processing is severed from the publisher's lifetime by the log. A publisher may complete and exit with consumers hours behind; nothing dangles. The promise resolves three ways (#1236): **success** = in the log at the floor; **failure** = NOT in the log (e.g. `NOT_ENOUGH_REPLICAS`, checked before the append), so a retry is a new publish; **outcome-unknown** (`PublishOutcomeUnknown`) = the owner appended but the floor was not confirmed (e.g. the peer-ack timeout), so the event MAY be in the log and visible to consumers — retry only with the same message ID (`publish(T, idempotencyKey)`, §8), or dedup sees two distinct events. There is no abort or tombstone protocol that could turn an outcome-unknown into a clean failure.
...
- Every durable event carries a publisher-assigned `messageId` plus `(topic, partition, offset)`; `messageId` survives DLQ redrive (§9), offsets do not. `publish(T)` mints a fresh KSUID per call; `publish(T, idempotencyKey)` (#1237) uses the caller's key as the `messageId`, so a retry of the same logical event carries the same identity. **A retry after an outcome-unknown result (§5) is dedup-safe only through the keyed overload with the same key** — the keyless form writes a second copy under a new `messageId` that no dedup here can match. `[mechanism: DurableTopicPublisher.publish(T, String) sets TopicEventEnvelope.messageId from the key; TypedPublisher forwards it]`
