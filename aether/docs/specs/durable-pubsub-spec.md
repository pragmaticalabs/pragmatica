...
| Duplicate exposure | None (a message is delivered 0 or 1 times). | On redelivery: crash/timeout after processing-before-ack, and cursor-commit lag after ack (§7). On **publisher retry**: a retry after an outcome-unknown result (§5) appends a second copy — under the same `messageId` only through `publish(T, idempotencyKey)`, under a fresh one otherwise (#1237). Bounded by the idempotency aspect where applied, within its stated limits (§8). |
...
- **Durable — `publish(T): Promise<Unit>`:** resolves when the event is durably appended (owner + `min-sync − 1 ≥ 1` peer acks). Publisher latency is bounded by replication latency and is **independent of subscribers** — subscriber processing is severed from the publisher's lifetime by the log. A publisher may complete and exit with consumers hours behind; nothing dangles. The promise resolves three ways (#1236): **success** = in the log at the floor; **failure** = NOT in the log (e.g. `NOT_ENOUGH_REPLICAS`, checked before the append), so a retry is a new publish — one known exception: a WAL fsync failure after the ring append reports failure for an event already visible in the owner's ring (#1235); **outcome-unknown** (`PublishOutcomeUnknown`) = the owner appended but the floor was not confirmed (e.g. the peer-ack timeout), so the event MAY be in the log and visible to consumers — retry only with the same message ID (`publish(T, idempotencyKey)`, §8), or dedup sees two distinct events. There is no abort or tombstone protocol that could turn an outcome-unknown into a clean failure.
...
- Every durable event carries a publisher-assigned `messageId` plus `(topic, partition, offset)`; `messageId` survives DLQ redrive (§9), offsets do not. `publish(T)` mints a fresh KSUID per call; `publish(T, idempotencyKey)` (#1237) uses the caller's key as the `messageId`, so a retry of the same logical event carries the same identity. **A stable key is a necessary condition for a dedup-safe retry after an outcome-unknown result (§5), not a sufficient one** — the keyless form writes a second copy under a new `messageId` that no dedup here can match. `[unverified: messageId is not yet delivered to subscribers — #1295]` **Limitation:** durable topics route keyless events round-robin, so a keyed retry may land on a different partition from the first copy and be dispatched concurrently with it — exception (i) below, which the in-process claim backend does not cover. The retry is not rerouted by key. `[mechanism: DurableTopicPublisher.publish(T, String) sets TopicEventEnvelope.messageId from the key; TypedPublisher forwards it]`
...
- **Rebuild requires the §8 claims guard (#1304 X2).** An unguarded projection's deliveries carry no position and no dedup key, so its replay could be neither ordered nor deduplicated; `rebuild()` refuses it up front with `UnguardedRebuild`, before anything is touched. `[mechanism: Projection.rebuild checks the guard before capture; pinned by ProjectionTest.rebuild_ofAnUnguardedProjection_isRefusedBeforeAnythingIsTouched]`
- **Rebuild is one operator procedure of three ORDERED steps (#1304)**: CAPTURE the replay range from the group cursor (per source partition, the earliest retained offset through the head) → RESET the store to a new generation in ONE indivisible step (generation advanced, read model cleared with the generation slot preserved, REBUILDING over the captured range) → REWIND the group cursor over that range. The generation in the idempotency key makes prior-pass claims inert — without it, retained claims from the first pass would dedup the entire replay into a no-op (review finding 3). Capture touches nothing, and the rewind comes last so no replay delivery reaches the old generation. `[mechanism: Projection.rebuild chains capture → ProjectionStore.resetToNewGeneration → rewind; order pinned by ProjectionTest.rebuild_capturesFirst_resetsInOneStep_andRewindsLast, which records the store's state at each cursor call]`
  - **Reset is one step, so nothing can land between the bump and the clear.** Were they separate, a write admitted under the new generation between them would be wiped by the clear while its §8 claim stayed DONE, and the replay would be suppressed — the event silently missing (#1304 R5, R6). `[mechanism: resetToNewGeneration is indivisible with every fenced write; pinned by ProjectionTest.IdempotencyGuard.rebuild_liveWriteAtTheNewGeneration_isNotLost and rebuild_staleGenerationRetryAtTheNewGeneration_isNotLost, both RED when the store splits bump and clear]`
  - **In-flight folds are fenced (#1298):** every read-model write carries the generation its fold was keyed under (the guarded path uses its claim's generation, never a fresh read) and is refused, retryably, once that generation is no longer current. `[mechanism: generation-fenced ProjectionStore.write; pinned by ProjectionTest.IdempotencyGuard.rebuild_inFlightGuardedFold_appliesExactlyOnce, rebuild_foldWhoseClaimSpansTheRebuild_appliesExactlyOnce and ProjectionTest.rebuild_inFlightUnguardedFold_doesNotWriteIntoTheResetModel]`
  - **While REBUILDING the replay is the only writer, in offset order.** A write at the new generation is admitted only when its delivery position is its partition's next replay offset; a position below it is acknowledged as already applied without writing (a redelivery after a crash between write and claim finalize); a replay delivery whose claim is already DONE (a duplicate messageId inside the range, #1237) completes without a write and still advances its partition (#1304 X1); anything else — a later offset, or a positionless write while any partition is replaying — is refused as retryable `Rebuilding`. Each captured offset is therefore applied at most once and in order. **LIVE is per partition (#1304 X3):** a partition passes its captured head and admits its live writes at once, without waiting for slower partitions; a partition outside the range is live from the start. `[mechanism: replay-order admission in ProjectionStore.write and ProjectionStore.markReplayed; pinned by ProjectionTest.IdempotencyGuard.rebuilding_refusesAnOutOfOrderLiveWrite_andBuildsInOffsetOrder (an order-sensitive fold), rebuilding_redeliveryOfAnAppliedReplayOffset_isNotReapplied, rebuilding_refusesAPositionlessWrite, rebuilding_aDeduplicatedReplayOffset_stillAdvancesTheReplay, rebuilding_aDeduplicatedHeadOffset_takesThePartitionLive and rebuilding_aFinishedPartition_admitsLiveWrites_whileAnotherStillReplays]`
  - **Limits, stated.** (i) A refused live delivery retries within its §6 budget; past it, it is dead-lettered and applies when redriven, after the replay. (ii) Any replay offset that completes without reaching the store — an attempt that met `IN_PROGRESS` and exhausted its retries into the DLQ, a poison fold, an undecodable event quarantined raw, a subscriber that does not route every event to `onEvent` — is SKIPPED on the group's committed cursor, never on a later delivery: the consumer commits past an event only once it is acknowledged or dead-lettered (`ConsumerRuntimeState.completeDeadLetter` → `advanceCursor`), so `Projection.onCursorCommitted(token, partition, cursor)` moves the partition to `max(next, cursor)` and takes it LIVE once the cursor passes its head. **Every report is stamped with the rewind's token** (#1304 X6): `replayRewound` mints it and the rebuild hands it to whatever rewinds the cursor, and a report carrying any other token is ignored. Gating on arrival order alone is not enough — commits are batched and asynchronous, and a zombie consumer can still report its pre-rewind position, so a stale IN-RANGE cursor arriving after the rewind would make the replay's own offsets answer ALREADY_APPLIED: acknowledged, never written, silently lost. That event is absent from the rebuilt model until its DLQ entry is redriven. Accepting any higher delivered offset instead would let an early or zombie delivery skip, and so lose, every replay offset below it. `[mechanism: ProjectionStore.cursorCommitted / replayRewound; pinned by ProjectionTest.IdempotencyGuard.rebuilding_skipsADeadLetteredReplayOffset_onTheCommittedCursor, rebuilding_goesLive_whenTheCommittedCursorPassesTheHead and rebuilding_ignoresACursorCommitReportedBeforeTheRewind]` The committed-cursor skip is the facade half; it cures the stall once #1333 wires the runtime call. **#1333's obligation, stated:** the runtime must carry the rewind token — `ReplayCursor.rewind(range, token)` hands it over, and every cursor report for that group must be stamped with it and reported in commit order, by the consumer that rewind started. `[unverified: NO PRODUCTION CALLER — the runtime call to Projection.onCursorCommitted is not wired, because a Projection instance is not reachable from the consumer runtime and the replay cursor (capture/rewind) is the unbuilt D3 surface, both tracked by #1333; and at rc4 the durable dispatch (StreamConsumerManager.deliverTopicEvent) delivers no MessageContext, tracked by #1295 / PR #1310]`
  - **Reachability, stated plainly.** Until #1310 (context delivery) and #1333 (projection registration and the replay-cursor surface) land, the §8 claims guard, the generation fence and REBUILDING admission are UNREACHABLE in a deployed node: no dispatcher calls the context-carrying `Projection.onEvent`, no runtime calls `onCursorCommitted`, and the default replay cursor refuses every rebuild. Everything above is pinned in-JVM at the facade, not on a live path. (iii) If the rewind fails after the reset, the store stays REBUILDING with nothing replaying and every write is refused; recovery: rebuild again. (iv) A refused capture — the default until D3 — touches nothing. `[design intent — unverified: no production ProjectionStore backing or replay cursor exists yet; a real backing must make the reset and the replay-order admission indivisible with the write, the same primitive class as the #1250 KV gaps]`
...
| Sync replica unavailable (replica set below the `min-sync` floor) | `publish` fails `NOT_ENOUGH_REPLICAS`, checked BEFORE the append: the event is not in the log (#1236). With `min-sync == replicas` (§3) this is every publish on the affected partitions until the replica returns or is replaced. |
| Peer acks time out after the owner appended | `publish` resolves `PublishOutcomeUnknown`: the event MAY be in the log and visible to consumers. Retry only with the same key (`publish(T, idempotencyKey)`), or dedup sees two events (§8). A publish-forward timeout on a non-owner is reported the same way. |
