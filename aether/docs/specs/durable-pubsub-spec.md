...
- **Durable — `publish(T): Promise<Unit>`:** resolves when the event is durably appended (owner + `min-sync − 1 ≥ 1` peer acks). Publisher latency is bounded by replication latency and is **independent of subscribers** — subscriber processing is severed from the publisher's lifetime by the log. A publisher may complete and exit with consumers hours behind; nothing dangles. The promise resolves three ways (#1236): **success** = in the log at the floor; **failure** = NOT in the log (e.g. `NOT_ENOUGH_REPLICAS`, checked before the append), so a retry is a new publish; **outcome-unknown** (`PublishOutcomeUnknown`) = the owner appended but the floor was not confirmed (e.g. the peer-ack timeout), so the event MAY be in the log and visible to consumers — retry only with the same message ID, or dedup (§8) sees two distinct events. There is no abort or tombstone protocol that could turn an outcome-unknown into a clean failure.
