...
|---|---|---|---|
| `SwimHealthy` | (a) `SwimProtocol.recordHealthyAndEmit` (SWIM probe Ack) → `MembershipFsm.onSwimObservation`; (b) QUIC `PeerConnected` → `MembershipFsm.onPeerConnected` synthesis for known-cluster + SWIM-alive peers (F.4, 2026-05-12); (c) `NodeLifecycle.ACTIVE` listener → `bootstrapSelfOnDutyOnActive` synthesis for self; (d) `LeaderChange-to-self` → `MembershipFsm.onLeaderChange` retry synthesis | local in-process; all four sources route through `onSwimObservation`'s leader-write gate; SWIM gossip is multi-source but the FSM sees only the local node's interpretation | Per-peer FIFO **on the local node**. Not globally ordered. The FSM handles re-ordering by mapping `state→event→state` deterministically; replay of out-of-order events converges (idempotence I3). Belt-and-suspenders between sources (a) and (b) is safe: whichever signal arrives first writes `ON_DUTY`; the second hits `(ON_DUTY, SwimHealthy) → nop`. |
| `SwimFaulty`, `SwimDeparted` | `SwimProtocol` → `MembershipFsm.onSwimObservation` | local in-process; SWIM gossip is multi-source but the FSM sees only the local node's interpretation | Per-peer FIFO **on the local node**. Not globally ordered. The FSM handles re-ordering by mapping `state→event→state` deterministically; replay of out-of-order events converges (idempotence I3). |
...
| **DECOMMISSIONED** | if `(nowMs - decommissionedAt) < revivalTtl`: →ON_DUTY; `Put(L=ON_DUTY)`; emit `NODE_ON_DUTY` (reason=revival). Else nop (zombie). *(Bootstrap-correction 2026-05-12; see §5.1 note 4)* | nop | nop | err (slot deleted) | nop | nop (idempotent) | err | nop (waiting for `DecommissionedAtomGc` to reset to UNTRACKED) |
...
5. **`(DECOMMISSIONED, SwimHealthy)` is TTL-bounded revival (Bootstrap-correction 2026-05-12).** A peer can un-decommission by sending healthy gossip **only if** the `DECOMMISSIONED` entry is recent (within `decommissionedRevivalTtl`, default **60 seconds**). The reducer computes `ageMs = nowMs - decommissionedAtMs` and admits the peer back (`→ ON_DUTY`, `Put(L=ON_DUTY)`, emit `NODE_ON_DUTY` with `reason=revival`) iff `ageMs < revivalTtl`. Older entries stay zombie until an operator explicitly clears them. This preserves zombie protection for genuinely-stale DECOMMISSIONED entries while admitting the **elastic-test-fixture restart pattern** (`docker start` brings a killed container back with the same NodeId) and short transient-decommission scenarios. The strict inequality `ageMs < ttl` makes the revival window a half-open interval `[0, ttl)`; equality is treated as past-TTL (zombie). The single-writer rule (I2) is preserved — the leader remains the sole writer of `NodeLifecycleKey`; the TTL relaxation is a transition rule, not a writer-identity change (see §10.1 note on temporal-bounded relaxation). Spec §12's `DecommissionedAtomGc` becomes optional post-RC1, superseded by this TTL-bounded revival.
...
void onLeaderElected(NodeId self) {
    ...
    //
    // 7b. Peer cold-boot convergence via QUIC PeerConnected (Bootstrap-correction
    //     extension 2026-05-12, F.4). Triggers (a) and (b) above cover self. For OTHER
    //     peers, the leader's local SWIM probe Ack is the canonical liveness signal —
    //     but SWIM probes are jittered (first probe ~8s, then ~1s with random target
    //     selection), so on a small static cluster the leader may not complete probe
    //     Acks with every peer within bounded time, leaving them stranded UNTRACKED.
    //     QUIC handshake by contrast completes deterministically within ~100ms of
    //     cluster boot. AetherNode therefore wires `QuicPeerStateListener.onPeerJoined`
    //     and `onPeerReconnected` to `membershipFsm.onPeerConnected(peer)`, which
    //     synthesizes `HealthyObserved(peer, 0L)` and routes through the same
    //     `onSwimObservation` leader-write gate. Filter conditions inside
    //     `onPeerConnected`:
    //       (i)   peer != self (self uses 7a/7b paths above);
    //       (ii)  peer in `TopologyConfig.coreNodes()` — real cluster peers only;
    //             auto-provisioned dynamic peers (fresh NodeIds outside static config)
    //             continue through the SWIM probe-Ack path;
    //       (iii) peer is currently in SWIM's alive member set
    //             (`SwimProtocol.currentHealth()` reports HEALTHY) — avoids races
    //             where QUIC connects to a peer SWIM has not yet admitted.
    //     Belt-and-suspenders with the SWIM probe-Ack path is idempotent: whichever
    //     signal arrives first writes `Put(L=ON_DUTY)`; the second hits the reducer's
    //     `(ON_DUTY, SwimHealthy) → nop` cell. Convergence to N ON_DUTY is now
    //     deterministic within QUIC handshake latency (~100ms post-boot) for the
    //     static cluster, rather than dependent on SWIM probe scheduling.
}
...
**Note on temporal-bounded relaxation of I2 (Bootstrap-correction 2026-05-12).** The `(DECOMMISSIONED, SwimHealthy) → ON_DUTY` revival rule introduces a *temporal-bounded relaxation* of the writer-identity-vs-prior-state coupling, **not** of the single-writer rule itself. The leader is still the sole writer of every `NodeLifecycleKey` atom. What the revival rule changes is *which transitions are legal as a function of the existing atom value plus elapsed time* — specifically, the `(DECOMMISSIONED → ON_DUTY)` edge becomes legal for `nowMs - decommissionedAt < revivalTtl`. The invariant is preserved: a follower cannot revive a decommissioned peer, and a non-leader observer cannot inject a revival write; only the leader's reducer evaluates the TTL predicate and proposes the `Put(L=ON_DUTY)` consensus write. The relaxation lives entirely inside the transition table.
...
- **Bootstrap-correction extension 2026-05-12** — *TTL-bounded `(DECOMMISSIONED, SwimHealthy)` revival*. The `(DECOMMISSIONED, SwimHealthy)` cell changes from unconditional `nop` to a conditional rule: if the `DECOMMISSIONED` entry is younger than `decommissionedRevivalTtl` (new `MembershipFsmConfig` field, default 60s), the reducer admits the peer back as `ON_DUTY` (`Put(L=ON_DUTY)`, emit `NODE_ON_DUTY` with `reason=revival`). Older entries stay zombie until an operator explicitly clears them. The strict inequality `ageMs < ttl` makes the revival window a half-open interval `[0, ttl)`; equality is past-TTL. Rationale: the elastic-test-fixture model restarts killed containers with the same NodeId via `docker start`; without the TTL window, the FSM would lock those revived peers out indefinitely. The TTL window admits the legitimate restart pattern while preserving zombie-protection for genuinely-stale decommissions. Spec §12's `DecommissionedAtomGc` becomes optional post-RC1 (now superseded by TTL-bounded revival; the GC remains valid as a longer-horizon atom-cleanup mechanism but is no longer correctness-load-bearing). See §5.1 note 5 and §10.1 (temporal-bounded relaxation note).
...
- **Bootstrap-correction extension 2026-05-12 (F.4)** — *QUIC PeerConnected → SwimHealthy synthesis for static-config peers*. Cold-boot convergence to N ON_DUTY was racy: `MembershipFsm` writes `Put(L=ON_DUTY)` only when `SwimHealthy(peer)` fires, and the only upstream emission path was `SwimProtocol.recordHealthyAndEmit` on probe Ack. SWIM probes are jittered (first probe ~8s, then ~1s with random target selection); on a small static cluster the leader may not complete probe Acks with every peer within bounded time, leaving them stranded UNTRACKED. QUIC handshake by contrast completes deterministically within ~100ms of cluster boot — a stronger liveness signal (authenticated + reachable + serving). `AetherNode` now wires `QuicPeerStateListener.onPeerJoined` / `onPeerReconnected` to `MembershipFsm.onPeerConnected(peer)`, which synthesizes `HealthyObserved(peer, 0L)` and routes through the same `onSwimObservation` leader-write gate. Filters inside `onPeerConnected`: (i) peer != self, (ii) peer in `TopologyConfig.coreNodes()`, (iii) peer currently SWIM-alive. Filter (ii) ensures auto-provisioned dynamic peers continue through the SWIM probe-Ack path; filter (iii) avoids premature writes for peers SWIM hasn't admitted yet. Single-writer (I2) preserved by routing through `onSwimObservation`; I6 preserved (event-driven, no Tick); idempotence by `(ON_DUTY, SwimHealthy) → nop`. Convergence is now deterministic within QUIC handshake latency for static clusters, rather than dependent on SWIM probe scheduling. See §4 (event sources table) and §6.2 step 7b.
