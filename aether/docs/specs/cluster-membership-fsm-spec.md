...
| `JOINING` | `state=JOINING` | present, `assignedNodeId=N` | A node started and registered itself; the leader has not yet promoted it to `ON_DUTY` (will do so on first `SwimHealthy(peer)` observation). | routing skips; status reports "joining" |
...
|---|---|---|---|---|---|---|---|---|
| **UNTRACKED** | →JOINING; `Put(L=JOINING)`; emit `NODE_JOINING` | nop (I5 bootstrap-safe) | nop | →JOINING; `Put(L=JOINING)`; emit `NODE_JOINING` | nop (no-op for unknown peer) | nop unless `force=true` → →DECOMMISSIONED; `Put(L=DECOMMISSIONED)`; emit `NODE_FAILED` | err | nop (timer for a peer that left JOINING; harmless) |
...
7. **No phase suppression in the table.** `COLD_BOOT` correctness is achieved structurally: at cold-boot, every peer's FSM is in `UNTRACKED`, and `(UNTRACKED, SwimFaulty) = nop` (I5). The only path that fires writes is `UNTRACKED → JOINING → ON_DUTY`, none of which produce `DECOMMISSIONED`. The lifecycle layer cannot emit a spurious failure during cold-boot. The SWIM-layer suppression (G1) is now redundant for correctness but kept for wire efficiency.
...
| `DrainCoordinator.prepareDrain` and `markDrainComplete` interface methods | `drain/DrainCoordinator.java` | ~6 | FSM transitions own these writes |
| `LegacyLifecycleWriterFixture` test fixture | `cluster/LegacyLifecycleWriterFixture.java` (test) | ? | Replaced by FSM-direct test wiring |
