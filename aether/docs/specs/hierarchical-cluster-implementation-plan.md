...
| Deployment | Complete deployment suite: 1302 passed after implicit-community retirement and the current-count formation latch correction. | The subsequent installed-electorate formation correction passed all 114 reconciler tests. Provider tests use fixtures, not paid cloud resources. |
...
| Node correction gate | Corrected full node suite: 1565 cases, zero failures/errors, one skipped benchmark. Later targeted gates cover notification isolation, metadata cuts, worker forwarding and readiness; the latest community-health gate passed eight cases plus nine node integration cases. | The full node checkpoint predates the later targeted corrections. Both previously failing Ember drain cases passed after notification and registrar fixes; self-drain slot reuse is under validation. |
| Actual multi-node Forge | Worker formation/governor loss, durable all-core restart and governor-report-only loss passed earlier targeted runs. The latest batch passed five instance-tag/provisioning cases, real worker HTTP execution and empty-community movement. | Loaded movement, held worker invocation/drain, core resize and multi-community metadata blackout recovery require passing reruns after the latest corrections. Failures are not waived. |
| JBCT | Explicit repository CLI checked 175 changed production files: zero introduced errors; 25 errors reproduce in baseline low-level contracts. Later modified production files were checked separately with zero errors. | The default build skips JBCT and is not lint evidence. Warnings were not exhaustively classified; repeat the explicit comparison for any further changes. |
