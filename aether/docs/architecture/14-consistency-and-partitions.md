...
| **Stream consume** (`stream.consume`) | Per-partition order; **at-least-once** while the consumer runs (RAM cursor advances only post-ack, failures retried). The cursor is unpersisted, so a restart **replays from offset 0** — duplicate-heavy but still at-least-once, since the per-partition WAL restores the log. Drops to at-most-once end-to-end **only if the log is lost** (RF=1 failover / fresh-disk replacement; avoidable with `min-sync-replicas ≥ 2`) — see [known-limitations](../reference/known-limitations.md) | Owner side; RF=1: empty after failover | LIVE · guarantees.md §4 |
