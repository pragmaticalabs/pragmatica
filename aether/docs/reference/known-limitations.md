...
- **~~DHT-backed routing/endpoint state is eventual and not crash-durable.~~ WITHDRAWN (#943, 2026-09-08) — this limitation never existed.** The entry claimed slice-node, HTTP-route and endpoint keys had migrated from consensus to the DHT `ReplicatedMap` (eventual, W=R=1, in-memory), making reads stale and restart-lossy (#384). **The read path never moved**, and the maps had no production writer, so they held nothing; routing state stayed on the consensus KV throughout and keeps its quorum-durable guarantee. The `ReplicatedMap` plane has been removed. This entry therefore overstated the system's weakness rather than its strength — a reader who avoided the product for this reason was misled.
...
- **The production streaming substrate is all-RAM.** Retention, cold-tier reads, and STRONG-stream creation are gated on the streaming-persistence plan (catalog rows 179/180/181/185/190/191); DEFAULT streams are what a stock node creates today. Their delivery has **three regimes**: (1) **while the consumer runs** — at-least-once: the cursor advances only after a successful callback and retries failures; (2) **across a restart with the log intact** — the per-partition WAL replays the log (A6, `StreamCrashDurabilityTest`). Cursor resume differs by consumer kind: an app `StreamAccess` consumer that explicitly commits and re-seeks (`committedOffset()`) gets **bounded-window** redelivery (disk-durable cursor, writable data dir); a **declarative `[streams.X]` consumer** gets consensus-KV-backed checkpointing (`ClusterCursorStore`, resume = the later of local and cluster by `(rewind epoch, offset)` — `max(local, cluster)` for a group never rewound, #1333) that also survives an ownership change, degrading to local-only on a swallowed consensus-write failure; a non-committing app, every **system consumer** (`SystemStreamFactories` → `none()`), and Forge / in-memory / unwritable-mount nodes **replay from offset 0** (duplicate-heavy, still at-least-once, not loss); (3) **only where the log itself is lost** — RF=1 owner failover or node replacement onto a fresh disk (the one-disk-deep caveat above) — a published event is delivered **zero times: at-most-once end-to-end**, avoidable with `min-sync-replicas ≥ 2`. Mechanism detail in [`guarantees.md`](guarantees.md) §4; per-operation row in [`../architecture/14-consistency-and-partitions.md`](../architecture/14-consistency-and-partitions.md).
- **`[streams.X]` compression, `encryption-key-id` and the per-consumer tuning keys are not supported in 1.0 (#677).** They are parsed and then REFUSED by #576's `StreamResourceValidator.guardInertConfig`: segments are written uncompressed, no stream encryptor is wired (#253 owns the key source), and every declarative consumer runs with the hardcoded `ConsumerConfig` defaults. `auto-offset-reset` accepts only `earliest` (permanent, #478). **That refusal does not currently block a deploy, which is the sharp edge:** `BlueprintService.streamBindings` is the validator's only production caller and it discards the failure (`.or(List.of())`), publishing an EMPTY alias→address binding set. A blueprint carrying one of these keys therefore PUBLISHES; a slice consuming the aliased stream then fails with `StreamAddressError.UnboundStreamAlias`, and if nothing consumes that alias the key is silently ignored after all. The keys remain in the schema so the refusal exists to be acted on once a deploy-time gate is wired.
...
- **Durable-entity CRUD + reads are wired**, not planned: `PartitionFencedDurableEntity` has been node-wired since #352 shipped, with fsync-before-ack (`logAndReplicate` writes the `PartitionWal` frame, `awaitDurable` awaits its group commit), RF=3 replication by default, and non-owner calls forwarded to the committed owner (#596) rather than failing. Both read consistencies are live — `BOUNDED_STALE` (offset-bounded, replica-forwarding) and `LINEARIZABLE` (owner-routed, no-op-round epoch fence; a given call can still return `EntityError.LinearizableUnavailable` if no barrier is registered for that partition — a freshness-vs-safety asymmetry, not dormancy). **Still genuinely planned:** `entity.timer`, `workflow.*`, and `saga.*`. Mechanism detail in [`guarantees.md`](guarantees.md) §6.
- **Storage at-rest encryption (`[storage.encryption]`, #253) has no plaintext-to-encrypted migration path.** Enabling `encrypted = true` (or `streams_encrypted = true`) is new-instance/fresh-data only in rc4: a local-disk directory already holding unmarked plaintext blocks refuses to boot as encrypted rather than mixing plaintext and ciphertext; a DHT tier (no directory to scan) instead fails closed per block on read. Re-encrypting existing blocks under a rotated key (including on tier demote/promote) is also not shipped — old keys stay resolvable for reads instead. The auto-synthesized default `artifacts` and `content` instances (used only when no explicit `[storage.artifacts]`/`[storage.content]` section is configured) track node-wide keyring presence instead: encrypted whenever `[storage.encryption]` is configured with a resolvable keyring, plaintext otherwise — `content` joined this coverage in #783, having previously bypassed the config/keyring wiring (and demotion/GC) via a separate factory path entirely. The `streams` instance's write-ahead log is excluded from `streams_encrypted` coverage regardless — it is a separate directory from the segment-block tiers encryption gates. See [`configuration.md`](configuration.md#storage-encryption-configuration-253) for the full coverage/exclusion list, including why this is unrelated to the already-dead `[streams.X].encryption-key-id` blueprint key (#576).
- **Upgrading a node that already has `[storage.encryption]` configured turns on `content` encryption by itself, and pre-existing `content` blocks become unreadable with no migration path (#783).** `content` encryption is **not opt-in**: the auto-synthesized `content` instance takes `encrypted = <a keyring is configured>`, so an operator who enabled `[storage.encryption]` for `artifacts` gets `content` encrypted by the upgrade alone — no `[storage.content]` section, no config change. Content blocks written before the upgrade are plaintext and are read afterwards through an encrypting tier, so they fail closed **per read** with `LegacyPlaintextBlock`. All of `content`'s pre-upgrade durable data is in the DHT tier — the pre-#783 bypass path had no disk tier, so the new disk directory is fresh and the disk-side boot refusal (`EnablingOverExistingPlaintext`) does not apply to `content`'s pre-upgrade data; the DHT tier has no directory to scan and therefore no forward-direction boot guard. Loud and typed, never a silent pass-through of unauthenticated bytes; but #253 ships detection, not migration, so there is no supported way to re-encrypt that data (tracked as #831). **To keep pre-existing content readable across the upgrade, set `[storage.content] encrypted = false` explicitly before upgrading.** Note also that Aether does not support rolling upgrades (#666/#434) — the supported path is a full-cluster stop, upgrade, start — so this is an illustration of that envelope rather than a separate defect: a mixed-version cluster would have an upgraded node writing ciphertext into the shared `content-blocks` namespace while an old node read it through the bare tier.
- **A previously-encrypted storage tier that boots with encryption disabled (or `[storage.encryption]` removed) is a new boot-failure mode, not a silent downgrade to plaintext access.** Both the local-disk and DHT tiers write a `.encryption-enabled` marker the first time encryption is enabled over them; if that marker is present and no keyring resolves for the instance, boot refuses with `EncryptedTierRequiresKeyring` rather than handing back framed `AEC1...` bytes as if they were the block's real content. This covers both directions on the per-instance disk/DHT path (`StorageFactory.createOne`) and both sides of the built-in `streams` segment tiers — the disk side (`defaultStreamStorage`'s no-keyring branch already refused on the disk marker before this round) and, since #849, the `stream-segments` DHT namespace, which is stamped and checked on the same post-formation admission as every `<name>-blocks` namespace. Recovery: restore `[storage.encryption]` with the same key id still resolvable in `[storage.encryption.keys]`, or migrate to a fresh, unmarked directory or DHT namespace (tracked separately — #831).
