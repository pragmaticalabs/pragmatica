...
- **`[streams.X]` compression, `encryption-key-id` and the per-consumer tuning keys are not supported in 1.0 (#677).** They are parsed and then REJECTED at deploy time (#576's `StreamResourceValidator.guardInertConfig`) rather than silently ignored: segments are written uncompressed, no stream encryptor is wired (#253 owns the key source), and every declarative consumer runs with the hardcoded `ConsumerConfig` defaults. `auto-offset-reset` accepts only `earliest` (permanent, #478). The keys remain in the schema as loud refusals so a blueprint written against the design target fails at deploy, not at runtime.
