...
| `expectedVersion` | Config version read from `GET /api/v1/cluster/config`; the request is rejected if it no longer matches. `0` (or an omitted field) is not a wildcard: against a stored config it is refused as an unfenced overwrite (#1086). |
...
- `expectedVersion` is `0` (or omitted) against a stored config — `UnfencedOverwrite`, the same #289
  fence `POST /api/v1/cluster/config` applies. Every stored config a scale can reach carries a
  version of at least 1, so `0` never means "fresh cluster" here; it is a mismatch. It is checked at
  the write, after validation, so a request the validator would refuse still answers with the
  validator's own cause (#1086).
