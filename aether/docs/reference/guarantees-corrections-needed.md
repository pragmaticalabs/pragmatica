...
| D10 | `:177` | "zero-copy MemorySegment reads" | ✅ **SUPERSEDED (#1248, 2026-09-19)** — the earlier rewrite here ("zero-copy consumer-slice read") was itself false: no application path is zero-copy. Storage is off-heap; delivery copies to heap (on the declarative-consumer path, three `byte[]` copies per event, then decode → re-encode → decode; `readSlice` returns a heap segment over a copy and has no production caller). Row 146 no longer claims zero-copy (the adapter was deleted in #577). See `streaming-performance-analysis.md` §4.3. |
