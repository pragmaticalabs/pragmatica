...
|---|----------|-------|-----------|--------|
| C1 | `aether/aether-stream/.../replication/ReplicaSetController.java:301`; catalog row 139 ("Complete") | "Sync replication ack waits for N replica acks" | RF=`clamp(minSyncReplicas,1,N)` ⇒ peers=`minSyncReplicas-1` < acks needed ⇒ **always `NOT_ENOUGH_REPLICAS`** for `minSyncReplicas≥1` (`ReplicationManagerTest:202-208`). | File issue: `RF = minSyncReplicas+1`. Downgrade catalog 139 → Partial. |
