...
| D3 | `:270/271/281` (rows 94/95/152) | "moved from consensus to ReplicatedMap … O(3) vs O(N)" (perf-only framing) | ✅ RESOLVED — see guarantees.md §7 item 7 (#384 CLOSED); rows now carry the eventual/not-crash-durable downgrade pointer. **Superseded (#943, 2026-09-08):** that pointer has been withdrawn — the DHT read side never existed and the maps had no production writer, so no downgrade ever applied; the `ReplicatedMap` plane is deleted and rows 94/95/152 record the removal instead. |
...
| D10 | `:177` | "zero-copy MemorySegment reads" | ✅ **SUPERSEDED (#1248, 2026-09-19)** — the earlier rewrite here ("zero-copy consumer-slice read") was itself false: no application path is zero-copy. Storage is off-heap; delivery copies to heap (on the declarative-consumer path, three `byte[]` copies per event with a local ring and about eight on a forwarded read, then decode → re-encode → decode; `readSlice` returns a heap segment over a copy and has no production caller). Row 146 no longer claims zero-copy (the adapter was deleted in #577). See `streaming-performance-analysis.md` §4.3. |
