...
| 19 | Streams | `stream.consume` | per-partition order | **no automatic cursor resume**; disk-durable cursor wired only for the app `StreamAccess` path, applied only if the app commits + re-seeks | **at-least-once** (app commit+re-seek) / **replay-from-0** (non-committing app, system consumers, Forge) | owner side; **RF=1: empty after failover** | LIVE |
...
- **`stream.consume`** — **at-least-once, with NO automatic cursor resume.** A production node never auto-loads a consumer cursor on attach. A disk-durable cursor is wired **only for the app `StreamAccess` path** (`AetherNode.java:2861`/`:5114` → `StreamAccessFactory.java:85` → `PartitionedStreamAccess.java:664-684`): to resume the un-consumed window the app must **explicitly `commit()` and re-seek via `committedOffset()`** (at-least-once; the `replaceRef` commit is non-atomic; survives a same-node restart with a writable data dir). Consumers that don't commit+re-seek, all **system `FrameworkStreamConsumer`** paths (`SystemStreamFactories.java:90/121/168` pass `none()`), and Forge / in-memory / unwritable-mount nodes all **replay from offset 0**. The ~1000-events / 30 s checkpoint cadence and attach-time fetch live in `ConsumerRuntimeState`, which is **test-only** — not the wired path. **Effectively-once** (dedup on `(group,stream,partition)` via `PgTransactionalCursorCommit`) remains **not wired into the node**.
