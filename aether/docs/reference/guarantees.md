...
| 15 | Streams | `stream.append` (sync-replicated) | quorum-durable *intended* | — | — | — | ✅ FIXED #378 (RF = `minSyncReplicas+1`; `minSyncReplicas≥1` now satisfiable on the APP path) |
...
> ✅ **Synchronous replicated durability is now satisfiable (fixed #378, `1f581e530`).** Placement previously set RF = `clamp(minSyncReplicas,1,N)`, giving `RF-1 = minSyncReplicas-1` peers — always one short of the acks required, so any `minSyncReplicas≥1` publish failed `NOT_ENOUGH_REPLICAS`. The APP path now sets RF = `clamp(minSyncReplicas+1,1,N)` (owner + the peers the acks require), so `minSyncReplicas≥1` publishes succeed. **Still open:** the STRONG/consensus cross-node publish path is **unwired**, and "zero-copy" applies only to the **consumer** read, not the producer path. See Known Gaps.
...
1. **✅ RESOLVED — Stream synchronous replication was unsatisfiable** — `RF = clamp(minSyncReplicas,1,N)` **included the owner** (`ReplicaPlacement.java:108`), but both publish paths await `minAcks = minSyncReplicas` *distinct non-self* acks (`PartitionedStreamAccess.java:527`, `DefaultStreamPublisher.java:226`; precheck `DefaultReplicationManager.java:162`). Peers = `minSyncReplicas-1` < required ⇒ every `minSyncReplicas≥1` publish failed `NOT_ENOUGH_REPLICAS` (verified at HEAD, 2026-06-29). **Fixed in `1f581e530`:** APP-branch `RF = clamp(minSyncReplicas + 1, 1, clusterSize)` (owner + the peers the acks require). *Tracking:* **#378** (CLOSED). #262 (CLOSED) is adjacent — its test `selfInReplicaSet_isNotCounted` **encodes** this shortfall as expected; catalog row 139 still claims "Complete".
