...
| 15 | Streams | `stream.append` (sync-replicated) | quorum-durable *intended* | — | — | — | ⚠ DEFECT (`minSyncReplicas>0` unsatisfiable — RF/ack off-by-one) |
...
> ⚠ **`DHTConfig.java:162-167` is factually wrong**: it claims "Full replication is always strongly consistent. R + W > N ensures any read will see the most recent write." `FULL` is W=R=1, so R+W=2 ≤ N, and the read path never reconciles versions. See Known Gaps.
...
- **Append epoch fence** — `rejectIfStale` rejects a deposed owner's late appends; the high-water advances by observing committed ownership. Effective only on epoch-stamped appends; may be **dormant** until the owner-change driver (1d-iii) is confirmed live.

> ⚠ **Synchronous replicated durability is currently unsatisfiable.** Placement sets RF = `clamp(minSyncReplicas,1,N)`, giving `RF-1 = minSyncReplicas-1` peers — always one short of the acks required, so any `minSyncReplicas≥1` publish fails `NOT_ENOUGH_REPLICAS` (matched by `ReplicationManagerTest:202-208`). The STRONG/consensus cross-node publish path is also **unwired**. "zero-copy" applies only to the **consumer** read, not the producer path. See Known Gaps.
...
**Code/behaviour** issues (beyond doc wording), surfaced for triage — none fixed by this audit. *Tracking* reflects a GitHub-issue search (open + closed) on 2026-06-29.

1. **⚠ Stream synchronous replication is unsatisfiable** — `RF = clamp(minSyncReplicas,1,N)` **includes the owner** (`ReplicaPlacement.java:108`), but both publish paths await `minAcks = minSyncReplicas` *distinct non-self* acks (`PartitionedStreamAccess.java:527`, `DefaultStreamPublisher.java:226`; precheck `DefaultReplicationManager.java:162`). Peers = `minSyncReplicas-1` < required ⇒ **every `minSyncReplicas≥1` publish fails `NOT_ENOUGH_REPLICAS`** (verified at HEAD, 2026-06-29). Fix: `RF = minSyncReplicas + 1`. *Tracking:* **#378**. #262 (CLOSED) is adjacent — its test `selfInReplicaSet_isNotCounted` **encodes** this shortfall as expected; catalog row 139 still claims "Complete".
2. **⚠ KV `Remove` is unfenced** (`KVStore.java:131`) — a deposed owner/leader can delete a fenced key while Put is protected. Fence `Remove` symmetrically, or document the asymmetry. *Tracking:* **#379** (sub-issue of epic **#345**).
3. **⚠ `DHTConfig` FULL docstring is wrong** (`DHTConfig.java:162-167`) — claims R+W>N strong consistency; FULL is W=R=1 and the read path never reconciles versions. *Tracking:* **#380**.
