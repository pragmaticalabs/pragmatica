...
| 29 | Durable-entity | `entity.get(key[, BOUNDED_STALE])` | local committed-prefix read (single-owner in-memory cut) | none (in-process map) | — | single-owner | LIVE — honest per-call default (#382 resolved) |
| 30 | Durable-entity | `entity.get(key, LINEARIZABLE)` | owner-routed + no-op round + post-round epoch fence (entity-native `LinearizableEntityServe`, shared ownership substrate); non-owner → `NotCurrentOwner`, deposed → `StaleEpochRead` | (rides #349) | — | committed owner | PARTIAL — mechanism BUILT + unit-tested (#345 item 1e-b); production-DORMANT (degrades to local) until entity node wiring #352 + persistence #349 |
