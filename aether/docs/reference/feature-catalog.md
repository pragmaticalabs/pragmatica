...
| 136 | Docker scaling test infrastructure | Battle-tested | 5-node Docker Compose cluster with auto-provisioning via DockerComputeProvider. Chaos tests (node kill, leader kill, multi-kill, kill-under-load with auto-heal verification). Scaling tests (scale-up/down under load, quorum safety rejection). Config-driven API key, port offset for dynamic nodes |
