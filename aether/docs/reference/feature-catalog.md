...
| 185 | Consumer cursor persistence | Partial | `CursorStore` persists consumer group cursors in AHSE via named references (`aether-stream/segment/CursorStore.java`). **As-built:** `CursorStore` **is** constructed and disk-wired for the app `StreamAccess` path (`AetherNode.java:2861`/`:5114` → `StreamAccessFactory.java:85` → `PartitionedStreamAccess`), durable across a same-node restart with a writable data dir. **Gap:** there is **no automatic cursor resume** — the app must explicitly `commit()` and re-seek via `committedOffset()`; system `FrameworkStreamConsumer` paths receive `none()` (`SystemStreamFactories.java:90/121/168`) and replay from 0; `ConsumerRuntimeState` (the attach-fetch/checkpoint class) is test-only, not the wired path. **Plan:** auto-resume + system-path cursor tracked as #478 |
