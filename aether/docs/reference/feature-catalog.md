...
| 206 | KV-Store durable backup | Complete | Cluster metadata serialized to TOML file in local git repo. Git provides versioning, history, diffs, optional remote push. BackupService with automatic periodic + on-change triggers. REST API, CLI. **Note:** git/disk backup is OPT-IN — `resolvePersistence` defaults to in-memory `RabiaPersistence::inMemory` unless a `[backup]` path is configured (`AetherNode.java:542-547`). A single-node restart survives via the consensus peers' replicated log; a full-cluster cold restart needs `[backup]` configured |
...
| 141a | Replica-state observability surface | Complete | Read-only per-partition replica-state snapshot — the regression sensor for the stream-replication class (#260/#261/#333). Per replica: `state` (`SYNCING`/`CAUGHT_UP`/`LAGGING`), `confirmedOffset`, `isHrwOwner`; partition-level: resolved `hrwOwner`, `servedByOwner`, `ownerHeadOffset` (owner true tail), `earliestRetainedOffset`. Lets an operator/LLM spot a `CAUGHT_UP` replica whose `confirmedOffset` lags the owner's tail (the #333 write-idle residual). Assembled from the answering node's `ReplicaRegistry` + HRW owner resolver (snapshot read, no hot-path cost); **owner-aware, not owner-forwarded** — authoritative only on the HRW owner (`servedByOwner`), as per-partition-owner forwarding is not a management `RouteTarget` and the stream forward transport is event-only. `GET /api/streams/replicas/{name}/{partition}`, `aether stream replicas <stream> <partition>` |
...
| 185 | Consumer cursor persistence | Partial | `CursorStore` persists consumer group cursors in AHSE via named references (`aether-stream/segment/CursorStore.java`). **Gap:** in prod the `CursorStore` is never constructed — `ConsumerRuntimeState` receives `none()` and guards every commit with `onPresent`, so consumer offsets are RAM-only and lost on restart. **Plan:** Phase A of [`streaming-persistence-implementation-plan.md`](../internal/progress/streaming-persistence-implementation-plan.md) registers `CursorStore` and switches to the cursor-bearing stream-access overload |
