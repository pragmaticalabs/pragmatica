...
| 136 | Docker integration test infrastructure | Battle-tested | 5-node cluster + passive LB via Docker Compose. Chaos tests (19/19): kill, leader re-election, multi-kill quorum, kill-under-load with 0% error rate through LB, auto-heal provisioning. Scaling tests (16/16): quorum safety rejection, scale-up 5→7 (2s), scale-down 7→5 under load (34s). ManageableNode interface enables LB to serve management API locally. PassiveNode.apply() for consensus proposals from passive nodes |
