...
| 18b | API versioning (path mode) | Partial | routes.toml `[api]` section + per-version `[vN.routes]`/`[vN]` blocks (#198 Phase 1+2): bind key resolves to `getV{N}` (D8, or explicit `method` override), route mounts at `{api.prefix}/v{N}/{path}`. Five compile-time checks via the unit-tested `VersionSchemaValidator` (schema mixing, duplicate bind key, single `defaultIfMissing`, `sunset` ISO-date, method resolution). Version metadata (`deprecated`/`sunset`/`defaultIfMissing`, `requireVersionHeader`) parsed + stored in the manifest (envelope 1002). Both versions serve over HTTP (Forge-proven). Deferred: header-mode version extraction, Deprecation/Sunset/Link header emission, version-registry endpoint |
