...
**§5.4 (`a97a9e753`) — Docker-VALIDATED 2026-05-30 → ❌ FAILED: it BREAKS COLD-START FORMATION.** On `a97a9e753`, cluster A never assembled a 5-node membership (`[FAIL] 5 nodes ... timed out after 360s`) even though all 5 containers were "Up (healthy)" — processes alive, but no quorum/leader. Root cause: removing the static `coreNodes` seed left the QUIC-dialed `nodeStatesById` empty, and **the SWIM-discovery→QUIC-dial path does NOT actually drive formation in production** — static seeding was silently doing the formation job. The unit tests pass ONLY because they inject discovery by hand (`handleDiscoveredNodes`); the real `ANNOUNCE`→discovery→`handleDiscoveredNodes`→dial flow (v2 §5 steps 1-9) is **not functional end-to-end** — discovery never populates the dial set at cold-start. So v2-architecture §5's claimed formation path is **not implemented/working**, and §5.4 cannot land until it is.

**⇒ DECISION (user, 2026-05-30): go FORWARD, not back. §5.4 stays in place (`a97a9e753` NOT reverted).** The §5.4 failure exposed the real gap — v2-§5's SWIM-discovery formation path was never actually implemented; static seeding was masking it, and reverting would only re-hide it. So the next session's job is to **BUILD the real formation path**: SWIM `ANNOUNCE`→discovery→`handleDiscoveredNodes`→QUIC-dial that forms a cluster on its own (per v2 §5 steps 1-9), then PEERS→SWIM-only (§5.4) becomes correct and cold-start works. **Accepted intermediate state: branch HEAD does NOT cold-start in Docker until that formation path lands — do not push for release until it does.** (The departed-gate is explicitly NOT the path — that's backward; forward is the real discovery-driven formation.)

## 4. Open issues for next session (priority order)
1. **BUILD the real v2-§5 SWIM-discovery formation path (forward; user-decided).** §5.4 stays; the next step is to make SWIM `ANNOUNCE`→discovery→`handleDiscoveredNodes`→QUIC-dial actually form a 5-node cluster on its own at cold-start (today it does not — formation timed out with discovery never populating `nodeStatesById`). FIRST: trace why, in production, SWIM discovery doesn't drive `handleDiscoveredNodes` / the dial set at cold-start (is `ANNOUNCE` emitted on transport-ready? is the swim→`TopologyObserver` discovery bridge wired? does QUIC dial discovered peers?). That bridge is the missing piece. Once cold-start forms via discovery, re-run Docker `--suites 00,02` (then full suite). **HEAD does not cold-start until this lands — do not push for release as-is.**
