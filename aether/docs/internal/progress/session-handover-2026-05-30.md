...
**⇒ RECOMMENDATION: revert `a97a9e753` (§5.4)** so branch HEAD returns to the validated-good `540cb6aa3` (collapse + QUIC guard — cold-start works, first wedge fixed). Re-approach PEERS→SWIM-only ONLY after the SWIM `ANNOUNCE`→discovery→`handleDiscoveredNodes`→QUIC-dial formation path is built and proven to form a cluster on its own (that is the real §5 gap). The simpler **departed-gate** (keep static seed; stop re-dialing a departed configured peer — the agent's first green attempt) is the pragmatic alternative that fixes the static-seed re-dial wedge WITHOUT breaking cold-start.

## 4. Open issues for next session (priority order)
1. **DECIDE §5.4's fate (it currently breaks cold-start — see §3).** Two paths: **(revert)** `git revert`/drop `a97a9e753`, returning HEAD to the validated `540cb6aa3`, then later build the REAL v2-§5 formation path (SWIM `ANNOUNCE`→discovery→`handleDiscoveredNodes`→QUIC-dial that forms a cluster *on its own*) before re-attempting PEERS→SWIM-only; or **(fix-forward)** build that formation path now so §5.4 actually forms. The departed-gate (static seed kept; departed configured peer not re-dialed) is the low-risk middle that fixes the re-dial wedge without the cold-start break. **Until one is done, the branch HEAD does NOT cold-start in Docker — do not push `a97a9e753` as-is.**
