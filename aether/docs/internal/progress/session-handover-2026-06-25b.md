...
### 2b-ii DONE — per-partition fence built (`5d6fe40a6`), repo-owner approved
**Fence-granularity fork RESOLVED → per-partition.** The entity now fences each write by its key's `(keyspace, partition)` epoch, not the coarse `"core"` governor arc. `aether-dht`: `EntityPartitionArc` (key→`(keyspace,partition)`), `PartitionOwnerEpochGate` (fences each write against its arc's high-water — the real fix: `HighWaterOwnerEpochGate` was *ignoring* the per-call key and pinning everything to `"core"`), `PartitionOwnerEpochSource`/`KvPartitionOwnerEpochSource` (per-partition epoch from `StreamPartitionOwnershipValue`). `durable-entity`: `PartitionFencedDurableEntity` (additive) + extracted shared `PerKeySerialExecutor`. **PROVEN:** a deposed partition-owner's write is rejected on a **same-generation reshuffle** (`streamPartition` high-water 8:1→8:2, same `rabiaTerm`, NO governor/`core` change), while a different partition is unaffected — the exact gap the coarse fence misses. 8 + 7 fence tests; 2b-i + production factory intact; the coarse 1c files were only reformatted (no behavior change).

### STILL AHEAD for 2b — all #277-coupled (AetherNode wiring; sequence after #277)
1. **Live ownership minting:** `StreamPartitionOwnershipWriter` must mint entity-keyspace `(keyspace,P)` ownership records so the per-partition epochs advance live (the 2b-ii test advances the high-water directly, which is what the writer will do).
2. **Owner-routed `update` invocation** — NET-NEW mechanism (none exists: publish/read forward to the owner, but nothing runs an arbitrary mutator on the owner).
3. **Quorum replication** (2b-i/ii are single-local-replica) + **production `DurableEntityFactory` wiring** (still returns `InMemoryDurableEntity` — needs `ProvisioningContext.extension` injection of the engine/fence/owner-resolver).
4. **2b-iii** owner-routed linearizable `get`, then **2c** durable timers (wheel + persisted fenced entries + handover-rebuild), then **P3** restart-durability (#349 path-a).
