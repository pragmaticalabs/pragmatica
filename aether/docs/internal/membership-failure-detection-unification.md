...
| 2026-05-24 | **NEW BUG found by Spike-2 (in <90s):** the auto-heal replacement reaching ON_DUTY triggers `IllegalStateException: Illegal (state,event): state=OnDuty, event=SlotClaimed` (`ClusterMembershipReducer.applyOnDuty:178`, via `MembershipFsm:770`), thrown uncaught into the executor (one per surviving peer's FSM). Non-fatal this run, but a real FSM robustness gap: a late/duplicate `SlotClaimed` against an already-ON_DUTY peer should be an **idempotent nop**, not a throw — a throwing reducer cell aborts that FSM tick (same failure shape as the codec bug). | Demonstrates the substrate's value (found a latent FSM gap fast) and is directly a **Pillar-1** concern (the sovereign FSM must tolerate redundant/late events). Proposed fix: `applyOnDuty` `SlotClaimed` cell `illegal`→`nop` (pending review). | follow-up task |
