...
| 1000 (no bump — GA freeze) | #1098: optional typed config components no longer wrapped in `Result.success` — `ConfigFacade.getInt/getLong/getDouble/getBoolean` now answer `Result<Option<T>>` (absent → `Success(None)`, present but unparseable → a failure naming the key, the value and the slice), so the generated `Result.all(...)` chain for a `@ResourceQualifier(type = ConfigurationSection.class)` record reads `ctx.config().getInt("section", "key")` directly instead of `Result.success(ctx.config().getInt(...))`. Before this a malformed optional value (`port = "80x"`) read as `none()` and the record was built with the caller's default. `Option<String>` components (`getString`, still a bare `Option<String>`) and every `require*` component are byte-for-byte identical to the prior output. Emitted under the GA freeze, so the stamp stays 1000 and the runtime accept-set is unchanged. |
