...
## ADDENDUM (same day, post-handover session continuation)

Everything below happened AFTER the handover above was written; it RESOLVES two of the "pre-release caveats" listed there.

- **Stale version refs: FIXED** (`5af6fa691`) — README/aether/core/jbct READMEs, step-composition pom, notification-hub blueprint coords, all → 1.0.0-rc2. `PackageSlicesMojo` `0.11.2`/`0.1.0` confirmed vestigial (platform classpath overrides generated-pom dep coords) — marked, derive-or-drop = rc3.
- **Version-drift hardening shipped** (`4867cb7c2`): jbct-init self-derives its version (filtered build-stamp, #403 BuildInfo pattern; `DEFAULT_VERSION` constant DELETED; chain GitHub→self→fail-loud naming the `--version` flags); `tools/version-sweep.sh` (shell-only; canonical-vs-pom check over READMEs/example poms/blueprints + src/main `version-literal:` marker contract; proven both directions); sweep wired into user-global `~/.claude/commands/pre-release-check.md` (config snapshot `2026-07-15T11-15` taken first). rc3-deferred: docs-as-code snippet generation, jbct-lint version-literal rule.
- **The RecordGenerator ">11 fields" caveat: OBSOLETE — fixed properly** (`3d07cd178`). Root: core ladders (`Fn`/`Tuple`/`Result.all`/`Option.all`/`Promise.all` + MapperN) ALL reach 15 (the "caps at 11" was a multi-line-signature grep artifact); the `<=11` was stale in the generator. Owner-designed batched template now in ALL SIX `.all(`-emitting codegen sites (completeness sweep found 3 more than known: FactoryClassGenerator transitive step-resources — previously unguarded-broken >15 — plus its two config-section fragments): parts = `.all(batch<=15).id()` materialized as monad-of-tuple, outer join, `Tuple.map` cascade; recursive (no 225 cliff); Promise parts constructed before the join (concurrency preserved). Slice-dependency >15 hard error REMOVED (capability unlock). **ENVELOPE_FORMAT_VERSION not bumped — owner-ruled** (≤15 outputs byte-identical, #277 precedent). Gates: pg-codegen 218/0, slice-processor 285/0, slice-processor-tests 46/0 (non-vacuous, verified), full build.sh green. Note: 230-`long` records hit the JVM 255-param-slot constructor limit — orthogonal wide-record property, documented in tests.
- **MEMORY.md compacted** 94KB→16.1KB (was silently truncating at load); full text archived (`project_memory_archive.md`); 2 new op-trap memories (detached-execution-mandatory, forge-tests-vacuous-pass).
- **Candidate tag decision still open**: `v1.0.0-rc2-candidate` at `6d96e7c58`, HEAD `3d07cd178` (5 ahead: handover, docs, hardening, codegen). None change aether-node.jar content; moving the tag re-triggers Release CI republish. Owner call: move per-batch convention vs move at release.
- **Remaining queue unchanged otherwise**: issue close-outs (drafts in session scratchpad; #445 now closable on 4-surface evidence incl. cloud), rc3 issue to file (in-JVM Ember RF=2 owner-kill test), VM-snapshot decision for clean cloud 15/15, `/release` act.
