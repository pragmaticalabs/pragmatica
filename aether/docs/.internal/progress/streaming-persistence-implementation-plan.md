...
Register `CursorStore.cursorStore(streamStorage)` as the stream `ProvisioningContext`/SPI extension (near `AetherNode.java:4493`). In `StreamAccessFactory` (`:45/:73/:109`), pull `context.extension(CursorStore.class)` and switch `ownerRoutedAccess`/`plainAccess` to the **cursor-bearing** `PartitionedStreamAccess.streamAccess(…)` overload (`:262` / owner-routed `:359`), passing `Option.some(cursorStore)` and `cursorWriter = (s,g,p,o) -> cursorStore.commit(g,s,p,o)`. `ConsumerRuntimeState` already fetches on attach (`:164`) and commits on progress (`:188,:213`) once `some`. **Auto-resume — DONE (2026-07-18, #478):** the wired consumer path is `PartitionedStreamAccess` (owner-routed / plain `StreamAccess`). Framework-driven auto-resume is now a first-class contract method — `StreamAccess.fetchFromCommitted(group, partition, maxEvents)` resolves the committed cursor (served from the disk `CursorStore` via `committedOffset()`; durable **after the next metadata snapshot** — ≤100 mutations / 30 s, `StorageFactory.java:44-45,163-170` — an un-snapshotted commit resumes from the prior cursor or 0) and reads from it in one call; no committed cursor → from 0; an explicit `fetch(offset)` still overrides. `ConsumerRuntimeState` (credited here) remains **test-only**, not on the production path — the auto-resume contract lives on `StreamAccess`. **Scope:** app consumers only — system `FrameworkStreamConsumer` consumers still receive `none()` and replay from 0 (unchanged). Proven by `CursorAutoResumeRestartTest` (aether-node).
...
Add `StreamPersistenceRestartTest` (sibling of A0; model on `StreamOwnershipDriverFenceTest` / EmberCluster `killNode`/`stop`/`start`). **Same-node restart preserving the data dir:** produce N → force seal (exceed ring capacity or seal trigger) → commit consumer cursor at offset K → restart the owning node **reusing its stream-segments dir** → assert the consumer **resumes at K** and reads **all N** (sealed segments via A5). **Cursor resume — DONE (2026-07-18, #478):** the resumes-at-K cursor leg is wired via `StreamAccess.fetchFromCommitted` and proven by `CursorAutoResumeRestartTest` (aether-node), which commits offset K through the app `StreamAccess`, rebuilds the stream storage over the same data dir (the production `StorageFactory.defaultStreamStorage` path: disk-tier blocks + snapshot-restored cursor refs), and asserts the re-attached consumer resumes at K automatically — not via an explicit re-seek. (Event-log durability across a full-cluster restart is the separate `StreamCrashDurabilityTest` gate.) Also add a **full-cluster `stop()`→`start()` variant asserting it FAILS** under memory-DHT — documenting the cold-cluster gap that motivates B/C. **Requires:** EmberCluster must give each node a stable data dir that survives restart (harness work in A0/A1).
