...
The website shows ~66 features. The repo has **227 feature rows** (generated by
`scripts/catalog-stats.sh`; the "173" previously recorded here is retired as of 2026-09-08, #928, and
the count is of rows *claiming* a capability, not of verified ones). These sections are completely
absent:
...
Website may show outdated counts. **The counts previously tabulated here (total 173) are retired as of
2026-09-08 (#928)** -- they matched neither the website nor the catalog. The catalog now generates its
Statistics table from its own rows; run `scripts/catalog-stats.sh --check` rather than copying a
snapshot into this file. Current generated stats:
...
`Critical` is no longer a status: it named an effort/priority axis rather than a completion level and
only ever applied to one row (#931). These are claimed counts, not verified ones.
