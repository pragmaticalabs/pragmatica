...
package org.pragmatica.aether.test.stream;
...
/// Minimal stream-only slice (no database) for the #265 STEP 0 streaming fan-out baseline.
///
/// Injects a [StreamPublisher] and [StreamAccess] both qualified to the `streams.test-events`
/// resource (declared in `resources.toml` with partitions=1 and count-based retention so a slow
/// consumer is never evicted). Exposes two app-HTTP routes:
///
///   - `POST /api/stream/publish` — append one payload to the log (partition 0).
///   - `POST /api/stream/read`    — fetch events from a caller-supplied offset (log/Kafka fan-out:
