...
trap 'rm -rf "$WORK" "$STATE_DIR"' EXIT
...
# hcloud: `server list` prints $STUB_DIR/hcloud-list.<n> (n-th list call) or hcloud-list,
# failing with the rc in hcloud-list-rc when present. `server describe <id>` reports "not
# found" when $STUB_DIR/gone-<id> exists, another error when describe-rc exists, else found.
stub_hcloud() {
    local n
    ...
    case "${1:-} ${2:-}" in
        "server list")
            ...
            n=$(grep -c '^server list' "$STUB_DIR/hcloud-calls")
            if [ -f "$STUB_DIR/hcloud-list.${n}" ]; then
                cat "$STUB_DIR/hcloud-list.${n}"
            else
                cat "$STUB_DIR/hcloud-list" 2>/dev/null
            fi
            return 0
            ;;
        "server describe")
...
check "$d" "B13 docker: an unreadable count is reported as a failed read, never as 0" \
    eval '[ "$(rc_of "$d")" = 1 ] && [ "$(lines_of "$d/restart-calls")" = 1 ] && has "$d" "last observed value: <read failed: _cluster_active_core_count_checked" && has "$d" "within 60s of restart" && ! has "$d" "integer expression expected"'
