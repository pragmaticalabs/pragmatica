...
test_viewer_cannot_mutate() {
    ...
    status=$(curl -s -o /dev/null -w "%{http_code}" \
        ...
        -d '{"targetNodes":3}' "${CLUSTER_ENDPOINT}/api/v1/scale")
    assert_eq "$status" "403" "Viewer cannot POST /api/v1/scale (403)"
}

test_admin_can_deploy() {
    local status
    status=$(http_status "${CLUSTER_ENDPOINT}/api/v1/blueprints/validate" \
        -X POST \
        -H "X-API-Key: ${ADMIN_API_KEY}" \
    ...
    if [ "$status" -ne 401 ] && [ "$status" -ne 403 ] 2>/dev/null; then
        log_pass "Admin can POST /api/v1/blueprints/validate (status: ${status})"
        return 0
    fi
    log_fail "Admin denied for /api/v1/blueprints/validate (status: ${status})"
    return 1
}
...
test_operator_can_scale() {
    ...
    local status
    status=$(http_status "${CLUSTER_ENDPOINT}/api/v1/scale" \
        -X POST \
    ...
    if [ "$status" -ne 401 ] && [ "$status" -ne 403 ] 2>/dev/null; then
        log_pass "Operator can POST /api/v1/scale (status: ${status})"
        return 0
    fi
    log_fail "Operator denied for /api/v1/scale (status: ${status})"
    return 1
}
