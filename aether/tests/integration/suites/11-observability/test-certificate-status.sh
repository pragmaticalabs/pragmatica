...
_cert_renewal_status() {
    local cert
    cert=$(api_get "/api/v1/certificates") || return 1
    local renewal
...
test_certificate_endpoint() {
    local cert
    if ! cert=$(api_get "/api/v1/certificates"); then
        log_fail "GET /api/v1/certificates failed (api_get returned non-zero)"
        return 1
    fi
...
test_expires_at_field() {
    local cert
    cert=$(api_get "/api/v1/certificates") || { log_fail "cert endpoint failed"; return 1; }
    local renewal
    ...
    if [ -z "$expires_at" ]; then
        log_fail "expiresAt field missing from /api/v1/certificates when TLS configured (renewal=${renewal:-unknown})"
        return 1
    fi
...
test_renewal_status_field() {
    ...
    renewal=$(_cert_renewal_status)
    if [ -z "$renewal" ]; then
        log_fail "renewalStatus / renewal / autoRenew field missing from /api/v1/certificates"
        return 1
    fi
    case "$renewal" in
