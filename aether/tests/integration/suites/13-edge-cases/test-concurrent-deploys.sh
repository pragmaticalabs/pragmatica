...
test_concurrent_deploy() {
    # A prior destructive test in this suite can have legitimately killed the node the
    # pinned CLUSTER_ENDPOINT points at (Hetzner-R1: leader self-fence → every raw curl
    # 000). Re-resolve to a live node first — a 000 from a dead pinned port proves
    # nothing about concurrent publish handling.
