...
| C4 | Concurrent deploys do not break cluster health (`assert_cluster_healthy` post-condition) | `aether/docs/specs/cluster-membership-fsm-spec.md §3.3` |
| C5 | Disruption-budget gate accepts the first N-K drains (where K = quorum-preserving minimum) and rejects the (N-K+1)-th with 409 | `[CONTRACT-GAP-13.A]` — no dedicated disruption-budget spec; behavioural contract pinned by `aether/docs/reference/management-api.md` drain endpoint + `cluster-membership-fsm-spec.md §5.1` (ON_DUTY+OperatorDecommission graceful drain) |
| C6 | A drained node can be reactivated via the activate endpoint; cluster returns to healthy | `cluster-membership-fsm-spec.md §5.1` (DECOMMISSIONED→ON_DUTY revival within TTL) |
...
| C8 | After route cleanup, app routes return ZERO 502/504 over a 10-probe window — the killed node's stale entries are pruned from the route table | `aether/docs/specs/slice-lifecycle.md §3`; `aether/docs/specs/cluster-membership-fsm-spec.md §3.3` (reconciler) |
| C9 | After kill-and-reconverge, cluster returns to N=5 nodes (CTM provisions replacement) | `cluster-membership-fsm-spec.md §3.3`; `quic-transport-spec.md §3.7` |
