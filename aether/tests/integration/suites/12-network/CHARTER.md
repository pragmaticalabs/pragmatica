...
| C9 | Partition quorum gate S05: a 2-vs-3 dual-signal split (`docker network disconnect` ⇒ QUIC drop + SWIM faulty) MUST NOT cost the 3-node majority its quorum or its leader. Prompt eviction of the dual-signal minority is INTENDED (LeaderReconciler two-signal co-confirmation, ~3s, no self-drain TTL) and is not a violation; the gate's minority-hold protection only applies to single-signal false positives, which `docker network disconnect` cannot produce. | `membership-architecture-v2-spec.md §5.1` (ON_DUTY+SwimFaulty); single-writer invariant I2; `aether/docs/internal/progress/v1-roadmap.md` #17 (CLOSED in db221dee4) |
...
| TC-12-NETWORK-016 | `test_partition_does_not_destabilize_majority` | `test-partition-quorum-gate.sh:214` | C9 | core | Injects a 2-vs-3 dual-signal partition (QUIC drop + SWIM faulty via `docker network disconnect`); polls majority quorum + leader 5×@1Hz for the 5s window; fails only if the majority loses quorum or its leader. Prompt minority eviction is intended co-confirmation behavior, NOT asserted against (the prior 5s minority-hold assertion was wrong for a dual-signal split — it tested the single-signal false-positive path, which this injection cannot produce). EXIT trap heals the partition. |
