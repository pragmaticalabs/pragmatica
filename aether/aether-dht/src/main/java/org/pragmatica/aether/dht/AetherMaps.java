...
public interface AetherMaps {
    ...
    static AetherMaps aetherMaps(DHTClient client) {
        ...
        record aetherMaps(ReplicatedMap<EndpointKey, EndpointValue> endpoints,
            ...
            @SafeVarargs
            private static void dispatchRemoveToFirst(byte[] rawKey, NamespacedReplicatedMap<?, ?>... maps) {
                for (var map : maps) {
    ...
    private static EndpointKey deserializeEndpointKey(byte[] bytes) {
        return EndpointKey.endpointKey(new String(bytes, StandardCharsets.UTF_8)).expect("DHT corruption: invalid EndpointKey bytes");
    }
    ...
    private static EndpointValue deserializeEndpointValue(byte[] bytes) {
        return new EndpointValue(NodeId.nodeId(new String(bytes, StandardCharsets.UTF_8)).expect("DHT corruption: invalid NodeId bytes for EndpointValue"));
    }
    ...
    private static SliceNodeKey deserializeSliceNodeKey(byte[] bytes) {
        return SliceNodeKey.sliceNodeKey(new String(bytes, StandardCharsets.UTF_8)).expect("DHT corruption: invalid SliceNodeKey bytes");
    }
    ...
    private static byte[] serializeSliceNodeValue(SliceNodeValue value) {
        ...
        return (value.state()
                     .name() + "|" + reason + "|" + value.fatal() + "|" + value.transitionedAt()).getBytes(StandardCharsets.UTF_8);
    }
    ...
    public static HttpNodeRouteKey deserializeHttpRouteKey(byte[] bytes) {
        return HttpNodeRouteKey.httpNodeRouteKey(new String(bytes, StandardCharsets.UTF_8)).expect("DHT corruption: invalid HttpNodeRouteKey bytes");
    }

    public static byte[] serializeHttpRouteValue(HttpNodeRouteValue value) {
        var encoded = value.artifactCoord()
                    + "|" + value.sliceMethod()
                    + "|" + value.state()
                    + "|" + value.weight()
                    + "|" + value.registeredAt();
