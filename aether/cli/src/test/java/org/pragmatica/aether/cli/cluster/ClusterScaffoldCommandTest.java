...
/// Exercises `ClusterScaffoldCommand.call()` end-to-end via picocli, not `DockerComposeTemplate`
/// directly, so the assertion holds for what the CLI actually prints.
class ClusterScaffoldCommandTest {
    ...
    @Test
    void call_dockerComposeTemplate_emitsNoLiteralSecretValue() {
        var exitCode = runScaffold("--name", "us-prod", "--template", "docker-compose", "--nodes", "3");

        assertThat(exitCode).isZero();
        assertThat(out.toString()).doesNotContain("change-me-cluster-secret");
    }
