...
@Command(name = "tasks", description = "Task group assignment management", subcommands = {ClusterTasksCommand.ListCommand.class, ClusterTasksCommand.StatusCommand.class, ClusterTasksCommand.ReassignCommand.class})
@SuppressWarnings("JBCT-RET-01")
class ClusterTasksCommand implements Callable<Integer> {
    ...
    public Integer call() {
        return clusterTarget.applyOverrides()
                            .flatMap(_ -> ClusterHttpClient.fetch(CLUSTER_TASKS_LIST))
    ...
    @Command(name = "list", description = "List all task group assignments")
    @SuppressWarnings("JBCT-RET-01")
    static class ListCommand implements Callable<Integer> {
        @CommandLine.ParentCommand
        private ClusterTasksCommand parent;
    ...
    @Command(name = "status", description = "Show assignment for a single task group")
    @SuppressWarnings({"JBCT-RET-01", "JBCT-PAT-01"})
    static class StatusCommand implements Callable<Integer> {
        @Parameters(index = "0", description = "Task group name (e.g., SCALING, METRICS)")
    ...
    @Command(name = "reassign", description = "Reassign a task group to a different node")
    @SuppressWarnings({"JBCT-RET-01", "JBCT-PAT-01", "JBCT-SEQ-01"})
    static class ReassignCommand implements Callable<Integer> {
        @Option(names = "--group", required = true, description = "Task group to reassign (e.g., SCALING, METRICS)")
        private String group;

        @Option(names = "--target", required = true, description = "Target node ID to receive the task group")
