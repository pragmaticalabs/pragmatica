...
/// Generate `FirewallRule` lists from a [FirewallPreset] + admin/internal CIDRs.
///
/// Port assignments mirror the framework defaults (kept as constants here so the
/// wizard does not depend on aether-config private internals):
/// - 8080 — Management API
/// - 8070 — App HTTP
/// - 7100 — Cluster (Rabia consensus)
/// - 7200 — SWIM gossip
public sealed interface FirewallPresets {
    ...
    /// Generate firewall rules for the given preset.
    ///
    /// `adminCidr` is used by RESTRICTIVE for management + app HTTP traffic.
    /// `internalCidr` is used by RESTRICTIVE for cluster + SWIM traffic.
    /// Both are ignored for STANDARD / OPEN / CUSTOM.
