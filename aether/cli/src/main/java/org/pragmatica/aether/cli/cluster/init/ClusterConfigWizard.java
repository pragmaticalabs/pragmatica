...
public class ClusterConfigWizard {
    ...
    private static StepResult stepClusterName(ClusterConfigAnswers state, Prompt prompt) {
        return guardedPrompt(prompt,
                             "Cluster name",
                             state.clusterName(),
                             raw -> validatedContinue(raw,
                                                      InputValidators::validateClusterName,
    ...
    private static StepResult stepDeploymentTarget(ClusterConfigAnswers state, Prompt prompt) {
        var target = prompt.choice("Deployment target",
                                   Arrays.asList(SourceType.values()),
                                   state.target());
    ...
    private static StepResult sshUserPrompt(ClusterConfigAnswers state,
        ...
        return guardedPrompt(prompt,
                             "SSH user",
                             defaultUser,
                             userRaw -> validatedContinue(userRaw,
                                                          ClusterConfigWizard::nonEmpty,
    ...
    private static StepResult sshPortPrompt(ClusterConfigAnswers state,
                                            List<String> hosts,
                                            String user,
                                            Path keyPath,
                                            SshAnswers existing,
        ...
        return guardedPrompt(prompt,
                             "SSH port",
                             defaultPort,
                             portRaw -> validatedContinue(portRaw,
                                                          InputValidators::validatePort,
                                                          _ -> sshPortPrompt(state,
                                                                             hosts,
                                                                             user,
                                                                             keyPath,
                                                                             existing,
                                                                             prompt),
                                                          port -> new StepResult.Continue(stateWithSsh(state,
                                                                                                       new SshAnswers(hosts,
                                                                                                                      user,
                                                                                                                      keyPath,
                                                                                                                      port)))));
    }
    ...
    private static StepResult stepTopology(ClusterConfigAnswers state, Prompt prompt) {
        if (state.target() == SourceType.SSH) {return sshDerivedTopology(state, prompt);}
    ...
    private static StepResult stepDatabase(ClusterConfigAnswers state, Prompt prompt) {
        ...
        return guardedPrompt(prompt,
                             "Database host",
                             defaultHost,
                             hostRaw -> validatedContinue(hostRaw,
    ...
    private static StepResult dbPasswordPrompt(ClusterConfigAnswers state,
                                               String host,
                                               int port,
                                               String name,
                                               String user,
            ...
            return guardedPrompt(prompt,
                                 ...
                                 envRaw -> validatedContinue(envRaw,
                                                             ...
                                                             envVar -> new StepResult.Continue(stateWithDatabase(state,
                                                                                                                 Option.some(new DatabaseAnswers(host,
                                                                                                                                                 port,
                                                                                                                                                 name,
                                                                                                                                                 user,
    ...
    private static ClusterConfigAnswers stateWithDatabase(ClusterConfigAnswers state,
    ...
    private static StepResult firewallInternalCidr(ClusterConfigAnswers state, String admin, Prompt prompt) {
        ...
        return guardedPrompt(prompt,
                             "Internal CIDR",
                             defaultInternal,
                             internalRaw -> validatedContinue(internalRaw,
                                                              InputValidators::validateCidr,
    ...
    private static TlsAnswers manualTls(Prompt prompt) {
        var certEnv = prompt.promptValidated("TLS cert path env var",
                                             "TLS_CERT_PATH",
                                             InputValidators::validateEnvVarName);
    ...
    private static ClusterConfigAnswers stateWithSecurity(ClusterConfigAnswers state,
                                                          TlsAnswers tls,
                                                          SecretAnswers secret) {
        return new ClusterConfigAnswers(state.clusterName(),
                                        state.clusterVersion(),
    ...
    private static StepResult guardedPrompt(Prompt prompt,
                                            String question,
                                            String defaultValue,
    ...
    private static <T> StepResult validatedContinue(String raw,
