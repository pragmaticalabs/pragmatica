...
@SuppressWarnings({"JBCT-SEQ-01", "JBCT-UTIL-02", "JBCT-EX-01"}) public final class ApplyOrchestrator {
    ...
    public static Result<ApplyResult> resume(ClusterBootstrapConfig desired, ClusterBootstrapConfig currentStored) {
        ...
        return ApplyState.load(clusterName).toResult(new ApplyAborted("No apply state found for cluster '" + clusterName + "'. Nothing to resume."))
    ...
    public static Result<ApplyResult> rollback(ClusterBootstrapConfig desired, ClusterBootstrapConfig currentStored) {
        var clusterName = desired.cluster().name();
        return ApplyState.load(clusterName).toResult(new ApplyAborted("No apply state found for cluster '" + clusterName + "'. Nothing to rollback."))
    ...
    private static Result<DiffPlan> rejectImmutableChanges(DiffPlan plan) {
        return plan.hasImmutableChanges()
              ? new ClusterConfigError.ImmutableFieldChange(plan.immutable().getFirst()
                                                                          .description()).result()
              : success(plan);
    }
    ...
    private static Result<Unit> checkClusterHealth(ClusterBootstrapConfig stored) {
        ...
        var managementPort = stored.operations().ports()
                                              .management();
    ...
    private static List<String> collectKnownAddresses(ClusterBootstrapConfig stored) {
        return stored.sources().values()
                             .stream()
                             .flatMap(s -> s.roles().values()
                                                  .stream())
    ...
    @Contract private static void warnHealthCheckFailed(Cause cause) {
        System.err.println("Warning: could not verify cluster health: " + cause.message() + ". Run 'aether cluster status' for details.");
    }
    ...
    @Contract private static void printPlan(String planText) {
        System.out.println();
    ...
    private static Result<ApplyResult> executeAndPersist(DiffPlan plan,
        ...
        return WaveExecutor.execute(plan, stored, desired).onSuccess(_ -> ApplyState.delete(clusterName))
                                   .onFailure(_ -> markStateFailed(state));
    }

    @Contract private static void markStateFailed(ApplyState state) {
    ...
    @Contract private static void logRollback(ApplyState state) {
    ...
    @Contract private static void destroyCreatedResources(ApplyState state) {
