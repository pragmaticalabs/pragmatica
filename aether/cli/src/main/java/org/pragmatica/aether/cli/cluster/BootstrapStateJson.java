...
sealed interface BootstrapStateJson {
    ...
    private static Map<String, SourceCleanupHandle> parseSources(JsonNode node) {
        ...
        for (var entry : node.properties()) {
            parseSourceCleanupHandle(entry.getValue()).onPresent(handle -> sources.put(entry.getKey(), handle));
        }
