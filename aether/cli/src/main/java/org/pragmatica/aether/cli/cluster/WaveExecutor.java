...
public final class WaveExecutor {
    ...
    public static Result<ApplyResult> execute(DiffPlan plan,
        ...
        return executeAdditions(plan.additions(), desired)
                   .flatMap(added -> executeModifications(plan.modifications())
                       .flatMap(modified -> executeRemovals(plan.removals(), stored)
                           .map(removed -> applyResult(plan, added, removed, modified))));
    }
    ...
    private static Result<Integer> provisionRole(String sourceName,
                                                  ...
                                                  int count,
                                                  ClusterBootstrapConfig desired) {
        return lookupSource(sourceName, desired.sources())
                   .flatMap(source -> dispatchProvision(sourceName, source, role, count))
                   .map(nodes -> logAndCount("+", sourceName + "." + role.value() + ": provisioned " + nodes.size() + " node(s)", nodes.size()));
    }
    ...
    private static Result<Integer> provisionScaleUp(String sourceName,
        ...
        var delta = to - from;
        return lookupSource(sourceName, desired.sources())
                   .flatMap(source -> rejectSshScaleUp(source, sourceName))
                   .flatMap(source -> dispatchProvision(sourceName, source, role, delta))
                   .map(nodes -> logAndCount("~", sourceName + "." + role.value() + ": scaled up by " + delta + " node(s)", nodes.size()));
    }
    ...
    private static Result<List<ProvisionedNode>> resolveCloudAndProvision(String sourceName,
        ...
        return ProviderResolver.resolveCloudCompute(source)
                   .flatMap(compute -> provisionViaCompute(compute, sourceName, role, count, source));
    }
    ...
    private static Result<List<ProvisionedNode>> provisionViaCompute(ComputeProvider compute,
                                                                      ...
                                                                      int count,
                                                                      SourceProfile source) {
        var instanceType = option(source.roles().get(role))
    ...
    private static Result<List<ProvisionedNode>> forgeProvisionPlaceholder(String sourceName,
                                                                            NodeRole role,
                                                                            int count) {
        logAction("+", sourceName + "." + role.value() + "/" + SourceType.FORGE.value()
                      + ": " + count + " in-process node(s) will be started by Forge");
    ...
    private static Result<List<ProvisionedNode>> sshProvisionPlaceholder(String sourceName,
                                                                          NodeRole role,
                                                                          SourceProfile source) {
        var hosts = option(source.roles().get(role))
                        .flatMap(rt -> rt.hosts())
                        .or(List.of());
        logAction("+", sourceName + "." + role.value() + "/ssh: " + hosts.size() + " pre-existing host(s) registered");
        var nodes = new ArrayList<ProvisionedNode>();
    ...
    @Contract
    private static void logModification(DiffAction action) {
        switch (action) {
            case DiffAction.RuntimeChange a ->
                logAction("~", a.sourceName() + "." + a.role().value()
                              + ": runtime change " + a.fromRuntime() + " -> " + a.toRuntime()
                              + " (rolling restart required)");
            case DiffAction.SourceFieldChange a ->
                logAction("~", a.sourceName() + ": " + a.field()
                              + " changed (replace-before-retire planned)");
            case DiffAction.ClusterLevelChange a ->
                logAction("~", "cluster." + a.field() + ": " + a.from() + " -> " + a.to()
                              + " (cluster-wide update)");
    ...
    private static Result<Integer> destroyEntireSource(String sourceName, ClusterBootstrapConfig stored) {
        return lookupSource(sourceName, stored.sources())
                   .flatMap(source -> destroyAllRoles(sourceName, source));
    }
    ...
    private static Result<Integer> destroyRole(String sourceName,
                                                ...
                                                ClusterBootstrapConfig stored) {
        return lookupSource(sourceName, stored.sources())
                   .flatMap(source -> dispatchDestroy(sourceName, source, role, count))
                   .map(_ -> logAndCount("-", sourceName + "." + role.value() + ": destroyed " + count + " node(s)", count));
    }
    ...
    private static Result<Integer> destroyScaleDown(String sourceName,
        ...
        var excess = from - to;
        return lookupSource(sourceName, stored.sources())
                   .flatMap(source -> dispatchDestroy(sourceName, source, role, excess))
                   .map(_ -> logAndCount("-", sourceName + "." + role.value() + ": scaled down by " + excess + " node(s) (LIFO)", excess));
    }
    ...
    private static Result<Unit> resolveCloudAndDestroy(SourceProfile source,
                                                        ...
                                                        int count) {
        return ProviderResolver.resolveCloudCompute(source)
                   .flatMap(compute -> destroyViaCompute(compute, sourceName, role, count));
    }

    private static Result<Unit> resolveDockerAndDestroy(String sourceName,
                                                         NodeRole role,
                                                         int count) {
        return ProviderResolver.resolveDockerCompute()
                   .flatMap(compute -> destroyViaCompute(compute, sourceName, role, count));
    ...
    private static Result<Unit> forgeDestroyPlaceholder(String sourceName, NodeRole role, int count) {
        logAction("-", sourceName + "." + role.value() + "/forge: " + count + " in-process node(s) will be stopped by Forge");
    ...
    private static Result<SourceProfile> lookupSource(String sourceName, Map<String, SourceProfile> sources) {
        return option(sources.get(sourceName))
                   .toResult(new ApplyError.SourceNotFound(sourceName));
    }
    ...
    /// Errors specific to apply/wave execution.
