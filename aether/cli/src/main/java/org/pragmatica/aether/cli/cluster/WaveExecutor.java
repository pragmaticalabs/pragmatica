...
@SuppressWarnings({"JBCT-SEQ-01", "JBCT-UTIL-02"}) public final class WaveExecutor {
    private WaveExecutor() {}

    public static Result<ApplyResult> execute(DiffPlan plan) {
        var nodesAdded = plan.additions().stream()
                                       .mapToInt(WaveExecutor::additionNodeCount)
                                       .sum();
        var nodesModified = plan.modifications().stream()
                                              .mapToInt(WaveExecutor::modificationNodeCount)
                                              .sum();
        var nodesRemoved = plan.removals().stream()
                                        .mapToInt(WaveExecutor::removalNodeCount)
                                        .sum();
