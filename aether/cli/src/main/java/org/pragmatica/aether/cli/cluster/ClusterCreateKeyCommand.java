...
@Command(name = "create-key", description = "Create a new cluster API key with an explicit authorization role") @SuppressWarnings({"JBCT-RET-01", "JBCT-PAT-01"}) class ClusterCreateKeyCommand implements Callable<Integer> {
    ...
    @Option(names = "--role", description = "Authorization role: ADMIN, OPERATOR, or VIEWER (default: VIEWER)", defaultValue = "VIEWER") private String role;

    @Option(names = "--name", description = "Human-readable name for the key (used in audit logs)", defaultValue = "") private String name;
    ...
    @Override public Integer call() {
        return clusterTarget.applyOverrides().flatMap(_ -> generateAndCreate())
    ...
    private Result<String> postCreateKey(String keyId, String keyHash, String authorizationRole) {
        ...
        var json = "{\"keyId\":\"" + keyId + "\",\"keyHash\":\"" + keyHash + "\",\"gracePeriodMs\":" + DEFAULT_GRACE_PERIOD_MS + ",\"auditAction\":\"CREATED\"" + ",\"operatorHint\":\"" + hint + "\"" + ",\"authorizationRole\":\"" + authorizationRole + "\"}";
    ...
    private static String buildSuccessJson(String keyId, String keyValue, String role) {
        return "{\"keyId\":\"" + keyId + "\",\"keyValue\":\"" + keyValue + "\",\"authorizationRole\":\"" + role + "\",\"status\":\"created\"}";
    }

    private int onSuccess(String json) {
        return OutputFormatter.printAction(json,
                                           parent.outputOptions(),
                                           "Key created with role " + role.toUpperCase() + ". Save the keyValue — the cluster only stores the hash.");
    }
