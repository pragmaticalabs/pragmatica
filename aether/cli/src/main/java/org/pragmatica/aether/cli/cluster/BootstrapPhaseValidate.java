...
@SuppressWarnings({"JBCT-SEQ-01", "JBCT-UTIL-02"}) sealed interface BootstrapPhaseValidate {
    ...
    static Result<BootstrapContext> execute(ClusterBootstrapConfig config, boolean fullCheck) {
        ...
        return ClusterBootstrapConfigValidator.validate(config).map(BootstrapPhaseValidate::emitWarnings)
    ...
    @Contract private static void printWarning(String warning) {
