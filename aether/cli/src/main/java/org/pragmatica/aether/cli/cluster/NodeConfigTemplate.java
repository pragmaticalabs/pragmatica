package org.pragmatica.aether.cli.cluster;

import org.pragmatica.aether.config.cluster.ClusterBootstrapConfig;

import java.util.List;
import java.util.stream.Collectors;
import java.util.stream.IntStream;


/// Generates per-node `aether.toml` configuration for on-premises deployment.
///
/// Produces a minimal config with node identity, cluster ports, peer list,
/// and TLS settings derived from the cluster bootstrap config.
sealed interface NodeConfigTemplate {
    record unused() implements NodeConfigTemplate{}

    static String render(ClusterBootstrapConfig config,
                         String nodeId,
                         int nodeIndex,
                         String clusterSecret,
                         List<String> allNodeIds,
                         List<String> allNodeIps) {
        var ports = config.operations().ports();
        var tlsEnabled = config.operations().tls()
                                          .autoGenerate();
        var coreCount = config.derivedCoreCount();
        var peers = buildPeersList(allNodeIds, allNodeIps, ports.cluster());
        var sb = new StringBuilder();
        sb.append("# Generated by: aether cluster bootstrap\n");
        sb.append("# Node: ").append(nodeId)
                 .append('\n');
        sb.append('\n');
        sb.append("[cluster]\n");
        sb.append("environment = \"docker\"\n");
        sb.append("nodes = ").append(coreCount)
                 .append('\n');
        sb.append("tls = ").append(tlsEnabled)
                 .append('\n');
        sb.append('\n');
        sb.append("[cluster.ports]\n");
        sb.append("management = ").append(ports.management())
                 .append('\n');
        sb.append("cluster = ").append(ports.cluster())
                 .append('\n');
        sb.append('\n');
        sb.append("[node]\n");
        sb.append("id = \"").append(nodeId)
                 .append("\"\n");
        sb.append("peers = \"").append(peers)
                 .append("\"\n");
        if (tlsEnabled) {
            sb.append('\n');
            sb.append("[tls]\n");
            sb.append("cluster_secret = \"").append(clusterSecret)
                     .append("\"\n");
        }
        return sb.toString();
    }

    private static String buildPeersList(List<String> nodeIds, List<String> nodeIps, int clusterPort) {
        return IntStream.range(0,
                               nodeIds.size()).mapToObj(i -> nodeIds.get(i) + ":" + nodeIps.get(i) + ":" + clusterPort)
                              .collect(Collectors.joining(","));
    }
}
