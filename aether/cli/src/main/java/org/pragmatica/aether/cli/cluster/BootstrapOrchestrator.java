...
sealed interface BootstrapOrchestrator {
    ...
    private static Result<String> resolveApiToken(ClusterManagementConfig config) {
        if (config.deployment()
                  .type() != DeploymentType.HETZNER) {
            return new BootstrapError.UnsupportedProvider(config.deployment()
                                                                .type()
                                                                .value()).result();
        }
    ...
    private static Result<BootstrapResult> waitAndFinalize(BootstrapContext ctx,
                                                           ...
                                                           String apiKey) {
        System.out.println("Step 7/12: Waiting for nodes to become healthy...");
        var managementPort = ctx.config()
                                .deployment()
                                .ports()
                                .management();
        var healthyCount = waitForHealth(nodes, managementPort);
        var requiredQuorum = quorumSize(ctx.config()
                                           .cluster()
                                           .core()
                                           .count());
        if (healthyCount < requiredQuorum) {
    ...
    private static Result<BootstrapResult> finalizeBootstrap(BootstrapContext ctx,
        ...
        var clusterName = ctx.config()
                             .cluster()
                             .name();
        var apiKeyEnvName = "AETHER_" + clusterName.toUpperCase()
                                                  .replace('-', '_') + "_API_KEY";
    ...
    private static Result<List<ProvisionedNode>> provisionInstances(BootstrapContext ctx) {
        ...
        for (int i = alreadyProvisioned; i < coreCount; i++) {
            var nodeId = generateNodeId(config.cluster()
                                              .name(),
                                        i);
    ...
    private static Result<ProvisionedNode> provisionSingleNode(String apiToken,
        ...
        var clusterName = config.cluster()
                                .name();
        var instanceType = config.deployment()
                                 .instances()
                                 .getOrDefault("core", "cx21");
        var location = firstZoneLocation(config);
        var image = config.deployment()
                          .runtime()
    ...
    private static String buildCreateServerJson(String name,
        ...
        return "{\"name\":\"" + name + "\"" + ",\"server_type\":\"" + serverType + "\"" + ",\"image\":\"" + image + "\""
               + ",\"location\":\"" + location + "\"" + ",\"user_data\":\"" + escapedUserData + "\""
               + ",\"start_after_create\":true" + ",\"labels\":{\"aether-cluster\":\"" + clusterName + "\""
               + ",\"aether-node-id\":\"" + name + "\"" + ",\"aether-role\":\"core\"}}";
    }
    ...
    private static void storeClusterConfig(String endpoint,
        ...
        var clusterName = config.cluster()
                                .name();
        var version = config.cluster()
                            .version();
        var coreCount = config.cluster()
                              .core()
                              .count();
        var deploymentType = config.deployment()
                                   .type()
                                   .value();
        var jsonBody = "{\"clusterName\":\"" + clusterName + "\"" + ",\"version\":\"" + version + "\""
                       + ",\"coreCount\":" + coreCount + ",\"deploymentType\":\"" + deploymentType + "\""
                       + ",\"configVersion\":1}";
    ...
    private static void registerLocally(String clusterName, String endpoint, String apiKeyEnvName) {
        var registryResult = ClusterRegistry.load()
    ...
    private static void printConnectionInfo(String clusterName,
        ...
        System.out.println();
        var envName = "AETHER_" + clusterName.toUpperCase()
                                            .replace('-', '_') + "_API_KEY";
