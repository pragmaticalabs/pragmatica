...
public sealed interface OutputFormatter {
    ...
    static int printQuery(String json, OutputOptions options, TableSpec tableSpec) {
        ...
        return switch (options.format()) {case JSON -> printJson(json);case VALUE -> printValue(json, options.field());case TABLE -> printTable(json,
                                                                                                                                                tableSpec);case CSV -> printCsv(json,
                                                                                                                                                                                tableSpec);};
    }
    ...
    static int printAction(String json, OutputOptions options, String successMessage) {
        if ( options.isQuiet()) {
        return ExitCode.SUCCESS;}
        return switch (options.format()) {case JSON -> printJson(json);case VALUE -> printValue(json, options.field());case TABLE -> printSuccessMessage(successMessage);case CSV -> printJson(json);};
    }
    ...
    static int printError(String message, OutputOptions options) {
        ...
        System.err.println("{\"error\":\"" + message.replace("\"", "\\\"") + "\"}");} else
        {
