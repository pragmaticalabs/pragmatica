...
@Command(name = "deploy", description = "Deploy and manage deployments", subcommands = {DeployCommand.ListCommand.class, DeployCommand.StatusCommand.class, DeployCommand.PromoteCommand.class, DeployCommand.RollbackCommand.class, DeployCommand.CompleteCommand.class})
@Contract
public class DeployCommand implements Callable<Integer> {
    @CommandLine.ParentCommand
    private AetherCli parent;

    @Parameters(index = "0", description = "Blueprint coordinates (group:artifact:version)", arity = "0..1")
    private String coordinates;

    @CommandLine.Option(names = "--canary", description = "Use canary deployment strategy")
    private boolean canary;

    @CommandLine.Option(names = "--blue-green", description = "Use blue-green deployment strategy")
    private boolean blueGreen;

    @CommandLine.Option(names = "--rolling", description = "Use rolling deployment strategy")
    ...
    @CommandLine.Option(names = "--drain-timeout", description = "Blue-green drain timeout in ms", defaultValue = "30000")
    ...
    @CommandLine.Option(names = "--timeout", description = "Wait timeout in seconds", defaultValue = "300")
    ...
    private String buildCanaryBody() {
        return "\"instances\":" + instances
             + ",\"canary\":{\"stages\":[{\"trafficPercent\":" + trafficPercent
             + ",\"observationMinutes\":10}]},\"thresholds\":{\"maxErrorRate\":" + errorRate
    ...
    private String buildBlueGreenBody() {
        return "\"instances\":" + instances
             + ",\"blueGreen\":{\"drainTimeoutMs\":" + drainTimeoutMs
             + "},\"thresholds\":{\"maxErrorRate\":" + errorRate
             + ",\"maxLatencyMs\":" + latencyMs
             + "}";
    }
    ...
    private String buildRollingBody() {
        return "\"instances\":" + instances
             + ",\"rolling\":{\"requireManualApproval\":" + manualApproval
             + "},\"thresholds\":{\"maxErrorRate\":" + errorRate
             + ",\"maxLatencyMs\":" + latencyMs
             + "}";
    }
    ...
    static class ListCommand implements Callable<Integer> {
        @CommandLine.ParentCommand
        private DeployCommand deployParent;
    ...
    @Command(name = "status", description = "Show deployment status")
    ...
    @Command(name = "promote", description = "Advance deployment (increase traffic or switch environment)")
    static class PromoteCommand implements Callable<Integer> {
        ...
        @CommandLine.Option(names = "--traffic", description = "New traffic percentage for canary (0-100)")
    ...
    @Command(name = "complete", description = "Finalize deployment")
