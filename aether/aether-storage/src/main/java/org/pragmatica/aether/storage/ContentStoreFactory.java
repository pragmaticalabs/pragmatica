...
public final class ContentStoreFactory implements ResourceFactory<ContentStore, ContentStoreConfig> {
    ...
    // slice's unload would shut the node's content storage under every other slice. The override
    // that used to sit here returned unitPromise() — a no-op that reported success and was
    // indistinguishable from a working close. Falling through to ResourceFactory's default
    // dispatch names the "no close convention" outcome and logs it instead.
}
