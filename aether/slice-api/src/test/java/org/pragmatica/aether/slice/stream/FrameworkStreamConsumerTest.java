...
class FrameworkStreamConsumerTest {
    ...
    class SealedHierarchy {
        ...
        @Test
        void frameworkStreamConsumer_permitsOnlyFrameworkRecord() {
            var permitted = FrameworkStreamConsumer.class.getPermittedSubclasses();

            assertThat(permitted).hasSize(1);
            assertThat(permitted[0].getSimpleName()).isEqualTo("SystemStreamConsumer");
        ...
        void permittedImplementation_isPackagePrivate() {
            var permitted = FrameworkStreamConsumer.class.getPermittedSubclasses()[0];
