...
public enum ManagementRoute {
    ...
    // Namespace-based stream API (spec event-stream-namespaces §13).
    // URL scheme: /api/streams/<verb>/<address...> — all literals before any params (rc1 positional router constraint).
    ...
    STREAMS_PUBLISH_BATCH(POST, "/api/streams/publish-batch", List.of("namespace", "stream", "version"), taskGroup(STREAMING)),
    STREAMS_GROUP_CREATE(POST, "/api/streams/groups/create", List.of("namespace", "stream", "version"), taskGroup(STREAMING)),
    STREAMS_GROUP_DELETE(DELETE, "/api/streams/groups/delete", List.of("namespace", "stream", "version", "group"), taskGroup(STREAMING)),
