...
public final class RouteAssembler {
    ...
    public static Result<String> assemble(ManagementRoute route, List<String> values) {
        if (values.size() != route.paramCount()) {
            return ManagementRouteError.wrongParamCount(route.name(), route.paramCount(), values.size()).result();
        }
        var sb = new StringBuilder(route.prefix());
        for (int i = 0; i < values.size(); i++) {
            var value = values.get(i);
            if (value == null) {
                return ManagementRouteError.missingParam(route.name(), route.paramNames().get(i)).result();
            }
            sb.append('/').append(URLEncoder.encode(value, StandardCharsets.UTF_8).replace("+", "%20"));
        }
