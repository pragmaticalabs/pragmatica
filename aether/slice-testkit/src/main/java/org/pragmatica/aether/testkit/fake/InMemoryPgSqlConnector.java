...
/// In-memory, statement-recording [PgSqlConnector] fake for `@PgSql` slices (spec §3.3). Same
/// contract as [InMemorySqlConnector]; runs no SQL. Use `Containers.postgres()` for real
/// PostgreSQL semantics and migrations.
public final class InMemoryPgSqlConnector implements PgSqlConnector {
    private final StatementRecorder recorder = new StatementRecorder();

    private InMemoryPgSqlConnector() {}

    public static InMemoryPgSqlConnector scripted() {
        return new InMemoryPgSqlConnector();
    }

    /// Script a single row returned by any query whose SQL contains `sqlSubstring`.
    public InMemoryPgSqlConnector onRow(String sqlSubstring, Map<String, Object> row) {
    ...
    public InMemoryPgSqlConnector onRows(String sqlSubstring, List<Map<String, Object>> rows) {
    ...
    public InMemoryPgSqlConnector onUpdate(String sqlSubstring, int affected) {
