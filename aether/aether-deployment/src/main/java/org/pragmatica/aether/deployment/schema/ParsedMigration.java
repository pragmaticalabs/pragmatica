...
public record ParsedMigration( MigrationEntry entry, MigrationType type, int version, String description) {
    ...
    private static Result<ParsedMigration> parseFilename(MigrationEntry entry, String filename) {
        ...
        return switch (prefix) {case 'R' -> parseRepeatable(entry, filename);case 'V' -> parseVersioned(entry,
                                                                                                        filename,
                                                                                                        MigrationType.VERSIONED);case 'U' -> parseVersioned(entry,
                                                                                                                                                            filename,
                                                                                                                                                            MigrationType.UNDO);case 'B' -> parseVersioned(entry,
                                                                                                                                                                                                           filename,
                                                                                                                                                                                                           MigrationType.BASELINE);default -> invalidMigrationFormat(filename,
                                                                                                                                                                                                                                                                     "unknown prefix '" + prefix + "', expected V/R/U/B").result();};
    }
