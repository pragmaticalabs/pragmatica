...
class MembershipFsmTest {
    ...
    /// seed-500 part 2 — the DEPARTING-edge listener ([`MembershipFsm#onEnteredDeparting`]) wired
    /// in AetherNode to the DHT ring-prune. It fires ONCE per fresh edge INTO DEPARTING, for the
    /// deliberate-departure ingresses (`DrainRequested` / graceful `SwimDeparted` /
    /// `DownHysteresisMet`) and NEVER for a transient transport-doubt flap — which only reaches
    /// SUSPECT in the [`MembershipState`] table, never DEPARTING. The fix that proves the SAFETY
    /// landmine (the reverted `d3e54717e` pruned on every `PeerDisconnected`): the prune side
    /// effect cannot be reached by a QUIC flap because the flap never enters DEPARTING.
    @Nested
    class DepartingEdgePrune {
        @Test
        ...
        @Test
        void drainThenDead_firesDepartingListenerExactlyOnce_idempotentWithDeadEdge() {
            var manager = activeManager();
            var pruned = new ArrayList<NodeId>();
    ...
    /// seed-500 part 2 (symmetry fix) — the DEPARTING-RECOVERY listener
    /// ([`MembershipFsm#onDepartingRecovery`]) wired in AetherNode to the DHT ring RE-ADD. It fires
    /// ONCE per fresh DEPARTING→MEMBER recovery (`recoverFromDepartingIfNewer`: a strictly-newer
    /// incarnation refuting the drain), undoing the [`#onEnteredDeparting`] ring prune. It is scoped
    /// to that recovery edge ONLY: a normal OBSERVED→MEMBER join (already covered by
    /// JOINED→`NodeJoined`→`onNodeJoined`) and a SUSPECT→MEMBER flap-recovery (never pruned) do NOT
    /// fire it, and a same-incarnation liveness in DEPARTING (no recovery) does not either.
    @Nested
    class DepartingRecoveryRingReadd {
        @Test
        ...
        @Test
        void prunedThenRecovered_pruneFiresOnce_recoveryFiresOnce_symmetric() {
            var manager = activeManager();
            var pruned = new ArrayList<NodeId>();
            var recovered = new ArrayList<NodeId>();
            manager.onEnteredDeparting(pruned::add);
    ...
    /// Join-grace-reap ACTUATION edge ([`MembershipFsm#onJoinGraceReap`]): the dedicated leader-side
    /// callback that closes the never-healthy-zombie leak — a CTM-provisioned replacement reaped
    /// OBSERVED→DEAD by the join-grace reaper (it booted but never reached SWIM-healthy) must have its
    /// lingering container/JVM terminated. The callback fires ONLY for the
    /// `JoinGraceExpiredNeverHealthy` → DEAD edge (uniquely from OBSERVED per the state table), NEVER
    /// for the other DEAD paths (graceful `SwimDeparted`, co-confirmed eviction). It is ADDITIVE to
    /// [`MembershipFsm#onConfirmedDeparture`], which still fires for every DEAD path.
    @Nested
    class JoinGraceReapActuation {
        ...
        /// The reap edge fires the dedicated actuation callback EXACTLY ONCE with the reaped id (a
        /// disconnected, never-healthy joiner is a true ghost → OBSERVED→DEAD via the reaper).
        @Test
        ...
        /// the new actuation callback does not displace it.
        @Test
        void onJoinGraceReap_neverHealthyReap_alsoFiresConfirmedDeparture() {
            var manager = joinGraceManager(FIRING_GRACE);
            var reaped = new ArrayList<NodeId>();
        ...
        /// NOT fire the join-grace-reap actuation, while it DOES fire the all-paths departure hook.
        @Test
        void onJoinGraceReap_gracefulDeparture_doesNotFire() {
            var manager = joinGraceManager(FIRING_GRACE);
            var reaped = new ArrayList<NodeId>();
            var departed = new ArrayList<NodeId>();
            manager.onJoinGraceReap(reaped::add);
        ...
        /// NEGATIVE: a co-confirmed eviction (SWIM-FAULTY ∧ liveness-gone) DEAD edge must NOT fire the
        /// join-grace-reap actuation (that path is handled by the transport link drop, not container
        /// termination), while it DOES fire the all-paths departure hook.
        @Test
        void onJoinGraceReap_coConfirmedEviction_doesNotFire() {
            var manager = joinGraceManager(FIRING_GRACE);
            var reaped = new ArrayList<NodeId>();
            var departed = new ArrayList<NodeId>();
            manager.onJoinGraceReap(reaped::add);
        ...
        /// Passing `null` resets the actuation listener to the no-op — a subsequent reap does not throw
        /// and does not invoke a stale listener.
        @Test
    ...
    /// Fix-1 (scale-down zombie-write, 2026-06-12): the DHT routing projection
    /// [`MembershipFsm#dhtRoutableMembers`] is NOT-DEAD MINUS DEPARTING (OBSERVED + MEMBER +
    /// SUSPECT). It is strictly narrower than [`#broadcastEligibleMembers`] (which keeps DEPARTING
    /// reachable for consensus broadcast + forward-routing). A drained node — driven to DEPARTING
    /// by the operator/CTM drain command (`onDrainRequested`) the instant it is commanded, ~seconds
    /// before it halts — must drop out of the DHT routing view AT THE DRAIN EDGE, so a write
    /// arriving shortly after a scale-down never selects the about-to-halt drainer as a replica
    /// target. Driven deterministically by direct ingress events (no SWIM/clock wait).
    @Nested
    class DhtRoutableMembers {
        @Test
        void dhtRoutableMembers_memberAndSuspectAndObserved_areAllRoutable() {
            var manager = activeManager();
        ...
        /// The crux: a member commanded to drain (DEPARTING) is EXCLUDED from the DHT routing view
        /// immediately on the drain edge — while it is still present in the broader
        /// broadcast-eligible set (consensus must keep reaching it as it drains in-flight work).
        @Test
        void dhtRoutableMembers_drainedMember_isExcludedButStaysBroadcastEligible() {
            var manager = activeManager();
        ...
        @Test
        void dhtRoutableMembers_deadMember_isExcluded() {
            var manager = activeManager();
        ...
        /// A drainer that RECOVERS (newer-incarnation SwimHealthy in DEPARTING → MEMBER) returns to
        /// the DHT routing view — the exclusion tracks live state, it is not a one-way latch.
        @Test
        void dhtRoutableMembers_drainerRecovers_returnsToRoutableSet() {
            var manager = activeManager();
