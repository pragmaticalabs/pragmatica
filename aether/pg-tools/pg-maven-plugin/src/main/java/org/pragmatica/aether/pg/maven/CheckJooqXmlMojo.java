...
public class CheckJooqXmlMojo extends AbstractMojo {
    ...
    @Override public void execute() throws MojoExecutionException {
        ...
        if (generated.isFailure()) {
            throw new MojoExecutionException("XML export failed: " + generated);
        }
        var existing = readExistingFile();
        var generatedXml = generated.unwrap();
        if (existing.equals(generatedXml)) {
            getLog().info("OK — jOOQ XML schema is up to date (" + schema.unwrap().tables().size() + " tables)");
        } else {
            reportDiff(existing, generatedXml);
            throw new MojoExecutionException(
                    "jOOQ XML schema is stale. Run 'mvn pg:export-jooq-xml' to regenerate.");
        }
    }
    ...
    private void reportDiff(String existing, String generated) {
        ...
        for (int i = 0; i < maxLen && diffCount < MAX_DIFF_LINES; i++) {
            var existingLine = i < existingLines.length ? existingLines[i] : "";
            var generatedLine = i < generatedLines.length ? generatedLines[i] : "";
        ...
        if (diffCount >= MAX_DIFF_LINES) {
            getLog().error("  ... (showing first " + MAX_DIFF_LINES + " differences)");
        }
    }
    ...
    private String readExistingFile() throws MojoExecutionException {
        try {
