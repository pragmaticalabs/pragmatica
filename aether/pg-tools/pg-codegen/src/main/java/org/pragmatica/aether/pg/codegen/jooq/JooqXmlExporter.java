...
public final class JooqXmlExporter {
    ...
    /// Pure transform: Schema → XML string.
    ...
    /// Convenience: write XML to a file.
    ...
    private static void emitColumns(XMLStreamWriter w, List<TableWithSchema> tables,
                                    String defaultSchema, JooqXmlConfig config) throws XMLStreamException {
    ...
    private static void emitColumn(XMLStreamWriter w, TableWithSchema tw, Column col,
                                   int ordinal, String defaultSchema, JooqXmlConfig config) throws XMLStreamException {
        ...
        typeMapping.characterMaximumLength().onPresent(v -> writeElementUnchecked(w, "character_maximum_length", String.valueOf(v)));
    ...
    private static void emitConstraints(XMLStreamWriter w, List<TableWithSchema> tables,
    ...
    private static void emitKeyColumnUsages(XMLStreamWriter w, TableWithSchema tw,
                                            Constraint constraint, JooqXmlConfig config) throws XMLStreamException {
    ...
    private static void emitCheckConstraint(XMLStreamWriter w, TableWithSchema tw,
                                            Check check, JooqXmlConfig config) throws XMLStreamException {
    ...
    private static void emitSequences(XMLStreamWriter w, Schema schema, JooqXmlConfig config) throws XMLStreamException {
        var sequences = schema.sequences().values().stream()
                              .filter(s -> isIncluded(s.schema(), config))
                              .toList();
        ...
        for (var seq : sequences) {
            ...
            writeElement(w, "sequence_schema", seq.schema().isEmpty() ? config.defaultSchemaName() : seq.schema());
            ...
            writeElement(w, "cycle_option", seq.cycle() ? "YES" : "NO");
    ...
    private static void emitIndexes(XMLStreamWriter w, List<TableWithSchema> tables,
                                    JooqXmlConfig config) throws XMLStreamException {
        ...
        for (var tw : tables) {
            ...
            for (var idx : indexes) {
                ...
                writeElement(w, "table_schema", tw.schema());
                writeElement(w, "table_name", tw.table().name());
                writeElement(w, "is_unique", idx.unique() ? "YES" : "NO");
                w.writeEndElement();
            }
        }
        w.writeEndElement();
        ...
        for (var tw : tables) {
            for (var idx : tw.table().indexes()) {
                for (int i = 0; i < idx.elements().size(); i++) {
                    ...
                    writeElement(w, "table_schema", tw.schema());
                    writeElement(w, "table_name", tw.table().name());
                    writeElement(w, "column_name", idx.elements().get(i).expression());
                    writeElement(w, "ordinal_position", String.valueOf(i + 1));
                    w.writeEndElement();
                }
            }
    ...
    private static void emitEnums(XMLStreamWriter w, Schema schema, String defaultSchema) throws XMLStreamException {
        var enums = schema.enumTypes().values().stream()
                          .sorted(Comparator.comparing(PgType.EnumType::name))
                          .toList();
        if (enums.isEmpty()) return;

        w.writeStartElement("enums");
        for (var e : enums) {
            var enumSchema = e.schema().isEmpty() ? defaultSchema : e.schema();
    ...
    private static List<TableWithSchema> collectTables(Schema schema, JooqXmlConfig config) {
        var tables = schema.tables().values().stream()
                           .filter(t -> isIncluded(t.schema(), config))
                           .map(t -> new TableWithSchema(t, t.schema().isEmpty() ? config.defaultSchemaName() : t.schema()));
        if (config.sortElements()) {
            tables = tables.sorted(Comparator.comparing(TableWithSchema::schema).thenComparing(tw -> tw.table().name()));
        }
    ...
    private static boolean isIncluded(String schemaName, JooqXmlConfig config) {
        ...
        if (schemaName.isEmpty()) return config.includedSchemas().contains(config.defaultSchemaName());
        return config.includedSchemas().contains(schemaName);
    }
