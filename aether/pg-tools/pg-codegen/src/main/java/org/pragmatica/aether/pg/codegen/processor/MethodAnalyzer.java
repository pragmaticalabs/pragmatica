...
public final class MethodAnalyzer {
    ...
    /// PostgreSQL SERIAL family pseudo-types. Each is effectively an integer column with a
    /// backing sequence — PostgreSQL fills in the value automatically on INSERT. They must be
    /// exempted from NOT NULL coverage checks even when the schema builder does not materialize
    /// the implicit default expression or identity spec.
    private static final Set<String> AUTO_GENERATED_TYPE_NAMES = Set.of("serial",
                                                                        "smallserial",
                                                                        "bigserial");
