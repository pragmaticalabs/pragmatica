...
public final class RecordGenerator {
    ...
    public Result<GeneratedFile> generate(Table table) {
        ...
        for (var col : table.columns()) {
            ...
            if (typeInfo.isEmpty()) {
                return Result.failure(new CodegenError.UnsupportedType(col.type().name()));
            }
    ...
    private String renderRecord(String className, java.util.List<FieldInfo> fields, TreeSet<String> imports) {
        ...
        // Record declaration
        sb.append("/// Generated from table: ").append(fields.isEmpty() ? "unknown" : fields.getFirst().columnName()).append("\n");
    ...
    private void renderRowMapper(StringBuilder sb, String className, java.util.List<FieldInfo> fields) {
    ...
    private void renderSimpleRowMapper(StringBuilder sb, String className, java.util.List<FieldInfo> fields) {
    ...
    private void renderNestedRowMapper(StringBuilder sb, String className, java.util.List<FieldInfo> fields) {
        // Split into groups of 9 for nested Result.all
        var groups = new ArrayList<java.util.List<FieldInfo>>();
        for (int i = 0; i < fields.size(); i += 9) {
            groups.add(fields.subList(i, Math.min(i + 9, fields.size())));
        }
        ...
        // Flatten tuple access — simplified, just reference fields directly
    ...
    record FieldInfo(String columnName, String fieldName, String fieldType, boolean nullable,
                     TypeMapper.JavaTypeInfo typeInfo) {}

    /// Interface placeholder for row access — to be provided by the database driver.
