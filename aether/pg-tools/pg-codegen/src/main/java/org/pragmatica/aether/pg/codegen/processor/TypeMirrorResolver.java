...
@Contract public final class TypeMirrorResolver {
    ...
    private static ResolvedReturn resolveInnerType(TypeMirror inner, Types typeUtils) {
        if ( inner instanceof DeclaredType declaredInner) {
            ...
            return switch (innerName) {case "Unit" -> new ResolvedReturn(MethodAnalyzer.ReturnKind.UNIT,
                                                                         "Unit",
                                                                         "Unit",
                                                                         false);case "Long" -> resolveMaybeBoxedLong(qualifiedName);case "Boolean" -> resolveMaybeBoxedBoolean(qualifiedName);case "Option" -> resolveOptionType(declaredInner);case "List" -> resolveListType(declaredInner);default -> new ResolvedReturn(MethodAnalyzer.ReturnKind.SINGLE,
    ...
    private static boolean isComplexType(String typeName) {
        return switch (typeName) {case "String", "Long", "Integer", "Boolean", "Double", "Float", "Short", "Byte", "Unit" -> false;default -> true;};
    }
}
