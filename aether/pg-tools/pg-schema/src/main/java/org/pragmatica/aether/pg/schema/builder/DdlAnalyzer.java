...
public final class DdlAnalyzer {
    ...
    public static Result<List<SchemaEvent>> analyze(CstNode cst) {
        ...
        if (nav.isEmpty()) {return Result.success(List.of());}
    ...
    private static void analyzeCreateTable(CstNavigator stmt, List<SchemaEvent> events) {
        ...
        for (var elem : tableElements) {if (elem.has("ColId") && elem.has("DataType")) {extractColumn(elem,
                                                                                                      columns,
                                                                                                      constraints);} else if (elem.has("TableConstraintElem") || elem.has("PrimaryKW") || elem.has("ForeignKW") || elem.has("UniqueKW") || elem.has("CheckKW") || elem.has("ExcludeKW")) {extractTableConstraint(elem,
                                                                                                                                                                                                                                                                                                                 constraints);} else {
            var tblConstraint = elem.child("TableConstraint");
            if (tblConstraint.isPresent()) {extractTableConstraint(tblConstraint.unwrap(), constraints);} else if (elem.has("ConstraintKW")) {extractTableConstraint(elem,
                                                                                                                                                                     constraints);}
        }}
        Option<Table.PartitionBy> partitioning = Option.empty();
        var partClause = stmt.findAll("PartitionByClause");
        if (!partClause.isEmpty()) {partitioning = extractPartitioning(partClause.getFirst());}
    ...
    private static void extractColumn(CstNavigator elem, List<Column> columns, List<Constraint> constraints) {
        ...
        for (var cc : colConstraints) {
            boolean hasGeneratedKw = !cc.findAll("GeneratedKW").isEmpty();
            boolean hasIdentityKw = !cc.findAll("IdentityKW").isEmpty();
            boolean hasStoredKw = !cc.findAll("StoredKW").isEmpty();
            // GENERATED ALWAYS AS IDENTITY / GENERATED BY DEFAULT AS IDENTITY
            if (hasGeneratedKw && hasIdentityKw) {
                identity = Option.present(extractIdentitySpec(cc));
                nullable = false;
                continue;
            }
            // GENERATED ALWAYS AS (expr) STORED
            if (hasGeneratedKw && hasStoredKw) {
            ...
            if (hasNot && hasNull) {nullable = false;} else if (hasNull && !hasNot) {nullable = true;} else if (hasDefault) {defaultExpr = Option.present(extractExprText(cc));} else if (hasPrimary) {
                constraints.add(new Constraint.PrimaryKey(Option.empty(), List.of(colName)));
                nullable = false;
            } else if (hasUnique) {constraints.add(new Constraint.Unique(Option.empty(), List.of(colName)));} else if (hasReferences) {extractInlineReference(cc,
                                                                                                                                                              colName,
                                                                                                                                                              constraints);}
        }
        var col = Column.column(colName, pgType, nullable);
        if (defaultExpr.isPresent()) {col = col.withDefault(defaultExpr.unwrap());}
        if (generatedExpr.isPresent() || identity.isPresent()) {
            col = new Column(col.name(), col.type(), col.nullable(),
                             col.defaultExpr(), generatedExpr, identity, col.comment());
        }
        columns.add(col);
    }

    private static Column.IdentitySpec extractIdentitySpec(CstNavigator cc) {
        // BY DEFAULT uses `ByKW DefaultKW`; ALWAYS uses `AlwaysKW`. Both appear as
        // flattened terminals within the ColConstraint since the parser may elide
        // the surrounding IdentityClause/GeneratedClause wrapper nodes.
    ...
    private static void extractTableConstraint(CstNavigator tblConstraint, List<Constraint> constraints) {
        ...
        } else if (e.has("UniqueKW")) {
            var cols = extractConstraintColumns(e);
            constraints.add(new Constraint.Unique(constraintName, cols));
        } else if (e.has("CheckKW")) {constraints.add(new Constraint.Check(constraintName, extractExprText(e)));} else if (e.has("ForeignKW")) {extractTableForeignKey(e,
                                                                                                                                                                       constraintName,
                                                                                                                                                                       constraints);} else if (e.has("ExcludeKW")) {constraints.add(new Constraint.Exclusion(constraintName,
                                                                                                                                                                                                                                                             "gist",
                                                                                                                                                                                                                                                             extractExprText(e)));}
    }
    ...
    private static void analyzeAlterTable(CstNavigator stmt, List<SchemaEvent> events) {
        ...
        if (actions.isEmpty()) {analyzeAlterTableAction(stmt, tableName, span, events);} else {for (var action : actions) {analyzeAlterTableAction(action,
                                                                                                                                                   tableName,
                                                                                                                                                   span,
                                                                                                                                                   events);}}
    }
    ...
    private static void analyzeAlterTableAction(CstNavigator action,
        ...
        if (action.has("DropKW") && (action.has("ColumnKW") || (!action.has("ConstraintKW") && !action.findAll("DropColumnAction")
                                                                                                              .isEmpty()))) {
            var colIds = action.findAll("ColId");
            if (!colIds.isEmpty()) {events.add(new SchemaEvent.ColumnDropped(span,
                                                                             tableName,
                                                                             CstExtractor.extractIdentifier(colIds.getFirst())
                                                                                                           .normalized()));}
            return;
        }
        if (action.has("DropKW") && action.has("ConstraintKW")) {
            var colIds = action.findAll("ColId");
            if (!colIds.isEmpty()) {events.add(new SchemaEvent.ConstraintDropped(span,
                                                                                 tableName,
                                                                                 CstExtractor.extractIdentifier(colIds.getFirst())
                                                                                                               .normalized()));}
            return;
    ...
    private static void analyzeAlterColumnCmd(CstNavigator cmd,
        ...
        } else if (hasSet && hasNot && hasNull) {events.add(new SchemaEvent.ColumnNullabilityChanged(span,
                                                                                                     tableName,
                                                                                                     colName,
                                                                                                     false));} else if (hasDrop && hasNot && hasNull) {events.add(new SchemaEvent.ColumnNullabilityChanged(span,
                                                                                                                                                                                                           tableName,
                                                                                                                                                                                                           colName,
                                                                                                                                                                                                           true));} else if (hasSet && hasDefault) {events.add(new SchemaEvent.ColumnDefaultChanged(span,
                                                                                                                                                                                                                                                                                                    tableName,
                                                                                                                                                                                                                                                                                                    colName,
                                                                                                                                                                                                                                                                                                    Option.present(extractExprText(cmd))));} else if (hasDrop && hasDefault) {events.add(new SchemaEvent.ColumnDefaultChanged(span,
                                                                                                                                                                                                                                                                                                                                                                                                                              tableName,
                                                                                                                                                                                                                                                                                                                                                                                                                              colName,
                                                                                                                                                                                                                                                                                                                                                                                                                              Option.empty()));}
    }
    ...
    private static void analyzeCreateIndex(CstNavigator stmt, CstNavigator root, List<SchemaEvent> events) {
        ...
        String indexName = "";
        String tableName = "";
        if (!qnames.isEmpty()) {tableName = CstExtractor.extractQualifiedName(qnames.getFirst()).normalized();}
        if (!colIds.isEmpty() && !qnames.isEmpty()) {
            ...
            if (firstColId.span().start()
                ...
                if (qnames.size() > 1) {tableName = CstExtractor.extractQualifiedName(qnames.get(1)).normalized();}
            }
        }
    ...
    private static void analyzeDropIndex(CstNavigator stmt, List<SchemaEvent> events) {
        var qnames = stmt.findAll("QualifiedName");
        for (var qnav : qnames) {events.add(new SchemaEvent.IndexDropped(stmt.span(),
                                                                         CstExtractor.extractQualifiedName(qnav)
                                                                                                          .normalized()));}
    }
    ...
    private static void analyzeDropSequence(CstNavigator stmt, List<SchemaEvent> events) {
        var qnames = stmt.findAll("QualifiedName");
        for (var qnav : qnames) {events.add(new SchemaEvent.SequenceDropped(stmt.span(),
                                                                            CstExtractor.extractQualifiedName(qnav)
                                                                                                             .normalized()));}
    }
    ...
    private static void analyzeDropType(CstNavigator stmt, List<SchemaEvent> events) {
        var qnames = stmt.findAll("QualifiedName");
        for (var qnav : qnames) {events.add(new SchemaEvent.TypeDropped(stmt.span(),
                                                                        CstExtractor.extractQualifiedName(qnav)
                                                                                                         .normalized()));}
    }
    ...
    private static void analyzeCreateSchema(CstNavigator stmt, List<SchemaEvent> events) {
        ...
        if (colId.isPresent()) {events.add(new SchemaEvent.SchemaCreated(stmt.span(),
                                                                         CstExtractor.extractIdentifier(colId.unwrap())
                                                                                                       .normalized()));}
    }

    private static void analyzeCreateExtension(CstNavigator stmt, List<SchemaEvent> events) {
        var colId = stmt.child("ColId");
        if (colId.isPresent()) {events.add(new SchemaEvent.ExtensionCreated(stmt.span(),
                                                                            CstExtractor.extractIdentifier(colId.unwrap())
                                                                                                          .normalized()));}
    }
    ...
    private static Constraint.FkAction extractFkAction(CstNavigator nav, String actionKeyword) {
        ...
        for (var action : actions) {if (action.has(actionKeyword)) {
