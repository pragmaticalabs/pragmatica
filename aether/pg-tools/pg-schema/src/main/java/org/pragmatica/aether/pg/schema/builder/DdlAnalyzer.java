...
public final class DdlAnalyzer {
    ...
    private static void analyzeCreateTable(CstNavigator stmt, List<SchemaEvent> events) {
        ...
        for (var elem : tableElements) {
            ...
            } else if (elem.has("TableConstraintElem") || elem.has("PrimaryKW") || elem.has("ForeignKW")
    ...
    private static void extractInlineReference(CstNavigator refClause, String colName, List<Constraint> constraints) {
        ...
        var refColumns = refClause.child("ColumnList")
            ...
            .map(ids -> ids.stream().map(id -> id.normalized()).toList())
            .or(List.of());

        var onUpdate = extractFkAction(refClause, "UpdateKW");
        var onDelete = extractFkAction(refClause, "DeleteKW");

        constraints.add(new Constraint.ForeignKey(
            Option.empty(), List.of(colName), refTableName, refColumns, onUpdate, onDelete));
    }
    ...
    private static void analyzeAlterTable(CstNavigator stmt, List<SchemaEvent> events) {
        ...
        if (!stmt.findAll("RenameKW").isEmpty()) {
            ...
            int tablePartCount = tableQname.isPresent() ? tableQname.unwrap().parts().size() : 0;
            var renameColIds = allColIds.subList(Math.min(tablePartCount, allColIds.size()), allColIds.size());
        ...
        if (setSchema.isPresent()) return; // TODO: handle

        // Attach/Detach partition
        if (stmt.has("AttachKW") || stmt.has("DetachKW")) return; // TODO: handle
    ...
    private static void analyzeAlterTableAction(CstNavigator action, String tableName, SourceSpan span,
        ...
        if (action.has("DropKW") && (action.has("ColumnKW") || (!action.has("ConstraintKW") && !action.findAll("DropColumnAction").isEmpty()))) {
            ...
            var colIds = action.findAll("ColId");
            if (!colIds.isEmpty()) {
                events.add(new SchemaEvent.ColumnDropped(span, tableName,
        ...
        if (action.has("DropKW") && action.has("ConstraintKW")) {
            ...
            var colIds = action.findAll("ColId");
            if (!colIds.isEmpty()) {
                events.add(new SchemaEvent.ConstraintDropped(span, tableName,
                    CstExtractor.extractIdentifier(colIds.getFirst()).normalized()));
            }
            return;
        }
    ...
    private static Option<CstNavigator> findFirst(CstNavigator nav, String ruleName) {
        ...
        return found.isEmpty() ? Option.empty() : Option.present(found.getFirst());
    }

    private static void analyzeAlterColumnCmd(CstNavigator cmd, String tableName, String colName,
                                               SourceSpan span, List<SchemaEvent> events) {
        ...
        } else if (hasDrop && hasNot && hasNull) {
            // DROP NOT NULL
            events.add(new SchemaEvent.ColumnNullabilityChanged(span, tableName, colName, true));
        } else if (hasSet && hasDefault) {
            // SET DEFAULT
            events.add(new SchemaEvent.ColumnDefaultChanged(span, tableName, colName,
                Option.present(extractExprText(cmd))));
        } else if (hasDrop && hasDefault) {
            // DROP DEFAULT
    ...
    private static void analyzeCreateIndex(CstNavigator stmt, CstNavigator root, List<SchemaEvent> events) {
        ...
        var elements = indexElems.stream().map(e -> {
    ...
    private static void analyzeCreateType(CstNavigator stmt, List<SchemaEvent> events) {
        ...
        if (!compositeFields.isEmpty()) {
            var fields = compositeFields.stream().map(f -> {
                var fieldName = f.child("ColId").map(CstExtractor::extractIdentifier).map(id -> id.normalized()).or("?");
                var fieldType = f.child("DataType").map(CstExtractor::extractDataType).map(DdlAnalyzer::resolveType)
    ...
    private static void analyzeAlterType(CstNavigator stmt, List<SchemaEvent> events) {
        ...
        if (stmt.has("ValueKW")) {
            ...
            if (!stringLiterals.isEmpty()) {
                var value = stringLiterals.getFirst().firstTokenText().or("");
                Option<String> before = stmt.has("BeforeKW") && stringLiterals.size() > 1
                    ? Option.present(stringLiterals.get(1).firstTokenText().or(""))
                    : Option.empty();
    ...
    private static void analyzeComment(CstNavigator stmt, List<SchemaEvent> events) {
        ...
        if (!targetType.isEmpty()) {
            ...
            Option<String> comment = !stringLit.isEmpty()
                ? Option.present(stringLit.getFirst().firstTokenText().or(""))
                : Option.empty();
    ...
    private static String extractExprText(CstNavigator nav) {
        ...
        return "(expr)"; // placeholder — proper implementation needs source text access
    }
    ...
    private static Option<Table.PartitionBy> extractPartitioning(CstNavigator partClause) {
        ...
        var strategyType = strategy.unwrap().has("ListKW") ? Table.PartitionStrategy.LIST
                         : strategy.unwrap().has("HashKW") ? Table.PartitionStrategy.HASH
