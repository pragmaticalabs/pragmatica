...
public final class TypeDesignRules {
    ...
    record PreferTextOverChar() implements LintRule {
        ...
        public String id() { return "PG101"; }
        public String description() { return "Prefer text over char(n)"; }
        ...
        public List<LintDiagnostic> check(SchemaEvent event, Schema schema) {
            return checkColumnType(event, type ->
                type instanceof PgType.BuiltinType bt && CHAR_TYPES.contains(bt.name()),
                id(), "char(n) pads with spaces and wastes storage", "Use text or varchar instead");
        }
    }
    ...
    record PreferTimestamptz() implements LintRule {
        ...
        public String id() { return "PG102"; }
        public String description() { return "Prefer timestamptz over timestamp"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }

        public List<LintDiagnostic> check(SchemaEvent event, Schema schema) {
            return checkColumnType(event, type ->
                type instanceof PgType.BuiltinType bt && BAD_TYPES.contains(bt.name()),
                id(), "timestamp without time zone loses timezone information",
    ...
    record AvoidMoneyType() implements LintRule {
        public String id() { return "PG103"; }
        public String description() { return "Avoid money type"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }

        public List<LintDiagnostic> check(SchemaEvent event, Schema schema) {
            return checkColumnType(event, type ->
                type instanceof PgType.BuiltinType bt && "money".equals(bt.name()),
                id(), "money type depends on lc_monetary locale setting",
    ...
    record PreferIdentityOverSerial() implements LintRule {
        static final Set<String> SERIAL_TYPES = Set.of("serial", "smallserial", "bigserial",
            "serial2", "serial4", "serial8");
        public String id() { return "PG104"; }
        public String description() { return "Prefer IDENTITY over serial"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }

        public List<LintDiagnostic> check(SchemaEvent event, Schema schema) {
            return checkColumnType(event, type ->
                type instanceof PgType.BuiltinType bt && SERIAL_TYPES.contains(bt.name()),
                id(), "serial types have permission and dependency quirks",
    ...
    record PreferJsonbOverJson() implements LintRule {
        public String id() { return "PG105"; }
        public String description() { return "Prefer jsonb over json"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }

        public List<LintDiagnostic> check(SchemaEvent event, Schema schema) {
            return checkColumnType(event, type ->
                type instanceof PgType.BuiltinType bt && "json".equals(bt.name()),
                id(), "json type lacks equality operator, breaks DISTINCT and GROUP BY",
                "Use jsonb instead");
        }
    }
    ...
    record PreferBigintForPrimaryKey() implements LintRule {
        static final Set<String> SMALL_INT_TYPES = Set.of("integer", "int", "int4", "smallint", "int2", "serial", "smallserial");
        public String id() { return "PG106"; }
        public String description() { return "Prefer bigint for primary keys"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }
                ...
                for (var col : e.columns()) {
                    if (pkColumns.contains(col.name()) && col.type() instanceof PgType.BuiltinType bt
                        ...
                        results.add(LintDiagnostic.warning(id(),
                            ...
                            e.span(), "Use bigint or bigserial for primary key columns"));
                    }
                }
                return results;
            }
    ...
    record AvoidFloatForMoney() implements LintRule {
        ...
        public String id() { return "PG107"; }
        public String description() { return "Avoid float/real for monetary or exact values"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }

        public List<LintDiagnostic> check(SchemaEvent event, Schema schema) {
            return checkColumnType(event, type ->
                type instanceof PgType.BuiltinType bt && FLOAT_TYPES.contains(bt.name()),
                id(), "floating point types lose precision — unsuitable for money or exact values",
    ...
    record AvoidTimetz() implements LintRule {
        ...
        public String id() { return "PG108"; }
        public String description() { return "Avoid timetz type"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }

        public List<LintDiagnostic> check(SchemaEvent event, Schema schema) {
            return checkColumnType(event, type ->
                type instanceof PgType.BuiltinType bt && BAD_TYPES.contains(bt.name()),
                id(), "timetz exists only for SQL compliance and is practically useless",
    ...
    record PreferTextOverVarcharWithLimit() implements LintRule {
        public String id() { return "PG109"; }
        public String description() { return "varchar(n) length limit is rarely needed"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }

        public List<LintDiagnostic> check(SchemaEvent event, Schema schema) {
            return checkColumnType(event, type ->
                type instanceof PgType.BuiltinType bt && "varchar".equals(bt.name()) && !bt.modifiers().isEmpty(),
                id(), "varchar(n) — changing length later requires ACCESS EXCLUSIVE lock, no performance benefit over text",
    ...
    record AvoidSmallintPrimaryKey() implements LintRule {
        public String id() { return "PG110"; }
        public String description() { return "smallint PK exhausts at ~32K rows"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }
                ...
                for (var col : e.columns()) {
                    if (pkColumns.contains(col.name()) && col.type() instanceof PgType.BuiltinType bt
                        ...
                        results.add(LintDiagnostic.warning(id(),
                            ...
                            e.span(), "Use bigint for primary keys"));
                    }
                }
                return results;
            }
    ...
    record PreferTimestampPrecision() implements LintRule {
        public String id() { return "PG111"; }
        public String description() { return "timestamp(0) rounds fractional seconds instead of truncating"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }

        public List<LintDiagnostic> check(SchemaEvent event, Schema schema) {
            return checkColumnType(event, type -> {
                if (type instanceof PgType.BuiltinType bt
                    && (bt.name().startsWith("timestamp") || bt.name().startsWith("time"))
                    && !bt.modifiers().isEmpty()
                    && bt.modifiers().getFirst() == 0) {
                    return true;
                }
                return false;
            }, id(), "timestamp(0)/timestamptz(0) rounds fractional seconds instead of truncating",
    ...
    record AvoidSqlAsciiEncoding() implements LintRule {
        public String id() { return "PG112"; }
        public String description() { return "Enum values cannot be removed or reordered"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }
        ...
        public List<LintDiagnostic> check(SchemaEvent event, Schema schema) {
            if (event instanceof SchemaEvent.TypeCreated e && e.type() instanceof PgType.EnumType) {
                return List.of(LintDiagnostic.warning(id(),
                    ...
                    e.span(), "Consider a text column with CHECK constraint for frequently changing value sets"));
            }
            return List.of();
        }
    }
        ...
        switch (event) {
                ...
                for (var col : e.columns()) {
                    if (typeCheck.test(col.type())) {
                        results.add(LintDiagnostic.warning(ruleId,
                            "Column '" + col.name() + "': " + message, e.span(), suggestion));
                    }
                }
            }
            case SchemaEvent.ColumnAdded e -> {
                if (typeCheck.test(e.column().type())) {
                    results.add(LintDiagnostic.warning(ruleId,
                        "Column '" + e.column().name() + "': " + message, e.span(), suggestion));
                }
            }
            case SchemaEvent.ColumnTypeChanged e -> {
                if (typeCheck.test(e.newType())) {
                    results.add(LintDiagnostic.warning(ruleId,
                        "Column '" + e.column() + "': " + message, e.span(), suggestion));
                }
            }
            default -> {}
        }
