...
public final class SchemaDesignRules {
    ...
    record TableWithoutPrimaryKey() implements LintRule {
        public String id() { return "PG201"; }
        public String description() { return "Table without primary key"; }
            ...
            if (event instanceof SchemaEvent.TableCreated e) {
                ...
                if (!hasPk) {
                    return List.of(LintDiagnostic.warning(id(),
                        ...
                        e.span(), "Add a PRIMARY KEY — needed for replication, row identification, and FK references"));
                }
            }
            return List.of();
        }
    ...
    record ForeignKeyWithoutIndex() implements LintRule {
        public String id() { return "PG202"; }
        public String description() { return "Foreign key without index on referencing columns"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }
        ...
        public List<LintDiagnostic> check(SchemaEvent event, Schema schema) {
            if (event instanceof SchemaEvent.ConstraintAdded e && e.constraint() instanceof Constraint.ForeignKey fk) {
                ...
                if (table.isPresent()) {
                    ...
                    var hasMatchingIndex = table.unwrap().indexes().stream()
                        .anyMatch(idx -> {
                    ...
                    if (!hasMatchingIndex) {
                        return List.of(LintDiagnostic.warning(id(),
                            ...
                            e.span(), "Create an index on the referencing columns"));
                    }
                }
            }
            return List.of();
    ...
    record UnnamedConstraint() implements LintRule {
        public String id() { return "PG203"; }
        public String description() { return "Unnamed constraint"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }
                ...
                for (var c : e.constraints()) {
                    if (c.name().isEmpty()) {
                        ...
                        results.add(LintDiagnostic.warning(id(),
                            ...
                            e.span(), "Name constraints explicitly with CONSTRAINT name_here"));
                    }
                }
            }
    ...
    record DuplicateIndex() implements LintRule {
        public String id() { return "PG204"; }
        public String description() { return "Potentially duplicate or overlapping index"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }
                    ...
                    for (var existing : table.unwrap().indexes()) {
                        ...
                        if (existingCols.size() >= newCols.size()
                            ...
                            return List.of(LintDiagnostic.warning(id(),
                                ...
                                e.span(), "The existing index already covers these columns"));
                        }
                    }
                }
            }
    ...
    record WideCompositeIndex() implements LintRule {
        public String id() { return "PG205"; }
        public String description() { return "Index with 4+ columns rarely improves performance"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }

        public List<LintDiagnostic> check(SchemaEvent event, Schema schema) {
            if (event instanceof SchemaEvent.IndexCreated e && e.index().elements().size() >= 4 && !e.index().unique()) {
                return List.of(LintDiagnostic.warning(id(),
                    "Index '" + e.index().name() + "' has " + e.index().elements().size() + " columns — rarely improves performance, high storage overhead",
                    e.span(), "Consider narrower index or partial index"));
            }
            return List.of();
        }
    }
    ...
    record MissingUpdatedAtColumn() implements LintRule {
        public String id() { return "PG206"; }
        public String description() { return "Table without updated_at/modified_at column"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }
            ...
            if (event instanceof SchemaEvent.TableCreated e) {
                ...
                if (!hasUpdatedAt && e.columns().size() > 2) {
                    return List.of(LintDiagnostic.warning(id(),
                        ...
                        e.span(), "Add updated_at timestamptz for change tracking"));
                }
            }
            return List.of();
        }
    ...
    record UppercaseTableOrColumnName() implements LintRule {
        public String id() { return "PG207"; }
        public String description() { return "Uppercase names require perpetual quoting"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }
            ...
            if (event instanceof SchemaEvent.TableCreated e) {
                if (!e.name().equals(e.name().toLowerCase())) {
                    results.add(LintDiagnostic.warning(id(),
                        ...
                        e.span(), "Use lowercase snake_case for table names"));
                }
                for (var col : e.columns()) {
                    if (!col.name().equals(col.name().toLowerCase())) {
                        results.add(LintDiagnostic.warning(id(),
                            ...
                            e.span(), "Use lowercase snake_case for column names"));
                    }
                }
            }
            return results;
    ...
    record ReservedWordAsName() implements LintRule {
        static final java.util.Set<String> RESERVED = java.util.Set.of(
            ...
            "update", "delete", "where", "from", "join", "left", "right", "on",
            "create", "drop", "alter", "primary", "key", "foreign", "references",
            "check", "constraint", "default", "null", "not", "and", "or", "in",
            "between", "like", "limit", "offset", "union", "except", "intersect",
            "case", "when", "then", "else", "end", "as", "is", "true", "false",
            "type", "function", "trigger", "view", "schema", "grant", "revoke"
        );

        public String id() { return "PG208"; }
        public String description() { return "Identifier is a reserved word"; }
        public LintDiagnostic.Severity defaultSeverity() { return WARNING; }
