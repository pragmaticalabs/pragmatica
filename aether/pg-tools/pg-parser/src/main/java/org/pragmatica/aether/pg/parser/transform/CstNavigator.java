...
public record CstNavigator(CstNode.NonTerminal node) {
    ...
    public List<CstNode> children() {
        return node.children().stream().filter(CstNavigator::isNonEmpty).toList();
    }
    ...
    public boolean has(String ruleName) {
        return node.children().stream().anyMatch(c -> hasRule(c, ruleName) && isNonEmpty(c));
    }
        ...
        return node.children().stream()
                   ...
                   .flatMap(c -> switch (c) {
                       ...
                       case CstNode.NonTerminal nt -> nt.children().stream()
                           .flatMap(cc -> cc instanceof CstNode.Token t ? Stream.of(t.text()) : Stream.empty());
    ...
    public Option<String> firstTokenText() {
        return node.children().stream()
                   .flatMap(c -> c instanceof CstNode.Token tok ? Stream.of(tok.text()) : Stream.empty())
                   .findFirst()
                   .map(Option::present)
                   .orElse(Option.empty());
    }
    ...
    private static CstNavigator ofNode(CstNode node) {
        return switch (node) {
            ...
            case CstNode.Token tok -> new CstNavigator(
                new CstNode.NonTerminal(tok.span(), tok.ruleName(), List.of(tok)));
            default -> throw new IllegalArgumentException("Expected NonTerminal or Token");
        };
    }
    ...
    private static <T> void collectListRecursive(CstNavigator nav, String itemRule,
                                                  Function<CstNavigator, T> mapper, List<T> result) {
