...
public final class CstExtractor {
    ...
    public static QualifiedName extractQualifiedName(CstNavigator nav) {
        ...
        return new QualifiedName(nav.span(), deduped.isEmpty() ? parts : deduped);
    }
    ...
    public static DataTypeName extractDataType(CstNavigator nav) {
        ...
        var arrayDims = arrayType.isPresent() ? countArrayDimensions(arrayType.unwrap()) : 0;
        ...
        if (scalarTokenText.isPresent() && !scalarTokenText.unwrap().isEmpty()) {
            ...
            if (scalarType.isPresent() && scalarType.unwrap().has("TypeModifiers")) {
                // Has modifiers — fall through to full extraction
            } else {
                var dt = DataTypeName.builtin(nav.span(), scalarTokenText.unwrap().trim());
                return arrayDims > 0 ? DataTypeName.array(dt, arrayDims) : dt;
            }
        }
        ...
        if (directToken.isPresent() && !directToken.unwrap().isEmpty() && !scalarType.has("TypeModifiers")) {
            var dt = DataTypeName.builtin(nav.span(), directToken.unwrap().trim());
            return arrayDims > 0 ? DataTypeName.array(dt, arrayDims) : dt;
        }
        ...
        if (baseName.isEmpty()) {
            ...
            if (qualTypeName.isPresent()) {
                ...
                return DataTypeName.array(DataTypeName.custom(nav.span(), qname), arrayDims);
            }
    ...
    private static String extractScalarTypeName(CstNavigator scalarType) {
        ...
        var typeRules = List.of(
            "NumericType", "CharType", "BooleanType", "JsonType", "UuidType",
            "ByteaType", "XmlType", "MoneyType", "SerialType", "BitType",
            "NetworkType", "TsvectorType", "DateType"
        );
    ...
    private static void collectNumericLiterals(CstNode node, List<Integer> result) {
        switch (node) {
            case CstNode.Token tok when tok.ruleName().equals("NumericLiteral") -> {
                try { result.add(Integer.parseInt(tok.text())); } catch (NumberFormatException e) { /* skip */ }
            }
