...
public record QualifiedName(SourceSpan span, List<Identifier> parts) {
    ...
    public Option<Identifier> schema() {
        return parts.size() > 1 ? Option.present(parts.getFirst()) : Option.empty();
    }

    /// Returns the normalized fully-qualified name string.
    public String normalized() {
        return String.join(".", parts.stream().map(Identifier::normalized).toList());
    }

    @Override
    public String toString() {
        return String.join(".", parts.stream().map(Identifier::toString).toList());
    }
}
