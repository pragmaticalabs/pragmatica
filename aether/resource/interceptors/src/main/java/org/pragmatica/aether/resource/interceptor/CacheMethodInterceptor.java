...
public record CacheMethodInterceptor( CacheBackend cache,
    ...
    @Override public <R, T> Fn1<Promise<R>, T> intercept(Fn1<Promise<R>, T> method) {
        return switch (strategy) {case CACHE_ASIDE, READ_THROUGH -> cacheAside(method);case WRITE_THROUGH -> writeThrough(method);case WRITE_BACK -> writeBack(method);case WRITE_AROUND -> writeAround(method);};
    }
    ...
    private <R, T> Fn1<Promise<R>, T> cacheAside(Fn1<Promise<R>, T> method) {
        return request -> {
            ...
            .or(() -> method.apply(request).onSuccess(value -> cache.put(key, value))));
        };
    }
