...
public record DatabaseConnectorConfig(Option<String> name,
    ...
    private static Option<Matcher> matchUrl(String url) {
        return option(url).map(URL_PATTERN::matcher).filter(Matcher::find);
    }

    private static Option<Integer> safeParsePort(String value) {
        return option(value).filter(v -> !v.isEmpty()).flatMap(DatabaseConnectorConfig::parseInteger);
    }
    ...
    private static Result<String> ensureJdbcUrl(String jdbcUrl) {
        return option(jdbcUrl).filter(u -> !u.isBlank()).toResult(cause("JDBC URL is required"));
    }
