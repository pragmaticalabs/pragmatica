...
public final class NotificationSenderFactory implements ResourceFactory<NotificationSender, NotificationConfig> {
    ...
    public Promise<NotificationSender> provision(NotificationConfig config) {
        return switch (config.backend()) {
            ...
            default -> new NotificationError.BackendNotConfigured("Unknown notification backend: " + config.backend())
                .<NotificationSender>promise();
        };
    }
    ...
    private static Promise<NotificationSender> provisionSmtp(NotificationConfig config) {
        return config.smtpConfig()
                     .map(smtpConfig -> {
                         ...
                         NotificationSender sender = new SmtpNotificationSender(client, config.effectiveRetryConfig());
                         return Promise.success(sender);
                     })
                     .or(new NotificationError.BackendNotConfigured("SMTP backend selected but no SMTP configuration provided")
                         .<NotificationSender>promise());
    }
    ...
    private static Promise<NotificationSender> provisionHttp(NotificationConfig config) {
        return config.httpConfig()
                     .map(httpConfig -> {
                         ...
                         NotificationSender notificationSender = new HttpNotificationSender(sender, config.effectiveRetryConfig());
                         return Promise.success(notificationSender);
                     })
                     .or(new NotificationError.BackendNotConfigured("HTTP backend selected but no HTTP configuration provided")
                         .<NotificationSender>promise());
    }
}
