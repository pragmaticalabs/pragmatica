                         ...
                         cause -> {
                             if (attempt >= retryConfig.maxAttempts()) {
                                 return new NotificationError.DeliveryFailed(
                                     "HTTP delivery failed after " + attempt + " attempts: " + cause.message())
    ...
    static EmailMessage toEmailMessage(Notification.Email email) {
        var body = switch (email.body()) {
            ...
            case NotificationBody.Html html -> html.fallback()
                .map(fallback -> EmailBody.Html.html(html.content(), fallback))
                .or(EmailBody.Html.html(html.content()));
        };

        var message = EmailMessage.emailMessage(email.from(), email.to(), email.subject(), body)
                                  .withCc(email.cc())
                                  .withBcc(email.bcc());
