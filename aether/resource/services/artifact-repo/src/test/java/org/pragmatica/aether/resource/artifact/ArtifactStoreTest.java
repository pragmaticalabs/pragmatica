...
class ArtifactStoreTest {
    ...
    class ResolveTimeoutTests {
        ...
        /// so a single block `get` whose durable tier never answers (never-resolving promise)
        /// hung the whole resolve forever — the bounded read RETRY only fires on a delivered
        /// transient FAILURE, never on a promise that simply never resolves. `deploy()` was
        /// bounded by DEPLOY_TIMEOUT; `resolve()` now has a symmetric chunk-count-scaled budget.
        ///
        /// Uses a tight resolve budget (base 200ms / per-chunk 50ms / ceiling 1s) and a storage
        /// whose `get` returns a never-resolving promise so the deadline must fire deterministically
        /// rather than the test hanging. Deploy uses the SAME storage with `get` armed only after
        /// deploy completes, so the clean write path is unaffected.
        ...
        void resolveWithMetadata_neverResolvingChunkRead_failsWithTimeoutNotHang() {
            var hangingStorage = new HangingGetStorage();
            var store = ArtifactStore.artifactStore(testDht(), hangingStorage,
                                                    FAST_BASE, FAST_PER_CHUNK, FAST_CEILING);
            var artifact = Artifact.artifact("org.example:resolve-timeout:1.0.0").unwrap();
            var content = new byte[CHUNK_SIZE + 1];  // 2 chunks
            fillWithPattern(content);
        ...
        @Test
        void resolveWithMetadata_healthyRead_succeedsWithinBudget() {
            var hangingStorage = new HangingGetStorage();
            var store = ArtifactStore.artifactStore(testDht(), hangingStorage,
                                                    FAST_BASE, FAST_PER_CHUNK, FAST_CEILING);
            var artifact = Artifact.artifact("org.example:resolve-healthy:1.0.0").unwrap();
            var content = new byte[CHUNK_SIZE + 1];  // 2 chunks
            fillWithPattern(content);
            ...
            // No armHang(): reads serve normally and must complete well within FAST budget.
            store.resolveWithMetadata(artifact)
                 .await(timeSpan(10).seconds())
                 .onFailureRun(Assertions::fail)
                 .onSuccess(resolved -> assertThat(resolved.content()).isEqualTo(content));
    ...
    /// Storage instance whose `get` returns a NEVER-resolving promise once armed, modeling a
    /// durable tier whose underlying transport silently drops the read (the Hetzner 3h hang).
    /// `put` and pre-arm `get` serve normally so deploy succeeds; `armHang()` flips reads to
    /// hang so the resolve aggregate timeout must fire.
    private static final class HangingGetStorage implements StorageInstance {
        private final ConcurrentHashMap<BlockId, byte[]> blocks = new ConcurrentHashMap<>();
        ...
        @Override
        public Promise<BlockId> put(byte[] content, BlockMetadata metadata) {
            return BlockId.blockId(content).async().onSuccess(id -> blocks.put(id, content));
        }

        @Override
        public Promise<Option<byte[]>> get(BlockId id) {
            return hang
                   ? Promise.promise()  // never resolves
                   : Promise.success(Option.option(blocks.get(id)));
        }
