...
final class ForecastAnalyzerImpl implements ForecastAnalyzer {
    ...
    @Override public TTMForecast analyze(float[] predictions,
        ...
        if (confidence <config.confidenceThreshold()) {return new TTMForecast(timestamp,
                                                                              predictions,
                                                                              confidence,
                                                                              ScalingRecommendation.NoAction.LOW_CONFIDENCE);}
        if (recentHistory.isEmpty()) {return new TTMForecast(timestamp,
                                                             predictions,
                                                             confidence,
                                                             ScalingRecommendation.NoAction.INSUFFICIENT_DATA);}
        if (predictions.length <= FeatureIndex.INVOCATIONS) {return new TTMForecast(timestamp,
                                                                                    predictions,
                                                                                    confidence,
                                                                                    ScalingRecommendation.NoAction.INSUFFICIENT_DATA);}
    ...
    private MinuteAggregate averageRecent(List<MinuteAggregate> history, int count) {
        if (history.isEmpty()) {return MinuteAggregate.EMPTY;}
