...
public record CronExpression(BitSet minutes, BitSet hours, BitSet daysOfMonth, BitSet months, BitSet daysOfWeek) {
    ...
    public static Result<CronExpression> parse(String expression) {
        ...
        if (parts.length != 5) {return INVALID_FORMAT.result();}
    ...
    public Result<Instant> nextFireTime(Instant now) {
        var dt = now.atZone(ZoneOffset.UTC).plusMinutes(1)
                           .withSecond(0)
                           .withNano(0);
    ...
    public Result<TimeSpan> delayUntilNext(Instant now) {
        return nextFireTime(now).map(next -> TimeSpan.timeSpan(java.time.Duration.between(now, next).toMillis())
                                                              .millis());
    }
    ...
    private Result<Instant> searchNextMatch(ZonedDateTime dt, ZonedDateTime limit) {
        ...
        while (current.isBefore(limit)) {
            ...
            if (!matchesDayFields(current)) {
                current = current.plusDays(1).withHour(0)
                                          .withMinute(0);
                continue;
            }
    ...
    private ZonedDateTime advanceToMatchingMonth(ZonedDateTime dt) {
        var current = dt;
        while (!months.get(current.getMonthValue())) {current = current.plusMonths(1).withDayOfMonth(1)
                                                                                  .withHour(0)
                                                                                  .withMinute(0);}
        return current;
    }
    ...
    private static Result<BitSet> parseField(String field, int min, int max, String fieldName) {
        ...
        for (var part : parts) {
            ...
            if (partResult.isFailure()) {return partResult;}
            partResult.onSuccess(bits::or);
        }
        if (bits.isEmpty()) {return Causes.cause("Empty " + fieldName + " field: " + field).result();}
    ...
    private static Result<BitSet> parsePart(String part, int min, int max, String fieldName) {
        ...
        if (slashIndex >= 0) {return parseStep(part, slashIndex, min, max, fieldName);}
        var dashIndex = part.indexOf('-');
        if (dashIndex >= 0) {return parseRange(part, dashIndex, min, max, fieldName);}
        if ("*".equals(part)) {return Result.success(rangeBitSet(min, max, max));}
    ...
    private static Result<BitSet> parseStep(String part, int slashIndex, int min, int max, String fieldName) {
        ...
        return parseValue(stepStr, 1, max, fieldName + " step").flatMap(step -> resolveStepRange(rangeStr,
                                                                                                 min,
                                                                                                 max,
                                                                                                 fieldName).map(range -> fillStepped(range[0],
                                                                                                                                     range[1],
                                                                                                                                     step)));
    }

    private static Result<int[]> resolveStepRange(String rangeStr, int min, int max, String fieldName) {
        if ("*".equals(rangeStr)) {return Result.success(new int[]{min, max});}
        var dashIndex = rangeStr.indexOf('-');
        if (dashIndex >= 0) {return parseValue(rangeStr.substring(0, dashIndex), min, max, fieldName).flatMap(start -> parseValue(rangeStr.substring(dashIndex + 1),
                                                                                                                                  min,
                                                                                                                                  max,
                                                                                                                                  fieldName).flatMap(end -> validateRange(start,
                                                                                                                                                                          end,
                                                                                                                                                                          fieldName).map(_ -> new int[]{start, end})));}
    ...
    private static Result<BitSet> parseRange(String part, int dashIndex, int min, int max, String fieldName) {
        return parseValue(part.substring(0, dashIndex), min, max, fieldName).flatMap(start -> parseValue(part.substring(dashIndex + 1),
                                                                                                         min,
                                                                                                         max,
                                                                                                         fieldName).flatMap(end -> validateRange(start,
                                                                                                                                                 end,
                                                                                                                                                 fieldName).map(_ -> rangeBitSet(start,
                                                                                                                                                                                 end,
                                                                                                                                                                                 max))));
    }

    private static Result<Unit> validateRange(int start, int end, String fieldName) {
        if (start > end) {return Causes.cause("Invalid " + fieldName + " range: " + start + "-" + end).result();}
    ...
    private static BitSet fillStepped(int start, int end, int step) {
        ...
        for (int i = start;i <= end;i += step) {bits.set(i);}
        return bits;
    }
    ...
    private static Result<Integer> parseValue(String str, int min, int max, String fieldName) {
        try {
            ...
            if (value <min || value > max) {return Causes.cause("Invalid " + fieldName + " value " + value + ": must be " + min + "-" + max)
                                                               .result();}
