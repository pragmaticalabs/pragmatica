...
public record AppHttpConfig(boolean enabled,
    ...
    public static Result<AppHttpConfig> appHttpConfig(boolean enabled,
                                                      ...
                                                      Map<String, ApiKeyEntry> apiKeys,
                                                      long forwardTimeoutMs) {
        return success(new AppHttpConfig(enabled, port, apiKeys, forwardTimeoutMs));
    }

    /// Default (disabled) configuration.
    public static AppHttpConfig appHttpConfig() {
        return appHttpConfig(false, DEFAULT_APP_HTTP_PORT, Map.of(), DEFAULT_FORWARD_TIMEOUT_MS).unwrap();
    }

    /// Enabled on default port.
    public static AppHttpConfig appHttpConfig(boolean enabled) {
        return appHttpConfig(enabled, DEFAULT_APP_HTTP_PORT, Map.of(), DEFAULT_FORWARD_TIMEOUT_MS).unwrap();
    }

    /// Enabled on specified port.
    public static AppHttpConfig appHttpConfig(int port) {
        return appHttpConfig(true, port, Map.of(), DEFAULT_FORWARD_TIMEOUT_MS).unwrap();
    }
    ...
    public static AppHttpConfig appHttpConfig(int port, Set<String> apiKeys) {
        return appHttpConfig(true, port, wrapSimpleKeys(apiKeys), DEFAULT_FORWARD_TIMEOUT_MS).unwrap();
    }
    ...
    public AppHttpConfig withEnabled(boolean enabled) {
        return appHttpConfig(enabled, port, apiKeys, forwardTimeoutMs).unwrap();
    }
    ...
    public AppHttpConfig withApiKeys(Set<String> apiKeys) {
        return appHttpConfig(enabled, port, wrapSimpleKeys(apiKeys), forwardTimeoutMs).unwrap();
    }
