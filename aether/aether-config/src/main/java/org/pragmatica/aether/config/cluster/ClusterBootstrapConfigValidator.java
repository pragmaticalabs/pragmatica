...
public final class ClusterBootstrapConfigValidator {
    ...
    private static final Set<RuntimeType> SSH_RUNTIME_TYPES = EnumSet.of(RuntimeType.CONTAINER, RuntimeType.JVM, RuntimeType.EMBER);

    private ClusterBootstrapConfigValidator() {}

    /// Validate and return the config if valid, or all accumulated errors.
    ...
    /// Validate and return warnings (non-fatal).
    ...
    private static void validateClusterLevel(ClusterBootstrapConfig config, List<String> errors) {
        validateClusterName(config.cluster().name(), errors);
        validateClusterVersion(config.cluster().version(), errors);
    ...
    private static void validateClusterName(String name, List<String> errors) {
        if (!CLUSTER_NAME_PATTERN.matcher(name).matches()) {
            errors.add("CL-01: Cluster name '" + name + "' must match ^[a-z][a-z0-9-]{0,62}$");
        }
    }
    ...
    private static void validateAtLeastOneCoreSubTable(ClusterBootstrapConfig config, List<String> errors) {
        var hasCoreSubTable = config.sources().values().stream()
    ...
    private static void validateRuntimeReferences(ClusterBootstrapConfig config, List<String> errors) {
        ...
        config.sources().forEach((sourceName, source) ->
            source.roles().forEach((role, sub) ->
                checkRuntimeRef(sourceName, role, sub.runtimeRef(), source.type(), runtimes, errors)));
    }

    private static void checkRuntimeRef(String sourceName, NodeRole role, String runtimeRef,
                                         SourceType sourceType, Map<String, RuntimeProfile> runtimes,
                                         List<String> errors) {
        if (isImplicitRuntime(runtimeRef, sourceType)) {
            return;
        }
        if (!runtimes.containsKey(runtimeRef)) {
            errors.add("CL-06: Source '" + sourceName + "' role '" + role.value()
    ...
    private static void validateMaxUnavailable(int maxUnavailable, int derivedCount, List<String> errors) {
        ...
        if (maxUnavailable > limit) {
            errors.add("REQ-3.3.7: maxUnavailable " + maxUnavailable + " must be <= (derivedCoreCount - 1) / 2 = " + limit);
        }
    }
    ...
    private static void validateSource(String name, SourceProfile source,
                                        Map<String, RuntimeProfile> runtimes, List<String> errors) {
    ...
    private static void validateSingleRoleConstraint(String name, SourceType type, NodeRole role,
                                                      RoleSubTable sub, List<String> errors) {
        switch (type) {
    ...
    private static void validateSshRole(String sourceName, NodeRole role, RoleSubTable sub, List<String> errors) {
        if (sub.hosts().equals(Option.none()) && sub.count().equals(Option.none())) {
            ...
            errors.add("PF-10: SSH source '" + sourceName + "' role '" + role.value()
                            + "' must use hosts (not count)");
        }
    }

    private static void validateCloudOrForgeRole(String sourceName, NodeRole role, RoleSubTable sub,
                                                  List<String> errors) {
        if (sub.count().equals(Option.none()) && sub.hosts().equals(Option.none())) {
            ...
            errors.add("PF-11: Source '" + sourceName + "' role '" + role.value()
                            + "' must use count (not hosts)");
        }
    }
    ...
    private static void validateElectedLbHasNonSpot(String name, SourceProfile source, List<String> errors) {
        ...
        var hasNonSpot = source.roles().keySet().stream()
    ...
    private static void validateRuntimeTypeCompatibility(String name, SourceProfile source,
                                                          Map<String, RuntimeProfile> runtimes,
                                                          List<String> errors) {
        source.roles().forEach((role, sub) ->
            checkRoleRuntimeType(name, role, sub.runtimeRef(), source.type(), runtimes, errors));
    }

    private static void checkRoleRuntimeType(String sourceName, NodeRole role, String runtimeRef,
                                              SourceType sourceType, Map<String, RuntimeProfile> runtimes,
                                              List<String> errors) {
        var resolved = resolveRuntimeType(runtimeRef, sourceType, runtimes);

        resolved.onPresent(runtimeType -> validateRuntimeForSource(sourceName, role, runtimeRef,
                                                                    sourceType, runtimeType, errors));
    }

    private static Option<RuntimeType> resolveRuntimeType(String runtimeRef, SourceType sourceType,
                                                           Map<String, RuntimeProfile> runtimes) {
        var profile = runtimes.get(runtimeRef);

        if (profile != null) {
            return Option.some(profile.type());
        }
    ...
    private static void validateRuntimeForSource(String sourceName, NodeRole role, String runtimeRef,
                                                  SourceType sourceType, RuntimeType runtimeType,
                                                  List<String> errors) {
        switch (sourceType) {
            case FORGE -> {
                if (runtimeType != RuntimeType.EMBER) {
                    errors.add("PF-19: Forge source '" + sourceName + "' role '" + role.value()
                                    + "' runtime '" + runtimeRef + "' must be EMBER type, got " + runtimeType.value());
                }
            }
            case DOCKER -> {
                if (runtimeType != RuntimeType.DOCKER) {
                    errors.add("PF-20: Docker source '" + sourceName + "' role '" + role.value()
                                    + "' runtime '" + runtimeRef + "' must be DOCKER type, got " + runtimeType.value());
                }
            }
            case CLOUD -> {
                if (!CLOUD_RUNTIME_TYPES.contains(runtimeType)) {
                    errors.add("PF-21: Cloud source '" + sourceName + "' role '" + role.value()
                                    + "' runtime '" + runtimeRef + "' must be CONTAINER or JVM, got " + runtimeType.value());
                }
            }
            case SSH -> {
                if (!SSH_RUNTIME_TYPES.contains(runtimeType)) {
                    errors.add("PF-22: SSH source '" + sourceName + "' role '" + role.value()
                                    + "' runtime '" + runtimeRef + "' must be CONTAINER, JVM, or EMBER, got "
                                    + runtimeType.value());
                }
            }
        }
    ...
    private static void validatePortConflictsOnSameHost(String name, SourceProfile source, List<String> errors) {
        ...
        source.roles().values().forEach(sub ->
            sub.hosts().onPresent(hosts -> collectDuplicateHosts(hosts, hostsSeen, duplicates)));

        duplicates.forEach(host ->
            errors.add("PF-09: SSH source '" + name + "' has host '" + host
    ...
    private static void addPort(int port, String name, Set<Integer> seen, List<String> names,
                                 List<String> errors) {
    ...
    private static void checkSourceCoreMajority(String name, SourceProfile source, int majority,
                                                   int totalCores, List<String> warnings) {
        ...
        if (count > majority) {
            warnings.add("CL-13: Source '" + name + "' holds " + count + " of " + totalCores
                             + " cores (majority risk)");
        }
    }
    ...
    private static void checkCapacityMajorityWarning(ClusterBootstrapConfig config, List<String> warnings) {
        ...
        perSource.stream()
                 ...
                 .forEach(entry -> warnings.add("CL-14: Source '" + entry.getKey() + "' holds " + entry.getValue()
                                                    + " of " + finalTotal + " non-spot nodes (capacity concentration risk)"));
    }

    private static int countNonSpotCapacity(SourceProfile source) {
        return source.roles().entrySet().stream()
                     .filter(e -> e.getKey() != NodeRole.SPOT)
                     .mapToInt(e -> e.getValue().count().or(0) + e.getValue().hosts().map(List::size).or(0))
                     .sum();
    }
}
