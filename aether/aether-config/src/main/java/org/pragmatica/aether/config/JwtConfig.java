...
public record JwtConfig(String jwksUrl,
    ...
    public static Result<JwtConfig> jwtConfig(String jwksUrl,
        ...
        return success(new JwtConfig(jwksUrl,
                                     issuer,
                                     audience,
                                     roleClaim.isBlank() ? DEFAULT_ROLE_CLAIM : roleClaim,
                                     cacheTtlSeconds <= 0 ? DEFAULT_CACHE_TTL_SECONDS : cacheTtlSeconds));
    }

    /// Convenience factory with defaults.
    public static Result<JwtConfig> jwtConfig(String jwksUrl) {
        return jwtConfig(jwksUrl, Option.empty(), Option.empty(), DEFAULT_ROLE_CLAIM, DEFAULT_CACHE_TTL_SECONDS);
    }
    ...
    public static Result<JwtConfig> jwtConfig(String jwksUrl, String issuer, String audience) {
        return jwtConfig(jwksUrl, option(issuer), option(audience), DEFAULT_ROLE_CLAIM, DEFAULT_CACHE_TTL_SECONDS);
    }
}
