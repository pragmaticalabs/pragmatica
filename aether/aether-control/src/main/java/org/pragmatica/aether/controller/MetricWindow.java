 ...
 *
 * <p>Uses circular buffer approach for O(1) operations.
 * Creates new instance on each {@link #record(double)} call to maintain immutability.
 *
 * <p>The relative change model compares current value against rolling average:
 * <ul>
 *   <li>{@code relativeLoad = currentValue / rollingAverage}</li>
 *   <li>{@code relativeLoad > 1.5} suggests scale up</li>
 *   <li>{@code relativeLoad < 0.5} suggests scale down</li>
 * </ul>
 *
 * <p>Implementation note: The array is cloned on each {@link #record(double)} call to
 ...
 *
 * <p><strong>WARNING:</strong> Do not use {@code valuesInternal()} directly - it exposes
...
public record MetricWindow(double[] valuesInternal, int head, int count, double sum, long recordCount) {
     ...
     *
     * <p>This is the core of the relative change detection model:
     * <ul>
     *   <li>Result > 1.0 means current is above average</li>
     *   <li>Result < 1.0 means current is below average</li>
     *   <li>Result = 1.0 means current equals average</li>
     * </ul>
     *
     ...
     *
     * <p>If window is full, the oldest value is evicted (circular buffer).
     *
     * <p>Periodically recalculates the sum from scratch to prevent floating-point
