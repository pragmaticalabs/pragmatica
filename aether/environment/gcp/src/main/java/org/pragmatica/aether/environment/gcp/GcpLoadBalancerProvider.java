...
public record GcpLoadBalancerProvider(GcpClient client,
    ...
    public Promise<Unit> onRouteChanged(RouteChange routeChange) {
        ...
        var attachOps = routeChange.nodeIps()
                                   .stream()
    ...
    public Promise<LoadBalancerInfo> loadBalancerInfo() {
        return client.listNetworkEndpoints(negName)
    ...
    public Promise<Unit> reconcile(LoadBalancerState state) {
        ...
        return client.listNetworkEndpoints(negName)
                     .map(GcpLoadBalancerProvider::currentIpsFromEndpoints)
    ...
    private Promise<Unit> reconcileDiff(Set<String> currentIps, Set<String> desiredIps) {
        ...
        var attachOps = ipsToAttach.stream()
                                   .map(this::attachEndpoint);
        var detachOps = ipsToDetach.stream()
                                   .map(this::detachEndpoint);
        var all = Stream.concat(attachOps, detachOps)
                        .toList();
    ...
    private static Promise<Unit> combineAll(Collection<Promise<Unit>> promises) {
        return Promise.allOf(promises)
                      .map(GcpLoadBalancerProvider::collectResults)
