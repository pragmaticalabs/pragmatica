...
public record HetznerLoadBalancerProvider(HetznerClient client,
    ...
    public Promise<Unit> onRouteChanged(RouteChange routeChange) {
        log.debug("Adding {} IP targets for route {} {}", routeChange.nodeIps().size(),
                  routeChange.httpMethod(), routeChange.pathPrefix());

        return combineAll(routeChange.nodeIps().stream()
                                     .map(this::addTarget)
                                     .toList());
    }
    ...
    public Promise<Unit> reconcile(LoadBalancerState state) {
        log.debug("Reconciling load balancer {} with {} active nodes", loadBalancerId, state.activeNodeIps().size());

        return client.getLoadBalancer(loadBalancerId)
                     .flatMap(lb -> applyDiff(extractCurrentIps(lb.targets()), state.activeNodeIps()));
    }
    ...
    private static Promise<Unit> combineAll(java.util.Collection<Promise<Unit>> promises) {
        ...
        return Promise.allOf(promises)
                      .map(results -> Result.allOf(results).map(Unit::toUnit))
