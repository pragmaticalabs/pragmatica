...
class DockerComputeProviderTest {
    ...
    class ProvisionTests {
        ...
        void provisionFailure_dockerRunFails_rollbackInvokedByContainerName() {
            ...
            // The 4th arg is the container name pre-generated by provision().
            // provision(InstanceType) uses ProvisionContext.provisionContext("default", "core", "default", ...)
            // and ProvisionSpec.provisionSpec(_, _, "default", ...) -> pool=default, cluster=default.
            assertThat(rollbackCmd.get(3)).startsWith("aether-default-default-node-0-");
        }

        @Test
        void provisionFailure_dockerRunAndRollbackBothFail_rollbackErrorLoggedNotPropagated() {
            // Both run and rollback fail — original error must still surface, no exception.
            testRunner.queuedResponses.add(new DockerError.CommandExecutionFailed(
        ...
        void provisionSuccess_noRollbackInvoked() {
            ...
            // Only the docker run; no rm -f follow-up.
    ...
    class ContainerNameTests {
        ...
        @Test
        void buildContainerName_includesClusterPoolIndexAndSuffix() {
            ...
            assertThat(name).matches("^aether-test-cluster-worker-pool-node-0-[0-9a-f]+$");
        }

        @Test
        void buildContainerName_defaultsClusterAndPoolWhenAbsent() {
            testRunner.nextResponse = Promise.success("id-0");
            ...
            var nameIdx = testRunner.lastCommand.indexOf("--name");
            var name = testRunner.lastCommand.get(nameIdx + 1);
            // provision(InstanceType) hardcodes pool="default" + cluster="default".
            assertThat(name).startsWith("aether-default-default-node-0-");
        }
    }
