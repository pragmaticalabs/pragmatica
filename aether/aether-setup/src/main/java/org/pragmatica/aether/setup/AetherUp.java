...
public final class AetherUp {
    ...
    @SuppressWarnings("JBCT-RET-01") public static void main(String[] args) {
    ...
    private static Generator findGenerator(AetherConfig config) {
        return GENERATORS.stream().filter(g -> g.supports(config))
                                .findFirst()
    ...
    private static void printDryRun(AetherConfig config, Path outputDir) {
        ...
        System.out.println("  Environment: " + config.environment().displayName());
        System.out.println("  Nodes: " + config.cluster().nodes());
        System.out.println("  Heap: " + config.node().heap());
        System.out.println("  TLS: " + config.tlsEnabled());
        System.out.println("  Output: " + outputDir);
    }

    @SuppressWarnings("JBCT-SEQ-01") private static void runGeneration(Generator generator,
                                                                       AetherConfig config,
                                                                       Path outputDir) {
    ...
    private static Map<String, String> parseArgs(String[] args) {
        ...
        Arrays.stream(args).reduce("", (prev, arg) -> parseArg(prev, arg, options));
        return options;
    }

    @SuppressWarnings("JBCT-SEQ-01") private static String parseArg(String prev,
                                                                    String arg,
        ...
        return switch (arg){
            ...
            default -> {
                if (Verify.Is.notEmpty(prev)) {options.put(prev, arg);}
                yield "";
            }
        };
    }
    ...
    private static Map<String, String> extractOverrides(Map<String, String> options) {
        ...
        if (options.containsKey("env")) {overrides.put("environment", options.get("env"));}
        if (options.containsKey("nodes")) {overrides.put("nodes", options.get("nodes"));}
        if (options.containsKey("heap")) {overrides.put("heap", options.get("heap"));}
        if (options.containsKey("tls")) {overrides.put("tls", "true");}
        if (options.containsKey("no-tls")) {overrides.put("tls", "false");}
        return overrides;
    }
