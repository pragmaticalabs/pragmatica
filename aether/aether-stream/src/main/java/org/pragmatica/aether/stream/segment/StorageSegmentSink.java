...
public final class StorageSegmentSink implements SegmentSink {
    ...
    @Override public Promise<Unit> seal(SealedSegment segment) {
        ...
        return storage.put(processedData.data()).flatMap(blockId -> storage.createRef(refName(segment),
                                                                                      blockId))
    ...
    private ProcessedData applyEncryption(byte[] data) {
        return encryptor.map(enc -> encryptData(enc, data)).or(ProcessedData.unencrypted(data));
    }

    private static ProcessedData encryptData(BlockEncryptor enc, byte[] data) {
        return enc.encrypt(data).map(encrypted -> new ProcessedData(prependIv(encrypted),
                                                                    true))
