...
public final class PgTransactionalCursorCommit implements TransactionalCursorCommit {
    ...
    @Override public <T> Promise<T> commitWithLogic(String groupId,
                                                    String streamName,
                                                    int partition,
                                                    long offset,
    ...
    private static <T> Promise<T> executeInTransaction(SqlConnector tx,
                                                       ...
                                                       long offset,
                                                       SqlConnector.TransactionCallback<T> logic) {
        return logic.execute(tx).flatMap(result -> commitCursor(tx, groupId, streamName, partition, offset, result));
    }
    ...
    private static <T> Promise<T> commitCursor(SqlConnector tx,
                                               ...
                                               long offset,
                                               T result) {
        return tx.update(UPSERT_CURSOR, groupId, streamName, partition, offset).map(_ -> result);
    }
