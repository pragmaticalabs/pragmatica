...
public interface CatchupTransport {
    ...
    CatchupTransport NOOP = (_, request) -> Promise.success(
        catchupResponse(request.replicaId(), request.streamName(), request.partition(),
                        request.fromOffset(), request.fromOffset(), List.of(), List.of()));
}
