...
final class PgStore implements PgStreamStore {
    ...
    private static final String INSERT_SEGMENT =
        "INSERT INTO aether_stream_segments (stream_name, partition_id, start_offset, end_offset, data) VALUES (?, ?, ?, ?, ?)";

    private static final String SELECT_SEGMENT =
        "SELECT data FROM aether_stream_segments WHERE stream_name = ? AND partition_id = ? AND start_offset = ?";

    private static final String DELETE_EXPIRED =
        "DELETE FROM aether_stream_segments WHERE stream_name = ? AND created_at < ?";

    private static final String UPSERT_CURSOR =
        "INSERT INTO aether_stream_cursors (consumer_group, stream_name, partition_id, committed_offset, updated_at) "
        + "VALUES (?, ?, ?, ?, NOW()) "
    ...
    private static final String SELECT_CURSOR =
        "SELECT committed_offset FROM aether_stream_cursors WHERE consumer_group = ? AND stream_name = ? AND partition_id = ?";
    ...
    private static void logCursorCommitted(String consumerGroup, String streamName, int partition, long offset) {
        log.debug("PG cursor committed: {}/{}/{} -> {}", consumerGroup, streamName, partition, offset);
    }
}
