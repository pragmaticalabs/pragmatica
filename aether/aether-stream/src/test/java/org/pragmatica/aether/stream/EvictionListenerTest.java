...
class EvictionListenerTest {
    ...
    class CapacityEviction {
        ...
        void listener_receivesEventsBeforeEviction() {
            ...
            EvictionListener listener = (_, _, events) -> accepted(captured.add(new ArrayList<>(events)));
        ...
        void listener_receivesCorrectOffsetRange() {
            ...
            EvictionListener listener = (_, _, events) -> accepted(capturedOffsets.addAll(events.stream().map(OffHeapRingBuffer.RawEvent::offset).toList()));
    ...
    class MultipleBatches {
        ...
        void listener_multipleEvictionBatches_calledMultipleTimes() {
            ...
            EvictionListener listener = (_, _, events) -> accepted(batchSizes.add(events.size()));
    ...
    /// A listener that records synchronously and takes the events — the ring reclaims them in the same pass
    /// (#1234).
