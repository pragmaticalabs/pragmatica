...
public interface ClusterSyncScheduler extends PeerObservationBuffer {
    ...
    @Contract
    void recordObservedEpoch(NodeId nodeId, Epoch epoch);
    ...
    @Contract
    default void setDrainTargets(Supplier<Set<NodeId>> supplier) {}
    ...
    @Contract
    void emitPeriodicConnectivityNow();
...
final class ClusterSyncSchedulerAdapter implements ClusterSyncScheduler {
    ...
    public boolean isActive() {
        return context.fsm()
                      .current() instanceof ClusterSyncState.Pinging;
    }
    ...
    public void onMembershipDecision(MembershipDecision decision) {
        switch (decision) {
            ...
            case NodeDecommissioned(NodeId removed, List<NodeId> newTopology, _, _) -> handleNodeRemoved(removed,
                                                                                                         newTopology);
            ...
            case NodeJoining(NodeId joining, List<NodeId> newTopology, _, _) -> context.setTopology(augmentWith(newTopology,
                                                                                                                joining));
            ...
            case MembershipDecision.NodeDraining _, MembershipDecision.NodeFailedDrain _, MembershipDecision.NodeShuttingDown _ -> {}
        }
    }

    private static List<NodeId> augmentWith(List<NodeId> topology, NodeId extra) {
        if (topology.contains(extra)) {
            return topology;
        }
    ...
    public void onQuorumStateChange(ClusterStateNotification notification) {
        if (!notification.advanceSequence(context.quorumSequence())) {
            return;
        }
