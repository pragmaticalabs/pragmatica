...
class SystemStreamWriteGateTest {
    ...
    class Rejected {
        ...
        @Test
        void flatForm_publish_toSystemStreamName_isGated() {
            var path = ManagementRoute.STREAM_PUBLISH.assemble("cluster-events").unwrap();

            assertThat(ManagementServerImpl.isSystemStreamWriteOverHttp("POST", path)).isTrue();
        }

        @Test
        void flatForm_delete_systemStreamName_isGated() {
            var path = ManagementRoute.STREAM_DELETE.assemble("cluster-events").unwrap();
        ...
        /// Same evasion vector as above, against the flat single-segment `name` param.
        @Test
        void flatForm_percentEncodedSystemStreamName_isGated() {
            assertThat(ManagementServerImpl.isSystemStreamWriteOverHttp(
                    "POST", "/api/v1/streams/publish/%63luster-events")).isTrue();
        }
    ...
    class Allowed {
        ...
        @Test
        void flatForm_publish_appStreamName_isNotGated() {
            var path = ManagementRoute.STREAM_PUBLISH.assemble("orders").unwrap();
