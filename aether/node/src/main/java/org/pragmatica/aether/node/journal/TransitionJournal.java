...
public final class TransitionJournal {
    ...
    private void autoFlushIfWarnClass(Entry entry) {
        ...
        if (entry.wallClockMs() - last < AUTO_FLUSH_MIN_INTERVAL_MS || !lastAutoFlushMs.compareAndSet(last,
                                                                                                      entry.wallClockMs())) {
            return;
        }
    ...
    private static boolean isWarnClass(Entry entry) {
        return switch (entry.layer()) {
            ...
            case PEER -> ("EVICTED".equals(entry.to()) && "CONNECTED".equals(entry.from())) || "REMOVED".equals(entry.to());
        };
    }
}
