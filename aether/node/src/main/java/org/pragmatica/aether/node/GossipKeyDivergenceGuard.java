...
public final class GossipKeyDivergenceGuard implements GossipEncryptor {
    ...
    /// magnitude. 10 minutes is intended to clear, by a similar margin, the time a probed restarted
    /// member takes to accumulate [#UNKNOWN_KEY_THRESHOLD] datagrams, while still ending the interval
    /// in which the process can be killed.
    ///
    /// Both margins were chosen deliberately wide. **Whether the behaviour is SENSITIVE to either
    /// number is unknown until the distributions are measured** (#1208) — this comment does not claim
    /// the numbers could be moved safely, only that they were not tuned. Do not read it as a reason
