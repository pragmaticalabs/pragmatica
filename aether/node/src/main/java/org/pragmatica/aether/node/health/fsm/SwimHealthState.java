    ...
    record Running(SwimHealthContext ctx,
        ...
        public void handle(SwimHealthEvents event, TransitionRequest<SwimHealthState, SwimHealthEvents> tx) {
            switch (event) {
                ...
                case PeerSuspect ps -> tx.handle(() -> ctx.reportHint(ps.member().nodeId(),
                                                                      HealthHint.SUSPECTED));
                case PeerFaulty pf -> handlePeerFaulty(pf.member(), pf.firstHand(), tx);
        ...
        private void handlePeerFaulty(SwimMember member,
            ...
            tx.handle(() -> routeFaultyPeer(member, firstHand));
        }

        /// Route a SWIM FAULTY verdict into the membership death path (leader hint + observation
        /// buffer; the per-observation `DepartedObserved` is the terminal-death driver). P1
        /// death-path co-confirmation lives UPSTREAM in `SwimProtocol`: an uncorroborated
        /// SECOND-HAND (gossip) FAULTY is downgraded to SUSPECT there and never raises a
        /// `PeerFaulty`. Hence every verdict arriving here is FIRST-HAND (own probe timeout) or a
        /// LOCALLY-CORROBORATED second-hand verdict (transport-observed peer-down) — both genuine
        /// local death evidence, so the death path proceeds. `firstHand` is logged for forensics.
        private void routeFaultyPeer(SwimMember member, boolean firstHand) {
            LOG.warn("SWIM member faulty: {} (currentLeader={}, firstHand={})",
                     member.nodeId(),
                     currentLeader,
                     firstHand);
