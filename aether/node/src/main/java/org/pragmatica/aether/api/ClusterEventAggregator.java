...
public final class ClusterEventAggregator {
    ...
    public void onNodeRemoved(TopologyChangeNotification.NodeRemoved event) {
        buffer.add(ClusterEvent.clusterEvent(EventType.NODE_LEFT,
                                             Severity.INFO,
                                             "Node " + event.nodeId().id() + " left cluster (now " + event.topology().size() + " nodes)",
                                             Map.of("nodeId", event.nodeId().id(),
                                                    "clusterSize", String.valueOf(event.topology().size()))));
    }
    ...
    public void onQuorumStateChange(QuorumStateNotification event) {
        switch (event) {
            ...
            case QuorumStateNotification.DISAPPEARED ->
                buffer.add(ClusterEvent.clusterEvent(EventType.QUORUM_LOST,
                                                     Severity.CRITICAL,
                                                     "Quorum lost",
                                                     Map.of()));
        }
    ...
    public void onDeploymentStarted(DeploymentEvent.DeploymentStarted event) {
        buffer.add(ClusterEvent.clusterEvent(EventType.DEPLOYMENT_STARTED,
                                             Severity.INFO,
                                             "Deploying " + event.artifact().asString() + " to " + event.targetNode().id(),
                                             Map.of("artifact", event.artifact().asString(),
                                                    "nodeId", event.targetNode().id())));
    }
    ...
    public void onDeploymentCompleted(DeploymentEvent.DeploymentCompleted event) {
        buffer.add(ClusterEvent.clusterEvent(EventType.DEPLOYMENT_COMPLETED,
                                             Severity.INFO,
                                             "Deployed " + event.artifact().asString() + " on " + event.nodeId().id(),
                                             Map.of("artifact", event.artifact().asString(),
                                                    "nodeId", event.nodeId().id())));
    }
    ...
    public void onDeploymentFailed(DeploymentEvent.DeploymentFailed event) {
        buffer.add(ClusterEvent.clusterEvent(EventType.DEPLOYMENT_FAILED,
                                             Severity.WARNING,
                                             "Deployment of " + event.artifact().asString() + " failed on " + event.nodeId().id() + " at " + event.failedAt().name(),
                                             Map.of("artifact", event.artifact().asString(),
                                                    "nodeId", event.nodeId().id(),
                                                    "failedAt", event.failedAt().name())));
    }
