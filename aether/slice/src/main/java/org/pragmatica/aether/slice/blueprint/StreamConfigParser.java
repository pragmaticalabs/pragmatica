...
public interface StreamConfigParser {
    ...
    private static RetentionPolicy parseRetention(TomlDocument doc, String section) {
        ...
        return switch (retentionType.toLowerCase()) {case "time" -> RetentionPolicy.retentionPolicy(Long.MAX_VALUE,
                                                                                                    Long.MAX_VALUE,
                                                                                                    parseTimeMs(retentionValue));case "size" -> RetentionPolicy.retentionPolicy(Long.MAX_VALUE,
                                                                                                                                                                                parseSizeBytes(retentionValue),
                                                                                                                                                                                Long.MAX_VALUE);case "count" -> RetentionPolicy.retentionPolicy(parseCount(retentionValue),
                                                                                                                                                                                                                                                Long.MAX_VALUE,
                                                                                                                                                                                                                                                Long.MAX_VALUE);default -> RetentionPolicy.retentionPolicy();};
    }
    ...
    private static ProcessingMode parseProcessingMode(String value) {
        return switch (value.toLowerCase()) {case "parallel" -> ProcessingMode.PARALLEL;default -> ProcessingMode.ORDERED;};
    }

    private static ConsistencyMode parseConsistencyMode(String value) {
        return switch (value.toLowerCase()) {case "strong" -> ConsistencyMode.STRONG;default -> ConsistencyMode.EVENTUAL;};
    }

    private static ReadPreference parseReadPreference(String value) {
        return switch (value.toLowerCase()) {case "nearest", "any" -> ReadPreference.NEAREST;case "follower-only", "follower_only", "follower" -> ReadPreference.FOLLOWER_ONLY;default -> ReadPreference.LEADER;};
    }

    private static ErrorStrategy parseErrorStrategy(String value) {
        return switch (value.toLowerCase()) {case "skip" -> ErrorStrategy.SKIP;case "stall" -> ErrorStrategy.STALL;default -> ErrorStrategy.RETRY;};
    }
