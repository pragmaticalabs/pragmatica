...
@SuppressWarnings({"JBCT-PAT-01", "JBCT-EX-01"}) public final class TopologyParser {
    ...
    @SuppressWarnings("JBCT-EX-01") private static List<SliceTopology> readTopologiesFromJar(URL jarUrl,
    ...
    private static List<SliceTopology.Route> parseRoutes(Properties props) {
        ...
        for (int i = 0;i <count;i++) {
            ...
            if (method != null && path != null) {routes.add(new SliceTopology.Route(method,
                                                                                    path,
                                                                                    handler != null
                                                                                    ? handler
                                                                                    : ""));}
    ...
    private static List<SliceTopology.ResourceDep> parseResources(Properties props) {
        ...
        for (int i = 0;i <count;i++) {
            ...
            if (type != null && config != null) {resources.add(new SliceTopology.ResourceDep(type, config));}
        }
        return resources;
    }
