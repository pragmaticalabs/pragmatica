...
public interface SliceStore {
    ...
    static SliceStore sliceStore(SliceActionConfig config) {
        record loadedSlice(Artifact artifact, Result<Slice> slice) implements LoadedSlice {}

        record store(Map<Artifact, Promise<Slice>> instances,
                     Map<Artifact, SliceClassLoader> classLoaders,
                     Map<Artifact, LoadedSlice> loadedInstances,
                     SliceActionConfig config) implements
                SliceStore {
            ...
            @Override
            public Promise<Slice> load(Artifact artifact, Repository repository) {
            ...
            public Promise<Unit> unload(Artifact artifact) {
                ...
                instances.computeIfPresent(artifact, (key, promise) -> {
                    ...
                    promise.flatMap(LoadedSlice::stop)
            ...
            public Promise<LoadedSlice> activateSlice(Artifact artifact) {
                ...
                return Option.option(loadedInstances.get(artifact))
                        .async(() -> Causes.cause("Slice not loaded: " + artifact).promise())
                        .flatMap(loadeSlice -> {
                            loadedSlice.slice()
                                       .map(activeSlice -> {
...
//                }
            ...
            private Promise<Slice> loadSlice(Artifact artifact, Repository repository) {
                return repository.locate(artifact)
                                 .withSuccess(location -> log.debug("Repository provided: {}", location))
                                 .withFailure(cause -> log.warn("Unable to load {}, cause: {}", artifact, cause))
            ...
            private Promise<Slice> loadService(Location location) {
            ...
            private Promise<Slice> loadService(ServiceLoader<Slice> serviceLoader, Location location) {
                var it = serviceLoader.iterator();

                return it.hasNext()
                        ? Promise.success(it.next())
            ...
            private Repository findRepository(Artifact artifact) {
                //TODO: extend with actual search
