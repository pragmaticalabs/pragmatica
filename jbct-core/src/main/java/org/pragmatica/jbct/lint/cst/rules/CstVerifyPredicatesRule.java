...
public class CstVerifyPredicatesRule implements CstLintRule {
    ...
    private static final List<ValidationPattern> PATTERNS = List.of(
        ...
        new ValidationPattern(
            "\\b\\w+\\s*>\\s*0\\b",
            "Verify.Is::positive",
            "Use Verify.ensure(value, Verify.Is::positive)"),
        new ValidationPattern(
            "\\b\\w+\\s*<\\s*0\\b",
            "Verify.Is::negative",
        ...
        new ValidationPattern(
            "\\.isEmpty\\s*\\(\\s*\\)",
            "Verify.Is::empty",
            "Use Verify.ensure(value, Verify.Is::empty)"),
        new ValidationPattern(
            "!\\.isEmpty\\s*\\(\\s*\\)",
            "Verify.Is::notEmpty",
        ...
        new ValidationPattern(
            "\\.isPresent\\s*\\(\\s*\\)",
            "Verify.Is::some",
            "Use Verify.ensure(option, Verify.Is::some)"),
        new ValidationPattern(
            "\\.isEmpty\\s*\\(\\s*\\).*Option",
            "Verify.Is::none",
    ...
    private Stream<Diagnostic> findValidationPatterns(CstNode stmt, String source, LintContext ctx) {
        ...
        return PATTERNS.stream()
                       .filter(pattern -> Pattern.compile(pattern.regex()).matcher(stmtText).find())
